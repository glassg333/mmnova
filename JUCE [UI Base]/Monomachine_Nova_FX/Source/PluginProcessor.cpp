...
void MonomachineNovaAudioProcessor::prepareToPlay(double rate,int block){
    ...
    level.setCurrentAndTargetValue(global[Level]);mix.setCurrentAndTargetValue(isSynthVersion?1:global[Mix]/127);
...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    level.setTargetValue(global[Level]);mix.setTargetValue(isSynthVersion?1:global[Mix]/127);
    ...
    if(selected!=activeMachine){activeMachine=selected;machine.clear();chain.clear();std::array<float,8> p;std::copy_n(base.data(),8,p.data());machine.set(nova::machines()[static_cast<size_t>(selected)].id,p);
