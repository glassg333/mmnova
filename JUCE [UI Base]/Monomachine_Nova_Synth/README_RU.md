# MonomachineNova Unified Synth • v1.2.0

**15 режимов синтеза, единый пиксельный GUI по предоставленному образцу.**
Это исходники отдельного Projucer/JUCE-проекта. Название проекта — MonomachineNova,
имя VST3 — MonomachineNova Unified Synth. Скомпилированный Windows/macOS-бинарник в архив не входит.
...
cmake --build build --config Release --target MonomachineNovaUnifiedSynth_VST3
```
...
cmake --build build --config Release --target MonomachineNovaUnifiedSynth_VST3 --parallel 1
```

Результат: `build/MonomachineNovaUnifiedSynth_artefacts/Release/VST3/`.
...
Тесты: конфигурация `-DNOVA_BUILD_TESTS=ON`, сборка `MonomachineNovaUnifiedSynthTests`, затем `ctest --test-dir build -C Release --output-on-failure`.
