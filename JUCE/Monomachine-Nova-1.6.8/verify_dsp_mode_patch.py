...
def main() -> int:
    ...
    present(text, "case DspFilter: return \"FILT: mnm = \\xd0\\xbd\\xd0\\xb0\\xd1\\x81\\xd1\\x82\\xd0\\xbe\\xd1\\x8f\\xd1\\x89\\xd0\\xb8\\xd0\\xb9 \\xd1\\x84\\xd0\\xb8\\xd0\\xbb\\xd1\\x8c\\xd1\\x82\\xd1\\x80",
    ...
    present(text, "\\xd0\\xa0\\xd0\\x95\\xd0\\x97\\xd0\\x95\\xd0\\xa0\\xd0\\x92", "PluginEditor.cpp: резервные режимы помечены отдельной группой")
    ...
    present(text, "mnmDelay.process(params[27],feedback,send,pingPong,repitchSlew,dryL,dryR,wetL,wetR)",
            "1.6.5 NovaDSP: wet добавляется один раз (unity thru)")
    present(text, "void setRepitch(int raw)", "1.6.5 NovaDSP: настройка скорости repitch")
    ...
    text = read(data)
    present(text, "OFF|FAST|MED|SLOW", "1.6.5 NovaData: параметр dly_repitch (debug)")
    ...
    present(text, "BUILD 1.6.6", "1.6.5 Editor: маркер версии")
