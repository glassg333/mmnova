...
def main() -> int:
    ...
    present(text, "BUILD 1.6.23", "1.6.20 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.23", "1.6.23 Editor: версия в About (титул окна)")
    ...
    present(text, 'clearButton.onClick=[this,wipeField]{for(const char* f:{"on","src","dest","depth","mode","lock"})wipeField(f);repaint();}', "1.6.22 Editor: CLEAR ЛКМ стирает ВСЁ в дефолт (краш висячей ссылки исправлен)")
    ...
    present(text, "if(firstOfCat||i==hover)pixel::text(g,cat,{250,y,174,20},14,false); // 1.6.23", "1.6.23 Editor: пикер -- ДВЕ колонки + белое выделение (1.6.20)")
    ...
    present(text, "struct Cord { juce::Point<int> from,to; int src=0; }", "1.6.22 Editor: CableLayer хранит проложенные провода")
    present(text, "void rebuildCables()", "1.6.22 Editor: постоянные провода маршрутов (перерисовка только при изменении)")
    present(text, "b.rightClick=b.jackRightClick=[this,i]{cablesFocus=(cablesFocus==4+i)?-1:(4+i);rebuildCables();}", "1.6.22 Editor: ПКМ по кнопке LFO = показать/скрыть её провода")
    present(text, "b.rightClick=b.jackRightClick=[this,m]{const int src=m==0?10:m==1?16:17;", "1.6.22 Editor: ПКМ по кнопке MSEG = показать/скрыть её провода")
    ...
    present(text, "if(e.x>=end+24&&e.x<end+48){arrowClick(1);return;}", "1.6.22 Editor: хитбоксы стрелок машин по 24 px без перекрытия")
    ...
    present(text, "BUILD 1.6.23", "1.6.23 Editor: маркер версии сборки обновлен")
    present(text, "Build 1.6.23 (+1 every patch)", "1.6.23 Editor: версия в About")
    present(text, "if(!showCords||viewport.isVisible()||modDragSrc>=0||modMsegAim>0)", "1.6.23 Editor: провода ВЫКЛ по умолчанию, гаснут под страницами и прицелом")
    present(text, 'menu.addItem(10,"SHOW PATCH CORDS (experimental)",true,showCords);', "1.6.23 Editor: тумблер проводов в MENU")
    present(text, "if (wasDrag || r < 0 || c < 2 || !wasRmb) return;", "1.6.23 Editor: попап значений матрицы -- только ПКМ (ЛКМ = драг)")
    present(text, "const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; for (int i = 0; i < 9; ++i)", "1.6.23 Editor: DEPTH-список -100% ... 0% ... +100%")
    present(text, "const int r = rowAt(e.y), c = colAt(e.x); if (r < 0 || c < 1) return; auto* p = cell(r, c);", "1.6.23 Editor: двойной ЛКМ-клик по ячейке = сброс в 0")
