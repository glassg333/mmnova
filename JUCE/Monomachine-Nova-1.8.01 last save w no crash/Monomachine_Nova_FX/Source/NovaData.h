...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        for(int page=0;page<9;++page) for(int i=0;i<8;++i) { // 1.8.0: страницы 6..8 = LFO4-6
            juce::String choices;
            if(page>=3) {
                if(i==0) choices=page<6?"PTCH|SYNT|AMP|FILT|EFFX|LFO1|LFO2|LFO3":"PTCH|SYNT|AMP|FILT|EFFX|LFO4|LFO5|LFO6"; // 1.8.0c: как у LFO1 на P1 (просьба юзера): PTCH + группы P2 + сами LFO4-6
        ...
        for(int l2t=0;l2t<24;++l2t)targets+="|LFO"+juce::String(4+l2t/8)+" "+pageLabel(3+l2t/8,l2t%8); // 1.8.0c: цели LFO4-6 (164..187) // 1.8.0: AMP/FILT/FX как у P1; 15 = PORT не нужен -- прочерк
        for(int row=0;row<64;++row) { // 1.7.7: 64 слота матрицы
            ...
            s.push_back({prefix+"dest","Route target",0,188,0,1,targets}); // 1.7.8: цель = значение-1 (0 = OFF); 1.8.0c: 0..188 -- P2 132..163 + LFO4-6 164..187 (LFO FM убран из списка)
