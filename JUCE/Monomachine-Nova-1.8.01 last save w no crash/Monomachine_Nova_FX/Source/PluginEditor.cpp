...
static juce::String targetName(MonomachineNovaAudioProcessor& p,int target){
    if(target>=164)return "LFO"+juce::String(4+(target-164)/8)+" "+nova::pageLabel(3+(target-164)/8,(target-164)%8); // 1.8.0c: LFO4-6 (страницы P2), цели 164..187
...
public:
    ...
    juce::String choiceName(int value)const{
        if(pageIndex>=3&&knob==1){const int page=juce::jlimit(0,7,juce::roundToInt(processor.parameters.getRawParameterValue(nova::pageParam(pageIndex,0))->load()));
            if(pageIndex>=6){ // 1.8.0c: как у LFO1 на P1 (просьба юзера): PTCH|SYNT|AMP|FILT|EFFX|LFO4|LFO5|LFO6
                const int k2=juce::jlimit(0,7,value);
                if(page==0){const char* ranges2[]{"1ST","2ST","3ST","5ST","7ST","1OCT","2OCT","3OCT"};return ranges2[k2];} // PTCH
                if(page==1){const auto& fxm2=MonomachineNovaAudioProcessor::p2FxMachines();auto* mr2=processor.parameters.getRawParameterValue("p2_machine");
                    const int sel2=juce::jlimit(0,static_cast<int>(fxm2.size())-1,juce::roundToInt(mr2?mr2->load():0.0f));
                    return juce::String(fxm2[static_cast<size_t>(sel2)].synthParams[static_cast<size_t>(k2)].name);} // SYNT = руки машины P2
                if(page<=4)return targetName(processor,140+(page-2)*8+k2); // AMP/FILT/EFFX P2
                return targetName(processor,164+(page-5)*8+k2);} // LFO4-6
...
private:
    uint8_t modulationTarget() const noexcept {
        ...
        if (pageIndex >= 6) return static_cast<uint8_t>(164 + (pageIndex - 6) * 8 + juce::jlimit(0, 7, knob)); // 1.8.0c: LFO4-6 -- цели 164..187 (56..63 = MSEG OUT, было столкновение + запись за границей msegDests)
        if (pageIndex >= 0) return static_cast<uint8_t>(juce::jlimit(0, 55, pageIndex * 8 + knob + 8)); // 1.8.0: цели 8..55
        return 0;
    }
...
public:
    ...
    void refreshNames(){
        ...
        if(lfo>=3){ // 1.8.0c: как на P1 (просьба юзера): PTCH/SYNT = руки машины P2, AMP/FILT/EFFX = P2, LFO4-6 = их страницы
            for(int k=0;k<8;++k){
                if(cur<=1)choices.add(targetName(processor,132+k)); // PTCH и SYNT -- одни цели (руки P2), имена как в списках
                else if(cur<=4)choices.add(targetName(processor,140+(cur-2)*8+k));
                else choices.add(targetName(processor,164+(cur-5)*8+k));
            }
        }
...
private:
    ...
    void showDestList() { // список параметров: автоматизированные (слоты) сверху с индикацией, остальные по папкам матрицы
        ...
        m.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&slotBtn), [safe](int res) { if (!safe || res <= 0) return; // 1.7.11: якорь списка = слот-кнопка
            ...
            const int tg = res - 10; if ((tg >= 0 && tg <= 66) || tg == 131 || (tg >= 132 && tg <= 163)) safe->bindPlockTarget(safe->curSlot(), tg); }); } // 1.8.0b: +LFO FM и P2 (раньше цели >66 молча отбрасывались)
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        for (int r = 0; r < visibleRows; ++r) { // 1.7.9: рисуем только видимые строки
            ...
            { // 1.6.24: AUX SOURCE -- вторичный источник масштабирует глубину (Sylenth-style)
                static const char* auxNames[]{"OFF", "KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "MSEG2", "MSEG3", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM", "MSEG4", "MSEG5", "MSEG6", "MSEG7", "MSEG8", "P2 LFO4", "P2 LFO5", "P2 LFO6"}; // 1.6.25: порядок = ModSource // 1.8.0b FIX: было 19 -- MSEG4-8 читались за границей массива; +P2 LFO4-6
        ...
        if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; for (int i = 0; i < 9; ++i) menu.addItem(i + 1, signedPercent(values[i])); } // -100% ... 0% ... +100%
            ...
            { juce::PopupMenu mxSub; // 1.7.5/1.7.7: цели-маршруты 1..64: номер строки и то, что она модулирует (модулируют её ГЛУБИНУ)
              for (int n = 0; n < 64; ++n) { auto* dp = processor.parameters.getRawParameterValue("r" + juce::String(n) + "_dest");
                  ...
                  mxSub.addItem(69 + n, juce::String(n + 1) + " - " + (dv > 1 ? targetName(processor, juce::jlimit(0, 163, dv - 1)) : juce::String("OFF")), true, selected == 68 + n); } // 1.8.0b: P2-цели в ROUTE
            ...
            static const int vals[]{17, 0, 1, 7, 8, 9, 4, 5, 6, 23, 24, 25, 13, 14, 15, 16, 10, 11, 12, 18, 19, 20, 21, 22, 2, 3}; // 1.8.0b: +P2 LFO4-6 (23..25) в папке LFO
            const int selected = juce::roundToInt(par->convertFrom0to1(par->getValue()));
            ...
            juce::PopupMenu lfom; for (int i = 0; i < 6; ++i) lfom.addItem(7 + i, i < 3 ? sourceName(vals[6 + i]) : "P2 " + sourceName(vals[6 + i]), true, selected == vals[6 + i]); // 1.8.0b: +P2 LFO4-6
            juce::PopupMenu arp; for (int i = 0; i < 4; ++i) arp.addItem(13 + i, sourceName(vals[12 + i]), true, selected == vals[12 + i]);
            juce::PopupMenu msegm; for (int i = 0; i < 8; ++i) msegm.addItem(17 + i, sourceName(vals[16 + i]), true, selected == vals[16 + i]);
            juce::PopupMenu mac; for (int i = 0; i < 2; ++i) mac.addItem(25 + i, sourceName(vals[24 + i]), true, selected == vals[24 + i]);
    ...
    void openAuxMenu(int r, juce::Point<int> screen) { // 1.6.31: AUX = копия меню SOURCE + OFF первым (папки, тот же порядок)
        ...
        auto* par = processor.parameters.getParameter("r" + juce::String(r) + "_aux"); const int cur = par ? juce::jlimit(0, 26, juce::roundToInt(par->convertFrom0to1(par->getValue()))) : 0; // 1.8.0b: 0..26
        m.addItem(1, "OFF", true, cur == 0); m.addItem(2, "RANDOM", true, cur == 18); // 1.6.34: сепаратор-«пустая ячейка» после RANDOM убран
        static const int vals[]{17, 0, 1, 7, 8, 9, 4, 5, 6, 23, 24, 25, 13, 14, 15, 16, 10, 11, 12, 18, 19, 20, 21, 22, 2, 3}; // 1.8.0b: +P2 LFO4-6; 1.6.34: MIDI, LFO, ARP, MSEG, MACRO
        juce::PopupMenu midi; for (int i = 0; i < 5; ++i) midi.addItem(3 + i, auxNames[vals[1 + i] + 1], true, cur == vals[1 + i] + 1);
        juce::PopupMenu lfom; for (int i = 0; i < 6; ++i) lfom.addItem(8 + i, i < 3 ? auxNames[vals[6 + i] + 1] : juce::String("P2 ") + auxNames[vals[6 + i] + 1], true, cur == vals[6 + i] + 1); // 1.8.0b: +P2 LFO4-6
        juce::PopupMenu arp; for (int i = 0; i < 4; ++i) arp.addItem(14 + i, auxNames[vals[12 + i] + 1], true, cur == vals[12 + i] + 1);
        juce::PopupMenu msegm; for (int i = 0; i < 8; ++i) msegm.addItem(18 + i, auxNames[vals[16 + i] + 1], true, cur == vals[16 + i] + 1);
        juce::PopupMenu mac; for (int i = 0; i < 2; ++i) mac.addItem(26 + i, auxNames[vals[24 + i] + 1], true, cur == vals[24 + i] + 1);
        m.addSubMenu("MIDI", midi); m.addSubMenu("LFO", lfom); m.addSubMenu("ARP", arp); m.addSubMenu("MSEG", msegm); m.addSubMenu("MACRO", mac);
        const juce::Component::SafePointer<MatrixPage> safe(this);
        m.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this).withTargetScreenArea(juce::Rectangle<int>(screen.x, screen.y, 1, 1)), [safe, r](int result) { if (!safe || result <= 0) return; if (safe->pushEdit) safe->pushEdit(); if (auto* p = safe->processor.parameters.getParameter("r" + juce::String(r) + "_aux")) { static const int vals2[]{17, 0, 1, 7, 8, 9, 4, 5, 6, 23, 24, 25, 13, 14, 15, 16, 10, 11, 12, 18, 19, 20, 21, 22, 2, 3}; /* 1.8.0b: +P2 LFO4-6 */ const int v = result == 1 ? 0 : result == 2 ? 18 : vals2[result - 2] + 1; p->beginChangeGesture(); p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(v))); p->endChangeGesture(); if (v > 0) if (auto* d = safe->processor.parameters.getParameter("r" + juce::String(r) + "_aux_depth")) { const int cur = juce::roundToInt(d->convertFrom0to1(d->getValue())); if (cur == 0) { d->beginChangeGesture(); d->setValueNotifyingHost(d->convertTo0to1(63.0f)); d->endChangeGesture(); } } } }); // 1.7.10: как в Serum -- выбрал AUX = глубина сразу 100% (иначе маршрут молчал) // 1.6.43 FIX: был result-3 -- всё смещалось на один (KEY -> RANDOM, VEL -> KEY)
    }
...
private:
    ...
    static juce::String sourceName(int v) { const char* n[]{"KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "MSEG2", "MSEG3", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM", "MSEG4", "MSEG5", "MSEG6", "MSEG7", "MSEG8", "P2 LFO4", "P2 LFO5", "P2 LFO6"}; return n[juce::jlimit(0, 25, v)]; } // 1.8.0b: +P2 LFO4-6 // 1.6.25/29: 23 источника
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, public juce::Slider::Listener, private juce::Timer {
    ...
    explicit Surface(MonomachineNovaAudioProcessor& p):processor(p),machineButton(""),menuButton("MENU"),tempoButton(""),arpButton("ARP"),gateOuter(p,"arp_length",1,127,64),matrixButton("MATRIX"),lfo1Button("1"),lfo2Button("2"),lfo3Button("3"),closeButton("BACK"),gateButton(""){
        ...
        lfoJack.beginDrag=[this](bool matrix){beginModDrag(4+lfoPageSel,matrix);cableLayer.begin(getLocalPoint(nullptr,lfoJack.jackScreenCentre()));};
        ...
        for(auto* lfo:{&lfo1Button,&lfo2Button,&lfo3Button,&lfoJack})addAndMakeVisible(*lfo);
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    void bind(){
        ...
        lfo1Button.label="1";lfo2Button.label="2";lfo3Button.label="3"; // 1.8.0b: в P1 -- LFO 1 2 3
    ...
    void bindP2(){
        ...
        for(int i=3;i<8;++i)cells[5][static_cast<size_t>(i)]->bind(juce::String(),"---",-1,i,-1,false);
        rebuildModeControls();
        lfo1Button.label="4";lfo2Button.label="5";lfo3Button.label="6"; // 1.8.0b: в P2 -- LFO 4 5 6
    ...
    void resized()override{
        ...
        lfo1Button.setBounds(975,97,36,24);lfo2Button.setBounds(1015,97,36,24);lfo3Button.setBounds(1055,97,36,24); // 1.8.0b: кнопки страниц БЕЗ гнёзд
        lfoJack.setBounds(1095,97,44,24); // 1.8.0b: ОДИН патч-корд рядом с кнопками LFO 1 2 3 (4 5 6)
