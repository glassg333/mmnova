...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer {
    ...
    void showDspModes(int section){
        ...
        auto addSection=[&](int index){
            ...
            for(int m=0;m<monomachine::dspModeCount;++m){
                bool isCurrent = current==m;
                juce::String label = juce::String(monomachine::dspModeLabel(m)) + " - " + juce::String(monomachine::dspModeProvenanceForMode(index,m));
                sub.addItem(index*100+m+1, label, true, isCurrent);
            }
            sub.addSeparator();
            sub.addItem(-1, monomachine::dspModeProvenance(index), false, false);
            juce::String curLabel = monomachine::dspModeLabel(current);
            menu.addSubMenu(juce::String(monomachine::dspSectionLabel(index))+"   ["+curLabel+"]",sub);
        };
        ...
        juce::Component::SafePointer<Surface> safe(this);
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&dspModeButton).withMinimumWidth(520),[safe](int result){
    ...
    void updateDspModeLabel(){
        // FIXED: erase extra m letters - button now just says DSP MODE, no repeated m/o per section
        juce::String text="DSP MODE";
        dspModeButton.setButtonText(text);
        juce::String tip="DSP modes (all visible with explanations):\n";
        for(int i=0;i<monomachine::DspSectionCount;++i){
            int mode = processor.dspMode(i);
            tip+=juce::String(monomachine::dspSectionLabel(i))+": "+monomachine::dspModeLabel(mode)+" - "+monomachine::dspModeProvenanceForMode(i,mode)+"\n";
        }
