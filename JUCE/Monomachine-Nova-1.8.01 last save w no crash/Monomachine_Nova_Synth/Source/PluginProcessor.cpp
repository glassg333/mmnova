...
MonomachineNovaAudioProcessor::MonomachineNovaAudioProcessor():AudioProcessor(isSynthVersion?
    ...
    for(int l=0;l<6;++l){const juce::String pfx="lfo"+juce::String(l+1)+"_";const char* suf[]{"page_locks","dest_locks","page_solo","dest_solo"};for(int k=0;k<4;++k)lfoLockRaw[static_cast<size_t>(l*4+k)]=parameters.getRawParameterValue(pfx+suf[k]);}
    ...
    for(int p=0;p<9;++p)for(int i=0;i<8;++i)pageRaw[static_cast<size_t>(p)][static_cast<size_t>(i)]=parameters.getRawParameterValue(nova::pageParam(p,i)); // 1.8.0: +LFO4-6
...
void MonomachineNovaAudioProcessor::prepareToPlay(double rate,int block){
    ...
    for(int j=0;j<6;++j){lfoLastPage[static_cast<size_t>(j)]=juce::jlimit(0,7,juce::roundToInt(pageRaw[static_cast<size_t>(3+j)][0]->load()));lfoLastDest[static_cast<size_t>(j)]=juce::jlimit(0,7,juce::roundToInt(pageRaw[static_cast<size_t>(3+j)][1]->load()));}
...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    for(size_t p=0;p<6;++p)for(size_t i=0;i<8;++i){ // 1.8.0: LFO1-6 (p3..p8); LFO4-6 максимум те же
        const float value=pageRaw[p+3][i]->load();const float maximum=i==0||i==1?7.0f:i==2?3.0f:i==3?10.0f:i==4?6.0f:127.0f;effectiveLfoParams[p][i]=std::clamp(effectiveLfoParams[p][i]+value-lfoParams[p][i],0.0f,maximum);lfoParams[p][i]=value;}
    ...
    for(size_t r=0;r<64;++r){const int dvv=juce::jlimit(0,188,juce::roundToInt(routeRaw[r][2]->load())); // 1.8.0c: значение до 188 -- +LFO4-6 (164..187) 164 -- P2-цели 132..163 // 1.7.10 FIX: было 131 -- LFO FM (значение 132) резалось в ROUTE 64; цель = значение-1 = 0..131
        matrix.configureRouting(r,routeRaw[r][0]->load()>0.5f&&dvv>0,static_cast<monomachine::ModSource>(juce::jlimit(0,25,juce::roundToInt(routeRaw[r][1]->load()))),static_cast<uint8_t>(dvv>0?dvv-1:0),static_cast<int8_t>(juce::jlimit(-64,63,juce::roundToInt(routeRaw[r][3]->load()))),(juce::roundToInt(routeRaw[r][4]->load())==1?monomachine::ModPolarity::Bipolar:monomachine::ModPolarity::Unipolar),static_cast<uint8_t>(auxRaw[static_cast<size_t>(r)]?juce::jlimit(0,18,juce::roundToInt(auxRaw[static_cast<size_t>(r)]->load())):0),static_cast<int8_t>(juce::jlimit(-64,63,auxDepthRaw[static_cast<size_t>(r)]?juce::roundToInt(auxDepthRaw[static_cast<size_t>(r)]->load()):0)));} // 1.8.0: кламп источников 0..25 (LFO4-6)
...
void MonomachineNovaAudioProcessor::trigger(int note,float vel,int channel){
    ...
    for(size_t i=0;i<3;++i){lfos[i].trigger(effectiveLfoParams[i]);lfos2[i].trigger(effectiveLfoParams[3+i]);} // 1.8.0: +LFO4-6
...
void MonomachineNovaAudioProcessor::render(juce::AudioBuffer<float>& buffer,int start,int length){
    ...
    while(length>0){
        ...
        std::array<float,24> lfo2{}; // 1.8.0c: цели LFO4-6 (164..187) -- базис lfoParams[3..5] + маршруты/страницы LFO P2
        for(size_t j=0;j<3;++j)std::copy(lfoParams[j].begin(),lfoParams[j].end(),before.begin()+32+j*8);
        for(size_t j=3;j<6;++j)std::copy(lfoParams[j].begin(),lfoParams[j].end(),lfo2.begin()+(j-3)*8); // 1.8.0c: базис LFO4-6 -- раньше читали ЗА границей after[56..79]
        matrix.setLfoOutputs(previousLfo[0],previousLfo[1],previousLfo[2]);matrix.setLfoOutputs6(previousLfo.data(),6); // 1.8.0
        std::array<float,8> msegMod{};std::array<float,3> arpMod{}; // 1.7.5: [2] = PITCH
        ...
        matrix.evaluate(before,after,msegMod.data(),arpMod.data(),routeExtra.data(),lfoFm.data(),before2.data(),after2.data(),lfo2.data()); // 1.8.0c: +lfo2 // 1.7.1: +цели ARP; 1.7.7: +ROUTE; 1.7.8: +LFO FM; 1.8.0: +P2
        ...
        for(size_t j=3;j<6;++j){const auto& p=effectiveLfoParams[j];const float delta=previousLfo[j]*(p[7]-64); // 1.8.0: LFO4-6 -- PAGE = P1SY|P1AM|P1FI|P1FX|P2SY|P2AM|P2FI|P2FX
            int page=juce::jlimit(0,7,static_cast<int>(p[0])),dest=juce::jlimit(0,7,static_cast<int>(p[1]));
            if(lfoPageSolo[j]>0)page=juce::jlimit(0,7,lfoPageSolo[j]-1);
            else if(((lfoPageLocks[j]>>page)&1)!=0)page=lfoLastPage[j];else lfoLastPage[j]=page;
            if(lfoDestSolo[j]>0)dest=juce::jlimit(0,7,lfoDestSolo[j]-1);
            else if(((lfoDestLocks[j]>>dest)&1)!=0)dest=lfoLastDest[j];else lfoLastDest[j]=dest;
            // 1.8.0c: страницы P2 LFO = как у LFO1 на P1 (просьба юзера): PTCH|SYNT|AMP|FILT|EFFX|LFO4|LFO5|LFO6
            if(page==0){const float ranges[]{1,2,3,5,7,12,24,36};pitchMod+=delta/64*ranges[dest];} // PTCH -- вибрато питча, как на P1
            else if(page==1)after2[static_cast<size_t>(dest)]+=delta; // SYNT = руки машины P2
            else if(page<=4)after2[static_cast<size_t>((page-2)*8+8+dest)]+=delta; // AMP/FILT/EFFX P2 = after2[8..31]
            else lfo2[static_cast<size_t>((page-5)*8+dest)]+=delta; // LFO4-6 модулируют друг друга, как LFO1-3
        }
        for(size_t j=0;j<6;++j){std::array<float,8> effective{}; // 1.8.0: LFO1-6 (3..5 = P2)
            for(size_t k=0;k<8;++k){
                const float maximum=k==0||k==1?7.0f:k==2?3.0f:k==3?10.0f:k==4?6.0f:127.0f;
                effective[k]=std::clamp(j<3?after[32+j*8+k]:lfo2[(j-3)*8+k],0.0f,maximum);} // 1.8.0c: LFO4-6 из lfo2 (было чтение ЗА границей after[56..79] -- мусор в параметрах P2 LFO)
            previousLfo[j]=(j<3?lfos[j]:lfos2[j-3]).process(effective,sr,bpm,n);effectiveLfoParams[j]=effective;
        }
...
int MonomachineNovaAudioProcessor::addMsegRoute(int msegIndex,uint8_t target){if(msegIndex>=msegPageCount())setMsegPageCount(msegIndex+1); // 1.6.30: бинд активирует страницу; 1.6.34: возвращает слот
    const int destination=juce::jlimit(0,188,static_cast<int>(target)); // 1.7.10 FIX: было 130; 1.8.0c: цели 0..187 (+LFO4-6 164..187) -- было +P2 (132..163)
    int slot=-1;
...
// ---- 1.7.1: P-LOCK ---------------------------------------------------------
void MonomachineNovaAudioProcessor::plockSetTarget(int slot,int target){if(slot<0||slot>=kPlockSlots)return;plockTargetI[static_cast<size_t>(slot)].store(juce::jlimit(-1,211,target));} // 1.8.0c: цели до 211 (LFO4-6 164..187 хранятся, runtime их скипает)
...
float MonomachineNovaAudioProcessor::plockScaled(int t,int slot,float v) const {
    ...
    if(t==66)return pol==0?(v-1.0f)/126.0f*48.0f-24.0f:(v-64.0f)/63.0f*24.0f; // 1.7.10: PITCH -- полутона, UNI -24..+24 (64 = ноль), BIP-дельта
...
void MonomachineNovaAudioProcessor::plockStep(int page,int step){
    ...
    for(int s=0;s<kPlockSlots;++s){
        ...
        const int t=plockTargetI[static_cast<size_t>(s)].load();if(t<0||(t>66&&t<132)||t>163){R.active=false;R.sliding=false;continue;} // 1.7.10: 66 = PITCH; 1.8.0b: +P2 132..163; 1.8.0c: 164+ не пишут за границу plockOn[164]
...
int MonomachineNovaAudioProcessor::addRouteFromSource(int src, uint8_t target){
    const int source=juce::jlimit(0,25,src); // 1.6.25: 18 источников (MSEG2/3 больше не зажимаются в 15) // 1.8.0: 26 -- +LFO4-6
    const int destination=juce::jlimit(0,188,static_cast<int>(target)); // 1.7.10 FIX: было 130; 1.8.0c: цели 0..187 (+LFO4-6 164..187) -- было +P2 (132..163)
...
void MonomachineNovaAudioProcessor::sortModRoutes(int column, bool descending){
    ...
    for(int r=0;r<64;++r)for(int c=0;c<7;++c)if(auto* p=parameters.getParameter("r"+juce::String(r)+"_"+fields[c])){const float v=rows[static_cast<size_t>(r)].values[static_cast<size_t>(c)];if(std::abs(p->getValue()-v)>1e-6f)p->setValueNotifyingHost(v);} // 1.6.34: пишем ВСЕ 7 полей (aux/aux_depth больше не теряются)
