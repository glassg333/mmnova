...
private:
    ...
    void showDestList() { // список параметров: автоматизированные (слоты) сверху с индикацией, остальные по папкам матрицы
        ...
        { juce::PopupMenu sub; // 1.8.0e: папка P2 LFO в списке P-LOCK (164..187, 212..235)
            for (int k = 0; k < 24; ++k) { const int tg = 164 + k; sub.addItem(400 + tg, targetName(processor, tg), true, processor.plockTarget(curSlot()) == tg); }
            for (int k = 0; k < 24; ++k) { const int tg = 212 + k; sub.addItem(400 + tg, targetName(processor, tg), true, processor.plockTarget(curSlot()) == tg); }
...
public:
    ...
    void openValueMenu(int r, int c, juce::Point<int> screen) { // 1.6.24: единый список значений ячейки
        ...
        if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; for (int i = 0; i < 9; ++i) menu.addItem(i + 1, signedPercent(values[i])); } // -100% ... 0% ... +100%
            ...
            { juce::PopupMenu sub; // 1.8.0e: папка P2 LFO -- P2 LFO1-3 (164..187) + P2 LFO4-6 (212..235)
                for (int k = 0; k < 24; ++k) { const int target = 164 + k; sub.addItem(400 + target, targetName(processor, target), true, selected == target + 1); }
                for (int k = 0; k < 24; ++k) { const int target = 212 + k; sub.addItem(400 + target, targetName(processor, target), true, selected == target + 1); }
