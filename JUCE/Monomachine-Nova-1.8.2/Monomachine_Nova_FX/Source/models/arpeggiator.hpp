#pragma once
#include <array>
#include <atomic> // 1.7.9: uiRnd-атомики для призрака PAGE RND
#include <cstddef>
#include <algorithm>
#include <string>
#include <vector>
...
public:
    ...
    struct ArpSettings {
        ...
        bool stepRnd = false; // 1.7.1: STEP RND -- случайный ПОРЯДОК степов страницы (очередь-мешок)
        // 1.7.7: PAGE RND -- transient-паттерн (не пишет параметры пользователя)
        struct RndStep { bool hold = false; int8_t transpose = 0; uint8_t velocity = 127; };
        RndStep rndSteps[8] {};
        bool autoSwap = false; // 1.6.42: AUTO SWAP PAGE -- цепочка страниц в пределах лимита
    ...
    void setSettings(const ArpSettings& s) {
        ...
        settings.stepRnd = s.stepRnd; if (!s.stepRnd) bagValid = false; // 1.7.1: STEP RND
        rndPage = 255; // 1.7.7: смена настроек -- паттерн перегенерится
    }
    ...
    ArpNoteEvent poll() {
        ...
        if (settings.play == Play::Step || settings.play == Play::StepChord) {
            ...
            uint8_t playPage = settings.stepPage; // 1.7.7: страница играет выбранная (рандомится СОДЕРЖИМОЕ шагов -- как старый рабочий STEP RND)
            if (settings.stepRandom > 0.5f && (newPass || playPage != rndPage)) { // каждый новый проход -- новый случайный паттерн 8 шагов (С ПИТЧЕМ)
                rndPage = playPage;
                for (auto& rs : settings.rndSteps) {
                    rng ^= rng << 13; rng ^= rng >> 17; rng ^= rng << 5;
                    rs.transpose = static_cast<int8_t>(static_cast<int>(rng % 25u) - 12); // +-1 октава
                    rs.velocity = static_cast<uint8_t>(1u + rng % 127u);
                    rs.hold = (rng % 100u) < 25u; } }
            if (settings.autoSwap) playPage = static_cast<uint8_t>(std::min<uint16_t>(15u, static_cast<uint16_t>(playPage) + stepIndex / 8u)); // 1.6.42: AUTO SWAP -- 8 шагов на странице, дальше следующая, по кругу
            playingPage = playPage; // 1.6.42: FOLLOW PLAY
            stepIndex %= 8; // внутри страницы шаг всегда по порядку
            if (settings.stepRnd) { // 1.7.1: STEP RND -- мешок: все 8 степов в случайном порядке, пересобирается каждый проход
                if (newPass || !bagValid) { for (int i = 0; i < 8; ++i) bag[static_cast<size_t>(i)] = static_cast<uint8_t>(i);
                    for (int i = 7; i > 0; --i) { rng ^= rng << 13; rng ^= rng >> 17; rng ^= rng << 5; const size_t j = static_cast<size_t>(rng % static_cast<uint32_t>(i + 1)); const uint8_t tmp = bag[static_cast<size_t>(i)]; bag[static_cast<size_t>(i)] = bag[static_cast<size_t>(j)]; bag[static_cast<size_t>(j)] = tmp; }
                    bagValid = true; }
                stepIndex = bag[stepIndex]; }
            stepEcho = static_cast<uint8_t>(stepIndex);
            if (settings.stepRandom > 0.5f) { // 1.7.7: PAGE RND играет transient-паттерн
                const auto& rs = settings.rndSteps[stepIndex];
                uiRndT[static_cast<size_t>(stepIndex)].store(static_cast<float>(rs.transpose), std::memory_order_relaxed); uiRndV[static_cast<size_t>(stepIndex)].store(static_cast<float>(rs.velocity), std::memory_order_relaxed); // 1.7.9
                if (rs.velocity == 0) return ev;
                noteIndex = settings.play == Play::StepChord ? 0 : std::min(total - 1, static_cast<size_t>(sequenceStep % std::max<size_t>(1, total)));
                if (settings.play == Play::StepChord) { chordRemaining = std::min<size_t>(count, 3) - 1; chordIndex = 1; chordTranspose = rs.transpose; chordVelocity = rs.velocity; }
                const int transposed = static_cast<int>(pool[noteIndex % count]) + static_cast<int>(rs.transpose);
                ev.note = static_cast<uint8_t>(std::clamp(transposed, 0, 127));
                outputVelocity = velocityFor(rs.velocity, velocity[pool[noteIndex % count]]);
                stepHold = rs.hold; }
            else { const auto& s = settings.steps[static_cast<size_t>(playPage) * 8 + stepIndex];
    ...
    uint8_t stepEcho = 0; uint8_t randomPage = 0; uint8_t playingPage = 0; uint8_t rndPage = 255; // 1.6.42 FOLLOW PLAY; 1.7.7: страница текущего PAGE RND-паттерна
    std::array<std::atomic<float>,8> uiRndT{}; std::array<std::atomic<float>,8> uiRndV{}; // 1.7.9: PAGE RND -- паттерн для UI (призрак в лейне)
...
private:
    ...
    std::array<uint8_t, 8> bag{}; bool bagValid = false; double modRateOct = 0.0; float modGateAdd = 0.0f; // 1.7.1
