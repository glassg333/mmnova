...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        static const float kChorusDef[8]={70,95,41,68,127,127,127,127}; // 1.8.0b: панель FX-CHORUS юзера: DEL DEP SPD MIX FB WID LP INP
        ...
        for(int page=0;page<15;++page) for(int i=0;i<8;++i) { // 1.8.0e: +страницы 9..14
            juce::String choices;
            if(page>=3) {
                if(i==0) choices=page<9 // 1.8.0g: 0..10 = свои (PTCH + группы + ВСЕ 6 своих LFO), 11..20 = папка чужой страницы (её SYNT/AMP/FILT/EFFX + её 6 LFO)
                    ?"PTCH|SYNT|AMP|FILT|EFFX|LFO1|LFO2|LFO3|LFO4|LFO5|LFO6|P2 SYNT|P2 AMP|P2 FILT|P2 EFFX|P2 LFO1|P2 LFO2|P2 LFO3|P2 LFO4|P2 LFO5|P2 LFO6"
                    :"PTCH|SYNT|AMP|FILT|EFFX|P2 LFO1|P2 LFO2|P2 LFO3|P2 LFO4|P2 LFO5|P2 LFO6|P1 SYNT|P1 AMP|P1 FILT|P1 EFFX|P1 LFO1|P1 LFO2|P1 LFO3|P1 LFO4|P1 LFO5|P1 LFO6";
        ...
        for(int row=0;row<64;++row)s.push_back({"r"+juce::String(row)+"_aux","Route "+juce::String(row+1)+" aux source",0,32,0,1, // 1.7.10 FIX: было 16; 1.8.1 FIX: было 26 -- P2 LFO1-6 (значения 27..32) не влезали в диапазон
        ...
        // ===== 1.8.0: страница P2 -- полноценный FX-движок (p2_ прописка). Пресеты до 1.8 не мигрируем. =====
        {
            ...
            s.push_back({"p2_machine","P2 FX machine",0,static_cast<float>(fxm.size()-1),static_cast<float>(chorusIdx),1,p2names}); // 1.8.0b: дефолт CHORUS (индекс 1 попадал в REVERB)
        ...
        targets+="|--"; // 1.8.0c: LFO FM убран из списка (просьба юзера); слот держит нумерацию значений, чтобы старые P2-маршруты не съехали
        for(int p2t=0;p2t<32;++p2t){ // 1.8.0: цели P2 132..163 (SYNT/AMP/FILT/EFFX страницы 2)
            const int sec=(p2t-8)/8;targets+=juce::String("|P2 ")+(p2t==15?"AMP MIX":p2t<8?"SYN "+juce::String(p2t+1):juce::String(sec==0?"AMP ":sec==1?"FILT ":"FX ")+pageLabel(sec,(p2t-8)%8));}
        for(int l2t=0;l2t<24;++l2t)targets+="|P2 LFO"+juce::String(1+l2t/8)+" "+pageLabel(9+l2t/8,l2t%8); // 1.8.0e: 164..187 = строки P2 LFO1-3
        for(int l3t=0;l3t<24;++l3t)targets+="|"+juce::String(sections[6+l3t/8])+" "+pageLabel(6+l3t/8,l3t%8); // 1.8.0e: 188..211 = строки LFO4-6 (P1)
        for(int l4t=0;l4t<24;++l4t)targets+="|"+juce::String(sections[12+l4t/8])+" "+pageLabel(12+l4t/8,l4t%8); // 1.8.0e: 212..235 = строки P2 LFO4-6 // 1.8.0: AMP/FILT/FX как у P1; 15 = PORT не нужен -- прочерк
        for(int row=0;row<64;++row) { // 1.7.7: 64 слота матрицы
            ...
            s.push_back({prefix+"src","Route source",0,31,0,1,"KEY|VEL|MACRO X|MACRO Y|LFO1|LFO2|LFO3|PITCH WHL|MOD WHL|AFTERTOUCH|MSEG1|MSEG2|MSEG3|STEP VEL|STEP TRANS|ARP GATE|ARP RATE|RANDOM|MSEG4|MSEG5|MSEG6|MSEG7|MSEG8|LFO4|LFO5|LFO6|P2 LFO1|P2 LFO2|P2 LFO3|P2 LFO4|P2 LFO5|P2 LFO6"}); // 1.8.0e: 32 источника -- LFO4-6 теперь P1, +P2 LFO1-6 (26..31)ядок = ModSource; 1.6.29: 23 источника, MSEG4..8 = 18..22
