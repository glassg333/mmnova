...
public:
    ...
    void paintButton(juce::Graphics& g,bool /*over*/,bool down)override{
        ...
        if(arrows){ // 1.6.24/25: стрелки в ОТДЕЛЬНОЙ рамке; нажатая стрелка белеет, вторая не реагирует
            ...
            g.drawRect(end-3,cy-13,56,26,1);g.drawVerticalLine(end-3,cy-13,cy+13);
            for(int i=0;i<5;++i){
...
public:
    ...
    void paint(juce::Graphics& g)override{ // 1.6.25: как в прошлой версии -- всё содержимое видно, рамка только на текущей цели прицела, без мигания
        ...
        if(flashTicks>0){g.setColour(ink);g.drawRect(getLocalBounds().reduced(2),3);}
        if(pickMode&&hoverNow){g.setColour(knockout);g.drawRect(getLocalBounds().reduced(2),3);} // 1.6.25: подсветка модулируемой цели -- белая рамка
    }
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.25 / Synth":"BUILD 1.6.25 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
        ampButton.setBounds(473,95,120,28);ampModeCombo.setBounds(716,97,91,25); // 1.6.25: правее, до конца зоны заголовка AMP; ширина = последняя ручка (91)
        syntModeCombo.setBounds(317,97,91,25);filtModeCombo.setBounds(317,337,91,25);effxDistCombo.setBounds(568,337,91,25);effxDlyCombo.setBounds(713,337,91,25); // 1.6.6/1.6.25: ширина = последняя ручка страницы
        ...
        struct Picker final : juce::Component {
            ...
            void paint(juce::Graphics& g) override {
                ...
                for(int row=0;row<static_cast<int>(order.size());++row){
                    ...
                    const juce::String cat(m.category),fam(m.name);
                    const bool firstOfFam=(lastFamily!=fam);
                    ...
                    pixel::text(g,cat,{8,y,colW-14,20},14,false); // 1.6.25: колонки ПЕРЕСТАВЛЕНЫ -- тип слева (каждой строкой), семейство справа
                    if(firstOfFam||row==hover)pixel::text(g,fam,{colW+4,y,getWidth()-colW-10,20},14,false);
                    if(firstOfFam)lastFamily=fam;
