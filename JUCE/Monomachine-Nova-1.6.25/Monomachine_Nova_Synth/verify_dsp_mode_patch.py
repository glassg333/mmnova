...
def main() -> int:
    ...
    present(text, "BUILD 1.6.25", "1.6.25 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.25", "1.6.25 Editor: версия в About (титул окна)")
    ...
    present(text, "pixel::text(g,cat,{8,y,colW-14,20},14,false);", "1.6.25 Editor: пикер -- ТИП слева каждой строкой, семейство справа; SYNTH-машины первыми, FX в конце")
    ...
    present(text, "BUILD 1.6.25", "1.6.25 Editor: маркер версии сборки обновлен")
    present(text, "Build 1.6.25 (+1 every patch)", "1.6.25 Editor: версия в About")
    ...
    present(text, "if(flashTicks>0){g.setColour(ink);g.drawRect(getLocalBounds().reduced(2),3);}", "1.6.25 Editor: вспышка при отпускании шнура сохранена")
    present(text, "if(pickMode&&hoverNow){g.setColour(knockout);g.drawRect(getLocalBounds().reduced(2),3);}", "1.6.25 Editor: подсветка -- белая рамка ТОЛЬКО на текущей цели")
    ...
    present(text, "ampModeCombo.setBounds(716,97,91,25);", "1.6.25 Editor: AMP MODE правее; все MODE -- ширина 91 (последняя ручка)")
