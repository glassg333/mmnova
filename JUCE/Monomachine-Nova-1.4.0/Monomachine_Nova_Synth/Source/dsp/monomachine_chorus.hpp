// Nova integration: sample-rate-sized ring, bounded delay taps, allocation-free clear.
#pragma once
...
public:
    ...
    void reset(double sampleRate = 44100.0) {
        ...
        kMaxDelaySamples = static_cast<size_t>(std::ceil(m_sampleRate * 0.04)) + 4;
        m_bufferL.assign(kMaxDelaySamples, 0.0f);
        m_bufferR.assign(kMaxDelaySamples, 0.0f);
        m_writePos = 0;
        m_lfoPhase = 0.0f;
        setParameters(m_delParam, m_depParam, m_spdParam, m_mixParam, m_widParam);
    }
    ...
    void processStereo(const float* inL, const float* inR,
        ...
        for (size_t i = 0; i < numFrames; ++i) {
            ...
            float delayL = std::clamp(m_baseDelaySamples + m_modDepthSamples * modL, 1.0f, static_cast<float>(kMaxDelaySamples - 2));
            float delayR = std::clamp(m_baseDelaySamples + m_modDepthSamples * modR, 1.0f, static_cast<float>(kMaxDelaySamples - 2));
