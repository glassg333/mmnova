// Nova: sum before clamping; validate destination indices.
#pragma once
...
public:
    ...
    static constexpr size_t kTargetCount = 56;
    ...
    void evaluate(const std::array<T, N>& baseParams,
                  std::array<T, N>& modulatedParams) const noexcept {
        std::array<float, N> sums {};
        // Start from base parameters
        for (size_t i = 0; i < N; ++i) {
            sums[i] = static_cast<float>(baseParams[i]);
        }
        ...
        for (const auto& r : m_routings) {
            if (!r.enabled || r.targetParamId >= N || r.depth == 0) continue;
    ...
    static std::string getDestinationName(uint8_t paramId) {
        ...
        } else return "LFO"+std::to_string((paramId-32)/8+1)+" PARAM "+std::to_string((paramId-32)%8+1);
    }
