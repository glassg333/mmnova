...
void MonomachineNovaAudioProcessor::render(juce::AudioBuffer<float>& buffer,int start,int length){
    ...
    while(length>0){
        ...
        else{std::copy_n(buffer.getReadPointer(0,start),n,l.data());std::copy_n(buffer.getReadPointer(stereo?1:0,start),n,r.data());for(int i=0;i<n;++i)dryDelay.process(l[static_cast<size_t>(i)],r[static_cast<size_t>(i)],machine.fxLatency(),dryL[static_cast<size_t>(i)],dryR[static_cast<size_t>(i)]);if(machine.idOf()!=15||synthParams[3]>0.0001f)machine.render(l.data(),r.data(),n,false);} // 1.8.1b: CHORUS MIX=0 -- wet=0, рендер не слышен (CPU)
        ...
        { // ===== 1.8.0: P2 -- полноценный FX-движок (вход = выход P1) =====
            ...
            if(machine2.idOf()==0){/* GND = P2 выключен -- прозрачный проход */}
            else if(machine2.idOf()==15&&h2[3]<=0.0001f){/* 1.8.1b: CHORUS MIX=0 -- wet=0, рендер не слышен (CPU) */}
            else{
                ...
                const float mix2=std::clamp(p2MixRaw?p2MixRaw->load():127.0f,0.0f,127.0f)/127.0f;
