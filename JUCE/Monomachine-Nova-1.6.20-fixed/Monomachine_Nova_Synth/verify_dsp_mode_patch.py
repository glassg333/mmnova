...
def main() -> int:
    ...
    present(text, "BUILD 1.6.20", "1.6.20 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.20\"", "1.6.20 Editor: версия в About")
    ...
    absent(text, '"MODE AMP"', "1.6.15 Editor: мёртвый пункт AMP MODE удалён из сетки")
    ...
    present(text, "static_cast<float>(static_cast<int>(monomachine::ModSource::MSEG))", "1.6.19 Processor: addMsegRoute пишет src=MSEG")
    text = read(editor)
    present(text, "addMsegRoute(cell->targetId())", "1.6.19 Editor: MSEG OUT = прицел как у LFO (отпускание над ручкой = маршрут)")
    ...
    present(text, "std::round(px * 16.0f) / 16.0f", "1.6.19 Editor: SNAP к 1/16 в редакторе MSEG")
    ...
    present(text, "clearButton.onClick=[this]{for(int r=0;r<16;++r)", "1.6.20 Editor: CLEAR в матрице стирает все модуляции разом")
    ...
    present(text, "const bool showCat=firstOfCat||i==hover;", "1.6.20 Editor: тип машины слева от имени и на выделенной строке")
    ...
    present(text, "if(dragTint)fill=juce::Colour(0xffc8c8c8);", "1.6.20 Editor: кнопка, которую тянут, чуть серее")
    present(text, "if(e.getDistanceFromDragStart()<6) return; dragStarted=true;", "1.6.20 Editor: прицел LFO стартует только с реальным драгом (клик = выбор)")
