...
public:
    ...
    void processStereo(const float* inL, const float* inR,
        ...
        for (size_t i = 0; i < numFrames; ++i) {
            ...
            // Soft-clip the resonant signal the way the DSP56300 fixed-point stage overflows musically.
            if (m_resDrive > 1.001f) {
                left = std::tanh(left * m_resDrive) / std::tanh(m_resDrive);
                right = std::tanh(right * m_resDrive) / std::tanh(m_resDrive);
            }
...
private:
    ...
    void updateCoefficients() {
        ...
        // Envelope offsets are bipolar on hardware (-64..+63) and sweep with a wider throw than the base knob.
        float effectiveBase = static_cast<float>(m_baseParam) + m_envValue * (static_cast<float>(m_bofsParam) / 64.0f) * 96.0f;
        float effectiveWidth = static_cast<float>(m_widthParam) + m_envValue * (static_cast<float>(m_wofsParam) / 64.0f) * 96.0f;
        ...
        if (lpFreq < hpFreq * 1.05f) lpFreq = hpFreq * 1.05f; // never collapse the band below the high-pass

        // Resonance reaches self-oscillation near max, like the OS 1.32B ladder.
        float hpQ = 0.6f + std::pow(static_cast<float>(m_hpqParam) / 127.0f, 1.35f) * 28.0f;
        float lpQ = 0.6f + std::pow(static_cast<float>(m_lpqParam) / 127.0f, 1.35f) * 28.0f;
        ...
        // Resonance drive: high Q colours and slightly saturates on the Monomachine.
        m_resDrive = 1.0f + std::pow(std::max(static_cast<float>(m_hpqParam), static_cast<float>(m_lpqParam)) / 127.0f, 2.0f) * 3.0f;
    }
