...
public:
    ...
    static constexpr size_t kTargetCount = 132; // 1.7.8: 131 = LFO FM (октавы, до ~20 kHz); 1.7.7: 67..130 = ROUTE 1..64 (66) + ROUTE 1..16 (67..82) -- модуляция глубины соседних маршрутов
    ...
    void evaluate(const std::array<T, N>& baseParams, std::array<T, N>& modulatedParams, float* msegDests = nullptr, float* arpDests = nullptr, float* routeExtra = nullptr, float* lfoFm = nullptr) const noexcept { // 1.7.1: +arpDests; 1.7.8: +lfoFm (0=RATE,1=GATE,2=PITCH); 1.7.5: +routeExtra (прибавки к глубинам маршрутов)
        ...
        for (const auto& route : m_routings) {
            if (route.targetParamId >= 131) { // 1.7.8: LFO FM -- сумма в октавах, применяется к rate LFO/MSEG-страниц
