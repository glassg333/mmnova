...
public:
    explicit ArpPage(MonomachineNovaAudioProcessor& p) : processor(p), lane(p, [this] { return pageDisplay(); }, [this](int pg, int st, const char* f, float v) { setStepParameter(pg, st, f, v); }), // 1.6.42: FOLLOW PLAY
        ...
        slotBtn.setTooltip("P-LOCK crosshair: LMB = aim, then click a knob on the face panel -- its parameter locks to the steps of the current page (Elektron-style). The button shows the current parameter. LMB drag = cycle slots. RMB = parameter list.");
        ...
        slotBtn.onClick = [this] { if (juce::Time::getMillisecondCounter() - slotDraggedMs < 400) return; if (armRequest) armRequest(curSlot()); }; // 1.7.11 FIX: ЛКМ = ПРИЦЕЛ (крестик теперь рабочий: клик по ручке = цель слота); после драга прицел НЕ вызывается
    ...
    void resized() override {
        ...
        destBox->setBounds(670, 342, 160, 26); polBtn.setBounds(834, 342, 54, 26); // 1.7.11: armBtn убран
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
 ...
 // 1.6.30: новый стандарт -- список НИЖЕ своего хитбокса
    ...
    void rebuildModeControls(){
        ...
        ampModeLink=std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(processor.parameters,"amp_mode",ampModeCombo); // 1.6.21: движок огибающей выбирается здесь (не в DSP-меню)
