...
MonomachineNovaAudioProcessor::MonomachineNovaAudioProcessor():AudioProcessor(isSynthVersion?
    ...
    retrigRaw=parameters.getRawParameterValue("arp_retrig_sync"); // 1.6.24
...
void MonomachineNovaAudioProcessor::prepareToPlay(double rate,int block){
    ...
    machine.prepare(sr);chain.volumeReference=isSynthVersion?127.0f:64.0f;chain.prepare(sr);midiGateBlend.reset(sr,0.005);envelope.reset(sr);arp.reset(sr);matrix.reset();for(auto& m:mseg)m.reset();msegPhase={0.0,0.0,0.0};msegValue={0.0f,0.0f,0.0f};liveStepVelocity=0.0f;liveStepTranspose=0.0f;liveArpGate=0.0f;selectedStepVelocity=1.0f;selectedStepTranspose=0.0f;selectedStepHold=0.0f;
...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    for(size_t r=0;r<16;++r)matrix.configureRouting(r,routeRaw[r][0]->load()>0.5f,static_cast<monomachine::ModSource>(juce::jlimit(0,17,juce::roundToInt(routeRaw[r][1]->load()))),static_cast<uint8_t>(juce::jlimit(0,55,juce::roundToInt(routeRaw[r][2]->load()))),static_cast<int8_t>(juce::jlimit(-64,63,juce::roundToInt(routeRaw[r][3]->load()))),(juce::roundToInt(routeRaw[r][4]->load())==1?monomachine::ModPolarity::Bipolar:monomachine::ModPolarity::Unipolar),static_cast<uint8_t>(auxRaw[static_cast<size_t>(r)]?juce::jlimit(0,18,juce::roundToInt(auxRaw[static_cast<size_t>(r)]->load())):0)); // 1.6.24: + aux
...
void MonomachineNovaAudioProcessor::panic(){
    ...
    envelope.reset(sr);dryDelay.clear();machine.clear();chain.clear();arp.reset(sr,bpm);matrix.reset();msegPhase={0.0,0.0,0.0};msegValue={0.0f,0.0f,0.0f};liveStepVelocity=0.0f;liveStepTranspose=0.0f;liveArpGate=0.0f;selectedStepVelocity=1.0f;selectedStepTranspose=0.0f;selectedStepHold=0.0f;for(auto& l:lfos)l.reset();
}
void MonomachineNovaAudioProcessor::trigger(int note,float vel,int channel){
    soundingNote=note;soundingChannel=channel;
    msegPhase={0.0,0.0,0.0}; msegValue={mseg[0].value(0.0f),mseg[1].value(0.0f),mseg[2].value(0.0f)}; liveArpGate=1.0f;
...
void MonomachineNovaAudioProcessor::render(juce::AudioBuffer<float>& buffer,int start,int length){
    ...
    while(length>0){
        if(global[ArpMode]>0.5f){for(int k=0;k<3;++k){auto event=arp.poll();if(!event.triggered)break;if(event.isNoteOff){liveArpGate=0.0f;release();}else{liveStepVelocity=event.velocity/127.0f;liveStepTranspose=0.0f;liveArpGate=1.0f;trigger(event.note,event.velocity/127.0f,currentKey>=0?currentKey/128:soundingChannel);}}arpStepEcho.store(arp.stepEcho);}
        int n=std::min(8,length);if(global[ArpMode]>0.5f)n=std::min(n,std::max(1,arp.samplesUntilEvent()));
        msegValue[0]=mseg[0].value(static_cast<float>(msegPhase[0]));msegValue[1]=mseg[1].value(static_cast<float>(msegPhase[1]));msegValue[2]=mseg[2].value(static_cast<float>(msegPhase[2])); // 1.6.21
        matrix.setMsegOutputs(msegValue[0],msegValue[1],msegValue[2]);
        matrix.setPitchWheel((wheels[static_cast<size_t>(juce::jlimit(0,15,soundingChannel))]-8192.0f)/8192.0f);
        matrix.setModWheel(modWheels[static_cast<size_t>(juce::jlimit(0,15,soundingChannel))]/127.0f);
        matrix.setAftertouch(aftertouch[static_cast<size_t>(juce::jlimit(0,15,soundingChannel))]/127.0f);
        ...
        if(global[ArpMode]>0.5f){ // 1.6.24: RETRIG SYNC -- первый арп после тишины стартует ровно на следующей доле хоста
            bool anyHeld=false;for(bool h:arpHeld)if(h){anyHeld=true;break;}
            if(anyHeld&&!prevAnyHeld&&retrigRaw&&retrigRaw->load()>0.5f){
                if(auto* ph=getPlayHead())if(auto posn=ph->getPosition())if(auto tis=posn->getTimeInSamples())if(*tis>=0){
                    const double beatSamples=60.0/bpm*sr;const long long off=static_cast<long long>(beatSamples)-(static_cast<long long>(*tis)%static_cast<long long>(beatSamples));
                    arp.advance(-static_cast<int>(juce::jlimit(1,static_cast<int>(beatSamples),static_cast<int>(off))));}}
            prevAnyHeld=anyHeld;}
        if(global[ArpMode]>0.5f)arp.advance(n);
        const double msegRates[3]={std::max(0.001,static_cast<double>(global[MsegRate])*(global[MsegSync]>0.5f?bpm/60.0:1.0)),
            std::max(0.001,static_cast<double>(global[Mseg2Rate])*(global[Mseg2Sync]>0.5f?bpm/60.0:1.0)),
            std::max(0.001,static_cast<double>(global[Mseg3Rate])*(global[Mseg3Sync]>0.5f?bpm/60.0:1.0))}; // 1.6.21
        const bool msegLoops[3]={global[MsegLoop]>0.5f,global[Mseg2Loop]>0.5f,global[Mseg3Loop]>0.5f};
        for(int mi=0;mi<3;++mi){msegPhase[static_cast<size_t>(mi)]+=msegRates[mi]*static_cast<double>(n)/sr;
            if(msegPhase[static_cast<size_t>(mi)]>=1.0){if(msegLoops[mi])msegPhase[static_cast<size_t>(mi)]=std::fmod(msegPhase[static_cast<size_t>(mi)],1.0);else msegPhase[static_cast<size_t>(mi)]=1.0;}}
        start+=n;length-=n;
    }
}
...
void MonomachineNovaAudioProcessor::getStateInformation(juce::MemoryBlock& out){
    auto state=parameters.copyState();state.setProperty("schema",monomachine::kDspModeSchemaVersion,nullptr); // 1.6.24: ГЛАВНЫЙ фикс сохранений -- старая schema=8 запускала миграции (DSP-remap) при КАЖДОЙ загрузке пресета
    ...
    const char* msegTags[]{"MSEG","MSEG2","MSEG3"}; // 1.6.21: сохраняем все три (первая -- ещё и как старый "MSEG")
    for(int mi=0;mi<3;++mi){auto msegState=juce::ValueTree(msegTags[mi]);if(mi==0)msegState.setProperty("count",msegPointCount(0),nullptr);else msegState.setProperty("count",msegPointCount(mi),nullptr);
...
void MonomachineNovaAudioProcessor::setStateInformation(const void* data,int size){
    ...
    if(auto xml=getXmlFromBinary(data,size))if(xml->hasTagName(parameters.state.getType().toString())){
        ...
        loadMseg("MSEG",0);loadMseg("MSEG2",1);loadMseg("MSEG3",2);
        {const juce::ScopedLock lock(stateLock);pendingSamples=samples;}triggerAsyncUpdate();requestPanic();
    }
}
void MonomachineNovaAudioProcessor::addMsegRoute(int msegIndex,uint8_t target){
    ...
    set("r"+juce::String(slot)+"_src",static_cast<float>(msegIndex==0?static_cast<int>(monomachine::ModSource::MSEG):msegIndex==1?static_cast<int>(monomachine::ModSource::MSEG2):static_cast<int>(monomachine::ModSource::MSEG3))); // 1.6.21
...
void MonomachineNovaAudioProcessor::addRouteFromSource(int src, uint8_t target){
    const int source=juce::jlimit(0,15,src);
    const int destination=juce::jlimit(0,55,static_cast<int>(target));
    ...
    for(int r=0;r<16;++r){
        ...
        if(on&&dest&&srcPar&&on->load()>0.5f&&juce::roundToInt(srcPar->load())==source&&juce::roundToInt(dest->load())==destination){slot=r;break;}
    }
...
void MonomachineNovaAudioProcessor::moveModRoute(int from,int to){
    ...
    const char* fields[]={"on","src","dest","depth","mode","aux"}; // 1.6.24: + aux
...
void MonomachineNovaAudioProcessor::sortModRoutes(int column, bool descending){
    if(column<1||column>5)return; // 1.6.8: колонка 1 ("on") тоже сортируется
    const char* fields[]={"on","src","dest","depth","mode"};
    struct RouteValue{std::array<float,6> values{};float key=0,tie=0;}; // 1.6.24: + aux
    std::array<RouteValue,16> rows{};
    for(int r=0;r<16;++r){
        for(int c=0;c<6;++c){if(auto* p=parameters.getParameter("r"+juce::String(r)+"_"+fields[c]))rows[static_cast<size_t>(r)].values[static_cast<size_t>(c)]=p->getValue();}
    ...
    for(int r=0;r<16;++r)for(int c=0;c<5;++c)if(auto* p=parameters.getParameter("r"+juce::String(r)+"_"+fields[c])){const float v=rows[static_cast<size_t>(r)].values[static_cast<size_t>(c)];if(std::abs(p->getValue()-v)>1e-6f)p->setValueNotifyingHost(v);}
