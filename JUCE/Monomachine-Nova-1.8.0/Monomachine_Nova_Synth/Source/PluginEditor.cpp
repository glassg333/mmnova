...
static juce::String targetName(MonomachineNovaAudioProcessor& p,int target){
    if(target>=132)return "P2 "+targetName(p,target-132); // 1.8.0: цели второй страницы
...
public:
    ...
    juce::String choiceName(int value)const{
        if(pageIndex>=3&&knob==1){const int page=juce::jlimit(0,7,juce::roundToInt(processor.parameters.getRawParameterValue(nova::pageParam(pageIndex,0))->load()));
            if(pageIndex>=6)return targetName(processor,page<4?page*8+juce::jlimit(0,7,value):132+(page-4)*8+juce::jlimit(0,7,value)); // 1.8.0: PAGE у LFO4-6 без PTCH: 0..3 = P1SY..P1FX, 4..7 = P2SY..P2FX
...
private:
    uint8_t modulationTarget() const noexcept {
        ...
        if (pageIndex >= 0) return static_cast<uint8_t>(juce::jlimit(0, 79, pageIndex * 8 + knob + (pageIndex >= 0 ? 8 : 0))); // 1.8.0: 79 -- +LFO4-6 (56..79)
        return 0;
    }
...
private:
    juce::String destName(int id) const { if (id == 131) return "LFO FM"; if (id >= 132) return targetName(processor, id - 1); if (id >= 67 && id < 131) { auto* dp = processor.parameters.getRawParameterValue("r" + juce::String(id - 67) + "_dest"); const int dv = dp ? juce::roundToInt(dp->load()) : 0; return juce::String(id - 66) + " - " + (dv > 1 ? targetName(processor, juce::jlimit(0, 163, dv - 1)) : juce::String("OFF")); } return targetName(processor, id); } // 1.7.7: строки-маршруты 1..64 в DEST -- номер и то, что модулирует // 1.8.0: +P2-значения, кламп 0..163
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    void bindP2(){
        ...
        const int page=6+lfoPageSel; // LFO4/5/6 -- кнопки 1/2/3 в шапке переключают их в P2
        ...
        for(int i=0;i<8;++i){cells[4][static_cast<size_t>(i)]->bind("p2_2_"+juce::String(i),nova::pageLabel(2,i),2,i,-1,false);cells[4][static_cast<size_t>(i)]->setTargetOverride(156+i);} // P2 EFFX; DTIM/DSND-меню живут (repitch/PP общие)
        cells[5][0]->bind("mseg_rate","RATE",-1,0,-1,false);
        cells[5][1]->bind("mseg_sync","SYNC",-1,1,-1,false);
        cells[5][2]->bind("mseg_loop","LOOP",-1,2,-1,false);
        for(int i=3;i<8;++i)cells[5][static_cast<size_t>(i)]->bind(juce::String(),"---",-1,i,-1,false);
        rebuildModeControls();
        lfo1Button.selected=(lfoPageSel==0);lfo2Button.selected=(lfoPageSel==1);lfo3Button.selected=(lfoPageSel==2);
        repaint();
    }
    ...
    void beginModDrag(int src,bool matrix){modDragSrc=src;modDragMatrix=matrix;modHover=nullptr;lfoHoldIndex=-1;
        ...
        if(src>=4&&src<=6){auto* pp=processor.parameters.getParameter(nova::pageParam((p2View?6:3)+src-4,0));auto* dd=processor.parameters.getParameter(nova::pageParam((p2View?6:3)+src-4,1)); // 1.8.0
    ...
    void aimLfoDest(uint8_t target){
        if(modDragSrc<4||modDragSrc>6)return; // прицел только с кнопок LFO1/2/3 (в P2 -- LFO4/5/6)
        const int lfo=modDragSrc-4, page=p2View?6+lfo:3+lfo; // 1.8.0
        if(p2View){auto wr=[this](const juce::String& id,float value){if(auto* p=processor.parameters.getParameter(id)){p->beginChangeGesture();p->setValueNotifyingHost(p->convertTo0to1(value));p->endChangeGesture();}};
            const int t2=target>=132?target-132:target; // PAGE: 0..3 = P1SY..P1FX, 4..7 = P2SY..P2FX
            wr(nova::pageParam(page,0),static_cast<float>(t2/8+(target>=132?4:0)));wr(nova::pageParam(page,1),static_cast<float>(t2%8));return;} // 1.8.0
    ...
    void endModDrag(juce::Point<int> screen,bool dragged,int lfoIndex,bool matrix){
        ...
        if(dragged&&!matrix&&modDragSrc>=4){
            ...
            if(over>=0&&over!=lfoIndex){aimLfoDest(static_cast<uint8_t>((p2View?56:32)+over*8));lfoPageSel=juce::jlimit(0,2,over);bind();} // 1.8.0
        }
        ...
        if(dragged&&!matrix&&src>=4&&src<=6&&!landed&&overLfoButton(screen)<0&&aimBackupValid){
            ...
            set(nova::pageParam((p2View?6:3)+src-4,0),aimBackupPage);set(nova::pageParam((p2View?6:3)+src-4,1),aimBackupDest); // 1.8.0
    ...
    juce::String matrixRouteName(int r) const { // 1.7.5/1.7.7: динамическое имя маршрута (0..63)
        ...
        return v > 0 ? targetName(processor, juce::jlimit(0, 163, v - 1)) : juce::String("OFF"); } // 1.8.0: 0..163
