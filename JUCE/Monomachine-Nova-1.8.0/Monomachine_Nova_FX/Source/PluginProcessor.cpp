...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    for(size_t r=0;r<64;++r){const int dvv=juce::jlimit(0,164,juce::roundToInt(routeRaw[r][2]->load())); // 1.8.0: значение до 164 -- P2-цели 132..163 // 1.7.10 FIX: было 131 -- LFO FM (значение 132) резалось в ROUTE 64; цель = значение-1 = 0..131
...
void MonomachineNovaAudioProcessor::render(juce::AudioBuffer<float>& buffer,int start,int length){
    ...
    while(length>0){
        ...
        matrix.evaluate(before,after,msegMod.data(),arpMod.data(),routeExtra.data(),lfoFm.data(),before2.data(),after2.data()); // 1.7.1: +цели ARP; 1.7.7: +ROUTE; 1.7.8: +LFO FM; 1.8.0: +P2
        ...
        for(size_t j=3;j<6;++j){const auto& p=effectiveLfoParams[j];const float delta=previousLfo[j]*(p[7]-64); // 1.8.0: LFO4-6 -- PAGE = P1SY|P1AM|P1FI|P1FX|P2SY|P2AM|P2FI|P2FX
            ...
            if(page<4)after[static_cast<size_t>(page*8+dest)]+=delta;
            else after2[static_cast<size_t>((page-4)*8+dest)]+=delta;
        }
...
int MonomachineNovaAudioProcessor::addMsegRoute(int msegIndex,uint8_t target){if(msegIndex>=msegPageCount())setMsegPageCount(msegIndex+1); // 1.6.30: бинд активирует страницу; 1.6.34: возвращает слот
    const int destination=juce::jlimit(0,163,static_cast<int>(target)); // 1.7.10 FIX: было 130 -- LFO FM (131) резался в ROUTE 64; цели 0..163 // 1.8.0: +P2 (132..163)
    int slot=-1;
...
void MonomachineNovaAudioProcessor::plockStep(int page,int step){
    ...
    for(int s=0;s<kPlockSlots;++s){
        ...
        const int t=plockTargetI[static_cast<size_t>(s)].load();if(t<0||t>66){R.active=false;R.sliding=false;continue;} // 1.7.10: 66 = PITCH
