...
public:
    ...
    void evaluate(const std::array<T, N>& baseParams, std::array<T, N>& modulatedParams, float* msegDests = nullptr, float* arpDests = nullptr, float* routeExtra = nullptr, float* lfoFm = nullptr, T* base2 = nullptr, T* modulated2 = nullptr) const noexcept { // 1.8.0: +P2-цели 132..163 (base2/modulated2, 32 значения); 1.7.1: +arpDests; 1.7.8: +lfoFm; 1.7.5: +routeExtra
        ...
        for (const auto& route : m_routings) {
            ...
            if (route.targetParamId >= 132) { if (base2 != nullptr) sums2[static_cast<size_t>(route.targetParamId - 132)] += source * depth; continue; } // 1.8.0: P2
