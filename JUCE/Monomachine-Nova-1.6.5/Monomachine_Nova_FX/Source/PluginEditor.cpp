...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer {
    explicit Surface(MonomachineNovaAudioProcessor& p):processor(p),machineButton(""),menuButton("MENU"),tempoButton(""),arpButton("ARP"),arpSettingsButton("SET"),matrixButton("MATRIX"),lfo1Button("LFO1"),lfo2Button("LFO2"),lfo3Button("LFO3"),msegButton("MSEG"),msegOutputButton("MSEG OUT"),closeButton("BACK"),gateButton(""){
        ...
        for(auto* button:{&machineButton,&menuButton,&tempoButton,&arpButton,&arpSettingsButton,&matrixButton,&lfo1Button,&lfo2Button,&lfo3Button,&msegButton})addAndMakeVisible(*button);addAndMakeVisible(msegOutputButton);msegOutputButton.textHeight=12;msegOutputButton.setTooltip("Drag this external MSEG output point onto a Matrix target or parameter.");
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.5 / Synth / mnm modes":"BUILD 1.6.5 / FX / mnm modes",850,570,370,13,juce::Justification::right);
    }
    void resized()override{
    ...
    void showDspModes(int section){
        ...
        auto addSection=[&](int index){
            ...
            sub.addItem(-1,juce::String("ПАПКА: ")+monomachine::dspSectionCategory(index),false,false);
            ...
            if(monomachine::kShowBackupModes){
                ...
                if(backups.size()>0){
                    sub.addSeparator();
                    sub.addItem(-1,"РЕЗЕРВ (не основной):",false,false);
        ...
        if(section<0||section==monomachine::DspRouting){
            ...
            route.addItem(-1,"ЦЕПЬ ОДНА (ПРОВЕРЕНА), ПЕРЕКЛЮЧАТЕЛЬ УБРАН В 1.6.5",false,false);
            ...
            menu.addSubMenu("ЦЕПЬ / ROUTE   [fixed]",route);
        }
        juce::Component::SafePointer<Surface> safe(this);
    ...
    void updateDspModeLabel(){
        ...
        juce::String tip="DSP MODE -- папки разделов, основной режим mnm:\n";
        for(int i=0;i<monomachine::DspSectionCount;++i){
            ...
            tip+= (mode==monomachine::dspModeMnm) ? " (основной)" : " (резерв)";
