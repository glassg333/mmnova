...
void MonomachineNovaAudioProcessor::snapshot(){
    for(int i=0;i<monomachine::DspSectionCount;++i){
        ...
        // FIXED: ensure all modes visible but collapse inappropriate ones to mnm for stability
        // filter-only (cascade/dual/raw) allowed only for FILT, also allowed as spare for FILT
        if(i!=monomachine::DspFilter && monomachine::dspModeIsFilterOnly(idx)){
            idx = monomachine::dspModeMnm;
        }
        // dist2 allowed for DIST and FILTER (as spare filter mode)
        if((i!=monomachine::DspDist && i!=monomachine::DspFilter) && monomachine::dspModeIsDistOnly(idx)){
            idx = monomachine::dspModeMnm;
        }
        // fm2/bbox2 allowed for SYNT and FILTER (as spare)
        if((i!=monomachine::DspSynt && i!=monomachine::DspFilter) && monomachine::dspModeIsSyntOnly(idx)){
            idx = monomachine::dspModeMnm;
        }
