...
#include <array>
namespace nova::odinOracle {using fx::Frame;using fx::clean;
struct CombSettings {float freqA=220,resA=75,freqB=331,resB=75,balance=50,mix=100;int routing=0;bool negativeA=false,negativeB=false;};
class OdinCombLine {
public:
...
public:
 ...
 void set(CombSettings s){s.routing=std::clamp(s.routing,0,2);if(s.routing!=p.routing){for(auto& f:b)f.reset();}p=s;for(auto& f:a)f.set(s.freqA,s.resA,s.negativeA);for(auto& f:b)f.set(s.freqB,s.resB,s.negativeB);}
...
private:std::array<OdinCombLine,2> a,b;CombSettings p;float amount=1,balance=.5f,smoothing=.001f;
};
}
