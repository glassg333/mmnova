...
enum class ModSource : uint8_t {
    ...
    MSEG7,
    MSEG8,
    LFO4 = 23,    // 1.8.0e: LFO4-6 = СИНТОВЫЕ P1 (страницы 6..8); P2 LFO -- 26..31
    LFO5,
    LFO6,
    P2LFO1 = 26,  // 1.8.0e: P2 LFO1-6 (страницы 9..14)
    P2LFO2,
    P2LFO3,
    P2LFO4,
    P2LFO5,
...
public:
    ...
    void setLfoOutputs6(const float* v, int n) noexcept { for (int i = 0; i < n && i < 12; ++i) m_lfoOutputs[static_cast<size_t>(i)] = v[i]; } // 1.8.0e: 12 LFO
    ...
    void evaluate(const std::array<T, N>& baseParams, std::array<T, N>& modulatedParams, float* msegDests = nullptr, float* arpDests = nullptr, float* routeExtra = nullptr, float* lfoFm = nullptr, T* base2 = nullptr, T* modulated2 = nullptr, float* lfo2Dests = nullptr, float* lfo1bDests = nullptr, float* lfo2bDests = nullptr, float* msegRateDests = nullptr) const noexcept { // 1.8.0e: +lfo1bDests (P1 LFO4-6, 188..211), +lfo2bDests (P2 LFO4-6, 212..235) // 1.8.0: +P2-цели 132..163 (base2/modulated2, 32 значения); 1.7.1: +arpDests; 1.7.8: +lfoFm; 1.7.5: +routeExtra
        ...
        for (const auto& route : m_routings) {
            ...
            if (route.targetParamId >= 236) { if (msegRateDests != nullptr) msegRateDests[route.targetParamId - 236] += source * depth; continue; } // 1.8.1: MSEG1..8 RATE (236..243) -- модуляция скорости кривых
            if (route.targetParamId >= 212) { if (lfo2bDests != nullptr) lfo2bDests[route.targetParamId - 212] += source * depth; continue; } // 1.8.0e: P2 LFO4-6 (212..235)
            if (route.targetParamId >= 188) { if (lfo1bDests != nullptr) lfo1bDests[route.targetParamId - 188] += source * depth; continue; } // 1.8.0e: P1 LFO4-6 (188..211)
            if (route.targetParamId >= 164) { if (lfo2Dests != nullptr) lfo2Dests[route.targetParamId - 164] += source * depth; continue; } // 1.8.0c: P2 LFO1-3 (164..187)
    ...
    static std::string getDestinationName(uint8_t paramId) {
        if (paramId >= 212) { const int q = paramId - 212; return "P2_LFO" + std::to_string(q / 8 + 4) + "_PARAM_" + std::to_string(q % 8 + 1); } // 1.8.0e: P2 LFO4-6
        if (paramId >= 188) { const int q = paramId - 188; return "LFO" + std::to_string(q / 8 + 4) + "_PARAM_" + std::to_string(q % 8 + 1); } // 1.8.0e: P1 LFO4-6 (до рекурсии P2 -- иначе "P2 MSEG")
        if (paramId >= 164) { const int q = paramId - 164; return "P2_LFO" + std::to_string(q / 8 + 1) + "_PARAM_" + std::to_string(q % 8 + 1); } // 1.8.0e: P2 LFO1-3
...
private:
    float uniOf(ModSource source) const noexcept { // 1.6.25: центрированные источники (-1..1) -> 0..1, остальные уже 0..1
        switch (source) {
            ...
            case ModSource::LFO3: case ModSource::LFO4: case ModSource::LFO5: case ModSource::LFO6: case ModSource::P2LFO1: case ModSource::P2LFO2: case ModSource::P2LFO3: case ModSource::P2LFO4: case ModSource::P2LFO5: case ModSource::P2LFO6: case ModSource::PitchWheel: case ModSource::StepTranspose: case ModSource::Random:
    ...
    float sourceValue(ModSource source) const noexcept {
        switch (source) {
            ...
            case ModSource::LFO4:          return m_lfoOutputs[3]; // 1.8.0e: LFO4-6 = P1
            case ModSource::LFO5:          return m_lfoOutputs[4];
            case ModSource::LFO6:          return m_lfoOutputs[5];
            case ModSource::P2LFO1:        return m_lfoOutputs[6]; // 1.8.0e: P2 LFO1-6
            case ModSource::P2LFO2:        return m_lfoOutputs[7];
            case ModSource::P2LFO3:        return m_lfoOutputs[8];
            case ModSource::P2LFO4:        return m_lfoOutputs[9];
            case ModSource::P2LFO5:        return m_lfoOutputs[10];
            case ModSource::P2LFO6:        return m_lfoOutputs[11];
    ...
    std::array<float, 12> m_lfoOutputs{}; // 1.8.0: 6 LFO (3 = P1, 3 = P2)
};
