...
void MonomachineNovaAudioProcessor::prepareToPlay(double rate,int block){
    ...
    machine.prepare(sr);machine2.prepare(sr);chain.volumeReference=isSynthVersion?127.0f:64.0f;chain2.volumeReference=chain.volumeReference;chain.prepare(sr);chain2.prepare(sr);midiGateBlend.reset(sr,0.005);envelope.reset(sr);envelope2.reset(sr);arp.reset(sr);matrix.reset();for(auto& m:mseg)m.reset();msegPhase.fill(0.0);msegValue.fill(0.0f);for(auto& p:msegPhaseUi)p.store(0.0);liveStepVelocity=0.0f;liveStepTranspose=0.0f;liveArpGate=0.0f;selectedStepVelocity=1.0f;selectedStepTranspose=0.0f;selectedStepHold=0.0f;
    for(auto& l:lfos)l.reset();for(auto& l:lfos2)l.reset();notes.fill({});pedal.fill(false);wheels.fill(8192);arpHeld.fill(false);
    for(int j=0;j<12;++j){lfoLastPage[static_cast<size_t>(j)]=juce::jlimit(0,7,juce::roundToInt(pageRaw[static_cast<size_t>(3+j)][0]->load()));lfoLastDest[static_cast<size_t>(j)]=juce::jlimit(0,7,juce::roundToInt(pageRaw[static_cast<size_t>(3+j)][1]->load()));} // 1.8.0e
    currentKey=-1;serial=0;activeMachine=-1;lastArpMode=-1;ccX=ccY=-1;
    dryDelay.clear();dryDelay2.clear();previousLfo.fill(0);
...
const std::vector<monomachine::MachineDef>& MonomachineNovaAudioProcessor::p2FxMachines(){ // 1.8.0: FX-машины для страницы P2 (в обеих версиях плагина)
...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    {const int am2=juce::jlimit(0,2,p2AmpRaw[4]?juce::roundToInt(p2AmpRaw[4]->load()):1); // 1.8.0: P2 AMP mode
    ...
    for(size_t r=0;r<64;++r){const int dvv=juce::jlimit(0,244,juce::roundToInt(routeRaw[r][2]->load())); // 1.8.0e: значение до 236 -- все LFO-строки 164 -- P2-цели 132..163 // 1.7.10 FIX: было 131 -- LFO FM (значение 132) резалось в ROUTE 64; цель = значение-1 = 0..131
        matrix.configureRouting(r,routeRaw[r][0]->load()>0.5f&&dvv>0,static_cast<monomachine::ModSource>(juce::jlimit(0,31,juce::roundToInt(routeRaw[r][1]->load()))),static_cast<uint8_t>(dvv>0?dvv-1:0),static_cast<int8_t>(juce::jlimit(-64,63,juce::roundToInt(routeRaw[r][3]->load()))),(juce::roundToInt(routeRaw[r][4]->load())==1?monomachine::ModPolarity::Bipolar:monomachine::ModPolarity::Unipolar),static_cast<uint8_t>(auxRaw[static_cast<size_t>(r)]?juce::jlimit(0,32,juce::roundToInt(auxRaw[static_cast<size_t>(r)]->load())):0),static_cast<int8_t>(juce::jlimit(-64,63,auxDepthRaw[static_cast<size_t>(r)]?juce::roundToInt(auxDepthRaw[static_cast<size_t>(r)]->load()):0)));} // 1.8.1 FIX: клампы 0..31 (было 0..25 -- P2 LFO-источники молча становились LFO6) и aux 0..32 (было 0..18 -- LFO4-6/P2 LFO в AUX играли как RANDOM)
    ...
    for(int pg=0;pg<16;++pg)for(int st=0;st<8;++st){auto& stepValue=settings.steps[static_cast<size_t>(pg*8+st)]; // 1.8.1b: указатели кэшированы -- String только в конструкторе
...
void MonomachineNovaAudioProcessor::panic(){
    ...
    envelope.reset(sr);dryDelay.clear();machine.clear();chain.clear();envelope2.reset(sr);dryDelay2.clear();machine2.clear();chain2.clear();arp.reset(sr,bpm);matrix.reset();msegPhase.fill(0.0);msegValue.fill(0.0f);for(auto& p:msegPhaseUi)p.store(0.0);liveStepVelocity=0.0f;liveStepTranspose=0.0f;liveArpGate=0.0f;selectedStepVelocity=1.0f;selectedStepTranspose=0.0f;selectedStepHold=0.0f;for(auto& l:lfos)l.reset(); // 1.8.0: PANIC глушит и P2-движок (ENV/латентность/машина/цепь)
}
void MonomachineNovaAudioProcessor::trigger(int note,float vel,int channel){
    ...
    machine.on(note);machine2.on(note);if(nova::machines()[static_cast<size_t>(activeMachine)].id==7){lastBboxSample.store(machine.bbox.playingSlot());++bboxHitCounter;}envelope.on();envelope2.on();chain.trigger();chain2.trigger();velocity.setTargetValue(vel); // 1.8.0: +P2
...
void MonomachineNovaAudioProcessor::release(){envelope.off();chain.release();envelope2.off();chain2.release();} // 1.8.0: +P2
...
void MonomachineNovaAudioProcessor::render(juce::AudioBuffer<float>& buffer,int start,int length){
    ...
    while(length>0){
        if(global[ArpMode]>0.5f){for(int k=0;k<3;++k){auto event=arp.poll();if(!event.triggered)break;if(event.isNoteOff){liveArpGate=0.0f;release();}else{liveStepVelocity=event.velocity/127.0f;liveStepTranspose=0.0f;liveArpGate=1.0f;trigger(event.note,event.velocity/127.0f,currentKey>=0?currentKey/128:soundingChannel);plockStep(arp.playingPage,arp.stepEcho);}arpStepEcho.store(arp.stepEcho);arpPageEcho.store(arp.playingPage);for(int u=0;u<8;++u){arpUiRndT[u].store(arp.uiRndT[u].load(std::memory_order_relaxed),std::memory_order_relaxed);arpUiRndV[u].store(arp.uiRndV[u].load(std::memory_order_relaxed),std::memory_order_relaxed);}}} // 1.6.42: страница для FOLLOW PLAY
        ...
        std::array<float,8> msegMod{};std::array<float,3> arpMod{};std::array<float,8> msegRateMod{}; // 1.8.1: +MSEG RATE (236..243) // 1.7.5: [2] = PITCH
        ...
        {const int p2sel=juce::jlimit(0,static_cast<int>(p2FxMachines().size())-1,juce::roundToInt(p2MachineRaw?p2MachineRaw->load():0));
         ...
         for(size_t i=0;i<3;++i)before2[static_cast<size_t>(12+i)]=p2GainRaw[i]?p2GainRaw[i]->load():64.0f;before2[15]=0.0f; // 1.8.0: PORT на P2 нет (глайд -- P1); цель 147 -- прочерк
        ...
        matrix.evaluate(before,after,msegMod.data(),arpMod.data(),routeExtra.data(),lfoFm.data(),before2.data(),after2.data(),lfo2.data(),lfo1b.data(),lfo2b.data(),msegRateMod.data()); // 1.8.0e: +lfo1b/lfo2b; 1.8.1: +MSEG RATE // 1.7.1: +цели ARP; 1.7.7: +ROUTE; 1.7.8: +LFO FM; 1.8.0: +P2
        ...
        matrix.setMsegOutputs(msegValue.data(),8);
        float pitchMod=0;
        for(size_t j=0;j<12;++j){const auto& p=effectiveLfoParams[j];const float delta=previousLfo[j]*(p[7]-64); // 1.8.0g: единая таблица 12 LFO
            // 1.6.12: замки PAGE/DEST (маска байт -- свои цели 0..7; папка 11..20 не замкается).
            int page=juce::jlimit(0,20,static_cast<int>(p[0])),dest=juce::jlimit(0,7,static_cast<int>(p[1]));
            if(lfoPageSolo[j]>0)page=juce::jlimit(0,20,lfoPageSolo[j]-1); // 1.8.1: solo по всем 21 страницам
            ...
            const bool p2lfo=j>=6; // 0..5 = P1, 6..11 = P2
            if(page==0){const float ranges[]{1,2,3,5,7,12,24,36};pitchMod+=delta/64*ranges[dest];} // PTCH -- вибрато питча (все 12)
            else if(!p2lfo){
                if(page<=4)after[static_cast<size_t>((page-1)*8+dest)]+=delta; // SYNT/AMP/FILT/EFFX P1
                else if(page<=7)after[static_cast<size_t>(32+(page-5)*8+dest)]+=delta; // строки LFO1-3 (P1)
                else if(page<=10)lfo1b[static_cast<size_t>((page-8)*8+dest)]+=delta; // строки LFO4-6 (P1)
                else if(page<=14)after2[static_cast<size_t>((page-11)*8+dest)]+=delta; // папка P2: SYNT/AMP/FILT/EFFX
                else if(page<=17)lfo2[static_cast<size_t>((page-15)*8+dest)]+=delta; // папка P2: строки P2 LFO1-3
                else lfo2b[static_cast<size_t>((page-18)*8+dest)]+=delta; // папка P2: строки P2 LFO4-6
            }else{
                if(page==1)after2[static_cast<size_t>(dest)]+=delta; // SYNT = руки машины P2
                else if(page<=4)after2[static_cast<size_t>((page-2)*8+8+dest)]+=delta; // AMP/FILT/EFFX P2
                else if(page<=7)lfo2[static_cast<size_t>((page-5)*8+dest)]+=delta; // строки P2 LFO1-3
                else if(page<=10)lfo2b[static_cast<size_t>((page-8)*8+dest)]+=delta; // строки P2 LFO4-6
                else if(page<=14)after[static_cast<size_t>((page-11)*8+dest)]+=delta; // папка P1: SYNT/AMP/FILT/EFFX
                else if(page<=17)after[static_cast<size_t>(32+(page-15)*8+dest)]+=delta; // папка P1: строки LFO1-3
                else lfo1b[static_cast<size_t>((page-18)*8+dest)]+=delta; // папка P1: строки LFO4-6
            }
        }
        ...
        if(isSynthVersion){std::fill_n(l.data(),n,0.0f);std::fill_n(r.data(),n,0.0f);if(envelope.stage!=0)machine.render(l.data(),r.data(),n,true);}
            ...
            if(!(chorusIdle.load(std::memory_order_relaxed)&&machine.idOf()==15&&synthParams[3]<=0.0001f))machine.render(l.data(),r.data(),n,false);} // 1.8.3: простой хоруса только по галке MENU (дефолт ВЫКЛ -- хвосты как раньше)
        std::array<float,32> velNow{},gbNow{}; // 1.8.0: P2-гейт разделяет velocity и FX MIDI/LATCH-бленд
        for(int i=0;i<n;++i){float a=envelope.tick(),vel=velocity.getNextValue();const float gb=midiGateBlend.getNextValue();velNow[static_cast<size_t>(i)]=vel;gbNow[static_cast<size_t>(i)]=gb;amp[static_cast<size_t>(i)]=isSynthVersion?a*vel:1+(a*vel-1)*gb;}
        ...
        { // ===== 1.8.0: P2 -- полноценный FX-движок (вход = выход P1) =====
            const int p2sel=juce::jlimit(0,static_cast<int>(p2FxMachines().size())-1,juce::roundToInt(p2MachineRaw?p2MachineRaw->load():0));
            const auto& def2=p2FxMachines()[static_cast<size_t>(p2sel)];
            std::array<float,32> v2{}; // модулированные P2 (hands/amp/filt/fx)
            for(int i=0;i<32;++i)v2[static_cast<size_t>(i)]=after2[static_cast<size_t>(i)];
            ...
            chain2.set(c2);
            chain2.setModes(p2ModeRaw[0]?juce::jlimit(0,1,juce::roundToInt(p2ModeRaw[0]->load())):0,p2ModeRaw[1]?juce::jlimit(0,1,juce::roundToInt(p2ModeRaw[1]->load())):0,p2ModeRaw[2]?juce::jlimit(0,1,juce::roundToInt(p2ModeRaw[2]->load())):0,0);
            ...
            // 1.8.0c: P2 = вставка + DRY/WET (решение юзера: своя крутилка на слоте PORT)
            if(machine2.idOf()==0){/* GND = P2 выключен -- прозрачный проход */}
            else if(chorusIdle.load(std::memory_order_relaxed)&&machine2.idOf()==15&&h2[3]<=0.0001f){/* 1.8.3: простой хоруса только по галке MENU (дефолт ВЫКЛ); буфер живёт -- хвосты остаются */}
            else{
                std::array<float,32> d2L{},d2R{};for(int i=0;i<n;++i)dryDelay2.process(l[static_cast<size_t>(i)],r[static_cast<size_t>(i)],machine2.fxLatency(),d2L[static_cast<size_t>(i)],d2R[static_cast<size_t>(i)]); // сухой = выход P1, задержанный на латентность машины (фаза)
                machine2.render(l.data(),r.data(),n,false); // FX-машина P2 (in-place как у FX-версии)
                const float mix2=std::clamp(p2MixRaw?p2MixRaw->load():127.0f,0.0f,127.0f)/127.0f; // 1.8.3: порог раньше -- wet-конвейер неслышим при MIX=0
              for(int i=0;i<n;++i){const float a2=envelope2.stage!=0?envelope2.tick():0.0f;amp2[static_cast<size_t>(i)]=isSynthVersion?a2*velNow[static_cast<size_t>(i)]:1+(a2*velNow[static_cast<size_t>(i)]-1)*gbNow[static_cast<size_t>(i)];} // 1.8.0 FIX: в FX при LATCH без нот P2 молчал ВЕСЬ сигнал (amp2=0) // гейт входа P2: по release гасит всё, что до него (в т.ч. хвосты P1-эффектов)
              chain2.process(l.data(),r.data(),amp2.data(),n,false);
              for(int i=0;i<n;++i){const size_t x=static_cast<size_t>(i);l[x]=d2L[x]*(1.0f-mix2)+l[x]*mix2;r[x]=d2R[x]*(1.0f-mix2)+r[x]*mix2;} // DRY/WET: 0 = чистый P1, 127 = весь P2
            }
        }
...
int MonomachineNovaAudioProcessor::addMsegRoute(int msegIndex,uint8_t target){if(msegIndex>=msegPageCount())setMsegPageCount(msegIndex+1); // 1.6.30: бинд активирует страницу; 1.6.34: возвращает слот
    const int destination=juce::jlimit(0,244,static_cast<int>(target)); // 1.8.0e: цели 0..235 (+LFO-строки); 1.8.1: +MSEG RATE (236..243) -- было +P2 (132..163)
    int slot=-1;
...
int MonomachineNovaAudioProcessor::plockAcquire(int slot,int target){ // 1.7.2: та же цель = тот же слот; иначе этот если пуст; иначе первый с той же целью; иначе первый свободный
    if(target<0||target>82)return juce::jlimit(0,kPlockSlots-1,slot); // 1.7.5: цели до 82
...
float MonomachineNovaAudioProcessor::plockScaled(int t,int slot,float v) const {
    ...
    if(t>=236)return v; // 1.8.1c: MSEG RATE -- применяющий код ждёт СЫРОЕ значение (ABS центр 64, REL /32); раньше проваливался в MSEG OUT (0..1) -> постоянные -2 октавы    if(t==66)return pol==0?(v-1.0f)/126.0f*48.0f-24.0f:(v-64.0f)/63.0f*24.0f; // 1.7.10: PITCH -- полутона, UNI -24..+24 (64 = ноль), BIP-дельта
...
void MonomachineNovaAudioProcessor::plockStep(int page,int step){
    ...
    for(int s=0;s<kPlockSlots;++s){
        ...
        const int t=plockTargetI[static_cast<size_t>(s)].load();if(t<0||(t>66&&t<132)){R.active=false;R.sliding=false;continue;} // 1.7.10: 66 = PITCH; 1.8.0e: P2 (132..163) и все LFO-строки (164..235) играют
...
void MonomachineNovaAudioProcessor::plockFillPage(int slot,int page){ // 1.7.1: бинд сразу назначает степы страницы (текущее значение цели)
    ...
    else if(t==66)v=64.0f; // 1.7.10: PITCH -- 64 = ноль полутонов
...
void MonomachineNovaAudioProcessor::plockFromTree(const juce::ValueTree& pl){ // 1.7.3: восстановление из дерева; слот без записи = пустой (undo снимает слоты честно)
    for(int s=0;s<kPlockSlots;++s){
        ...
        plockTargetI[static_cast<size_t>(s)].store(juce::jlimit(-1,66,static_cast<int>(n.getProperty("target",-1)))); // 1.7.10: +PITCH (66)
...
int MonomachineNovaAudioProcessor::addRouteFromSource(int src, uint8_t target){
    const int source=juce::jlimit(0,31,src); // 1.8.1 FIX: было 0..25 -- прицел P2 LFO1-6 создавал маршрут с источником LFO6 // 1.6.25: 18 источников (MSEG2/3 больше не зажимаются в 15) // 1.8.0: 26 -- +LFO4-6
    const int destination=juce::jlimit(0,244,static_cast<int>(target)); // 1.8.0e: цели 0..235 (+LFO-строки); 1.8.1: +MSEG RATE (236..243) -- было +P2 (132..163)
