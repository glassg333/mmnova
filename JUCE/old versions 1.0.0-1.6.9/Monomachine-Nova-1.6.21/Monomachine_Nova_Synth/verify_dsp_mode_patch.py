...
def main() -> int:
    ...
    present(text, "BUILD 1.6.21", "1.6.20 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.21\"", "1.6.20 Editor: версия в About")
    ...
    # ---- 22. обещания 1.6.20
    text = read(editor)
    present(text, "const int end=getWidth()-40,cy=getHeight()/2;", "1.6.20 Editor: стрелки машин прижаты к правому краю окна (не скачут с текстом)")
    ...
    present(text, 'clearButton.onClick=[this,&wipeField]{for(const char* f:{"on","src","dest","depth","mode","lock"})wipeField(f);repaint();}', "1.6.21 Editor: CLEAR ЛКМ стирает ВСЕ поля маршрутов (включая замки) в дефолт")
    ...
    present(text, "pixel::text(g,shortName(i),{10,y,240,20},14,false); // 1.6.21: имя ВСЕГДА на своём месте", "1.6.21 Editor: имя машины не сдвигается; категория подъезжает только на hover-строке")
    ...
    present(text, "gateOuter.rightClick=[this]{showSettings(false);}", "1.6.21 Editor: ПКМ по GATE = страница ARP (не драг значения)")
