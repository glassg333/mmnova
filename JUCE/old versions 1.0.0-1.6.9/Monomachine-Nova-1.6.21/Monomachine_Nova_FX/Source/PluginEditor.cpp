...
public:
    CableLayer(){setInterceptsMouseClicks(false,false);}
    void begin(juce::Point<int> localFrom){on=true;from=localFrom;to=localFrom;setAlwaysOnTop(true);repaint();}
    ...
    void paint(juce::Graphics& g) override {
        ...
        g.setColour(juce::Colours::white);g.fillEllipse(float(to.x)-5.0f,float(to.y)-5.0f,10.0f,10.0f);
    }
...
public:
    ...
    void mouseDrag(const juce::MouseEvent& e) override { if(!leftDrag)return; const int dy = dragY - e.getScreenPosition().y; setValue(dragBase + dy / 8.0 * uiSpeed(speedKey)); }
...
public:
    ...
    void resized() override {
        ...
        stepRnd.setBounds(620, 195, 110, 28); pageRndToggle.setBounds(620, 227, 110, 26); // 1.6.21: PAGE RND под STEP RND
...
public:
    explicit MatrixPage(MonomachineNovaAudioProcessor& p) : processor(p) {
        ...
        auto wipeField=[this](const char* f){for(int r=0;r<16;++r)if(auto* p=processor.parameters.getParameter("r"+juce::String(r)+"_"+f)){p->beginChangeGesture();p->setValueNotifyingHost(p->convertTo0to1(p->getDefaultValue()));p->endChangeGesture();}};
...
private:
    ...
    int flashLockOnly = -1; int hoverLock = 0; int lockPaint = -1, lockPaintRow = -1; bool lockPaintWasRmb = false; // 1.6.21: кисть замков
...
public:
    ...
    void setPage(int m) {
        ...
        undo.clear(); redo.clear(); selected = -1; hoverSeg = -1; bendSeg = -1; repaint();
    }
    void paint(juce::Graphics& g) override {
        g.fillAll(juce::Colours::black); g.setColour(ink);
    ...
    void mouseUp(const juce::MouseEvent&) override { if (draggingPoint) pushUndo(); draggingPoint = false; bendSeg = -1; if (routeDrag >= 0) { if (auto* d = processor.parameters.getParameter("r" + juce::String(routeDrag) + "_depth")) d->endChangeGesture(); routeDrag = -1; } }
...
private:
    ...
    juce::ToggleButton loop, sync; juce::Slider rate, shift; juce::ComboBox shape, grid;
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    explicit Surface(MonomachineNovaAudioProcessor& p):processor(p),machineButton(""),menuButton("MENU"),tempoButton(""),arpButton("ARP"),gateOuter(p,"arp_length",1,127,64),matrixButton("MATRIX"),lfo1Button("1"),lfo2Button("2"),lfo3Button("3"),msego1("1"),msego2("2"),msego3("3"),closeButton("BACK"),gateButton(""){
        ...
        gateOuter.rightClick=[this]{showSettings(false);}; // 1.6.21: ПКМ по GATE = страница ARP
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.21 / Synth":"BUILD 1.6.21 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
        ...
        struct Picker final : juce::Component {
            ...
            void paint(juce::Graphics& g) override {
                ...
                for(int i=0;i<static_cast<int>(nova::machines().size());++i){
                    ...
                    pixel::text(g,shortName(i),{10,y,240,20},14,false); // 1.6.21: имя ВСЕГДА на своём месте
                    const juce::String cat(m.category);
                    if(i==hover)pixel::text(g,cat,{10+shortName(i).length()*12+8,y,174,20},14,false); // категория ВПЛОТНУЮ к имени на выделенной строке
                    else if(firstOfCat)pixel::text(g,cat,{250,y,174,20},14,false); // обычно -- в правой колонке напротив первой машины категории
