...
public:
    ...
    std::function<void()> openSamples;std::function<void(int)> openEnvelope;std::function<void(uint8_t)> addMsegModulation;std::function<void(uint8_t)> pickTarget;
...
private:
    ...
    MonomachineNovaAudioProcessor& processor; juce::ToggleButton arpEnable, arpHold, hostTempo; juce::TextButton randomButton; juce::ComboBox clock, mode, rateGrid, velocityMode, page; juce::Slider timeRate, octaves, gate, wrap, pageLimit, random; std::array<juce::TextButton, 8> stepRandom; std::array<juce::ToggleButton, 8> stepHold; std::array<juce::Slider, 8> stepTranspose, stepVelocity;
...
class BBoxLoopRange final : public juce::Component, private juce::Timer {
public:
    explicit BBoxLoopRange(MonomachineNovaAudioProcessor& p):processor(p){setMouseCursor(juce::MouseCursor::PointingHandCursor);startTimerHz(20);}
    ~BBoxLoopRange() override {stopTimer();endGesture();}
    void paint(juce::Graphics& g) override {
        g.fillAll(juce::Colours::black); const auto area=getLocalBounds().reduced(14,8); const int left=area.getX(), right=area.getRight(), y=area.getCentreY();
        g.setColour(ink.withAlpha(0.28f));g.fillRect(left,y-3,area.getWidth(),6);
        const int a=xFor(startValue),b=xFor(endValue);g.setColour(ink.withAlpha(0.45f));g.fillRect(a,y-4,std::max(1,b-a),8);
        g.setColour(ink);g.fillRect(a-3,y-14,6,28);g.fillRect(b-3,y-14,6,28);g.drawRect(area,1);
        pixel::text(g,"START "+juce::String(startValue*100.0f,1)+"%",{left,area.getBottom()-2,130,18},11,false);pixel::text(g,"END "+juce::String(endValue*100.0f,1)+"%",{right-130,area.getBottom()-2,130,18},11,false);
    }
    void resized() override {repaint();}
    void mouseDown(const juce::MouseEvent& e) override {sync();const int mouse=e.x;selected=(std::abs(mouse-xFor(startValue))<=std::abs(mouse-xFor(endValue)))?0:1;beginGesture();setFromMouse(mouse);}
    void mouseDrag(const juce::MouseEvent& e) override {setFromMouse(e.x);}
    void mouseUp(const juce::MouseEvent&) override {endGesture();selected=-1;}
private:
    juce::RangedAudioParameter* parameter(const char* id) const {return processor.parameters.getParameter(id);}
    float read(const char* id,float fallback) const {if(auto* p=parameter(id))return p->convertFrom0to1(p->getValue());return fallback;}
    int xFor(float value) const {const auto area=getLocalBounds().reduced(14,8);return area.getX()+juce::roundToInt(juce::jlimit(0.0f,1.0f,value)*static_cast<float>(area.getWidth()));}
    float valueFor(int x) const {const auto area=getLocalBounds().reduced(14,8);return juce::jlimit(0.0f,1.0f,static_cast<float>(x-area.getX())/std::max(1,area.getWidth()));}
    void sync(){startValue=read("m7_loop_start",0.0f);endValue=std::max(startValue+0.001f,read("m7_loop_end",1.0f));endValue=std::min(1.0f,endValue);}
    void setFromMouse(int x){if(selected<0)return;const float v=valueFor(x);if(selected==0)startValue=juce::jlimit(0.0f,endValue-0.001f,v);else endValue=juce::jlimit(startValue+0.001f,1.0f,v);const char* id=selected==0?"m7_loop_start":"m7_loop_end";if(auto* p=parameter(id))p->setValueNotifyingHost(p->convertTo0to1(selected==0?startValue:endValue));repaint();}
    void beginGesture(){if(gesture)return;gesture=true;for(const char* id:{"m7_loop_start","m7_loop_end"})if(auto* p=parameter(id))p->beginChangeGesture();}
    void endGesture(){if(!gesture)return;gesture=false;for(const char* id:{"m7_loop_start","m7_loop_end"})if(auto* p=parameter(id))p->endChangeGesture();}
    void timerCallback() override {if(!gesture){const float oldS=startValue,oldE=endValue;sync();if(oldS!=startValue||oldE!=endValue)repaint();}}
    MonomachineNovaAudioProcessor& processor;float startValue=0.0f,endValue=1.0f;int selected=-1;bool gesture=false;
};
...
public:
    explicit SamplePage(MonomachineNovaAudioProcessor& p):processor(p),loopRange(p),list("BBOX slots",this){
        addAndMakeVisible(loopRange);addAndMakeVisible(loopMode);loopMode.setButtonText("LOOP");loopMode.setTooltip("Enable the two-handle BBOX loop.");loopLink=std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment>(p.parameters,"m7_loop",loopMode);
    ...
    void paint(juce::Graphics& g)override{g.fillAll(juce::Colours::black);g.setColour(juce::Colours::white);pixel::text(g,"BBOX / 24 SLOTS",{8,4,560,24},20);pixel::text(g,"TWO-HANDLE LOOP",{8,40,220,20},13,true);}
    void resized()override{oldKit.setBounds(getWidth()-532,4,168,28);restore.setBounds(getWidth()-356,4,168,28);load.setBounds(getWidth()-180,4,168,28);loopMode.setBounds(getWidth()-220,38,100,27);loopRange.setBounds(8,60,getWidth()-16,62);list.setBounds(8,130,getWidth()-16,getHeight()-138);}
private:
    ...
    MonomachineNovaAudioProcessor& processor;BBoxLoopRange loopRange;juce::ToggleButton loopMode;std::unique_ptr<juce::AudioProcessorValueTreeState::ButtonAttachment> loopLink;juce::ListBox list;juce::TextButton load,restore,oldKit;std::unique_ptr<juce::FileChooser> chooser;
};
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer {
    ...
    PixelButton machineButton,menuButton,tempoButton,arpButton,arpSettingsButton,matrixButton,lfoButton,lfo3Button,msegButton;MsegOutputButton msegOutputButton;PixelButton closeButton,gateButton;juce::Slider level;
