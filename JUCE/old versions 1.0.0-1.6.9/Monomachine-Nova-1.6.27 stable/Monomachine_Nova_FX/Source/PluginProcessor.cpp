...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    settings.stepPage=static_cast<uint8_t>(juce::jlimit(0,15,juce::roundToInt(global[ArpStepPage])));settings.stepPageLimit=static_cast<uint8_t>(juce::jlimit(1,16,juce::roundToInt(global[ArpStepPageLimit])));settings.stepRandom=juce::jlimit(0.0f,1.0f,global[ArpStepRandom]/100.0f); // 1.6.25: строка была проглочена комментарием -- PAGE RND и выбор страницы не работали
...
void MonomachineNovaAudioProcessor::sortModRoutes(int column, bool descending){
    ...
    const char* fields[]{"on","src","dest","depth","mode","aux"}; // 1.6.25: + aux -- чтение c<6 раньше выходило за границы массива
