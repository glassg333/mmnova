...
def main() -> int:
    ...
    present(text, "BUILD 1.6.27", "1.6.27 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.27", "1.6.27 Editor: версия в About (титул окна)")
    ...
    present(text, "msegIndex==0?static_cast<int>(monomachine::ModSource::MSEG):msegIndex==1?static_cast<int>(monomachine::ModSource::MSEG2):static_cast<int>(monomachine::ModSource::MSEG3)", "1.6.21 Processor: addMsegRoute пишет src MSEG/MSEG2/MSEG3")
    text = read(editor)
    ...
    present(text, "SIGNED AMOUNT (INDEPENDENT OF PAGE / DEST / DPTH)", "1.6.19 Editor: маршруты MSEG со знаковой величиной")
    present(text, "const int den = grid.getSelectedItemIndex() == 0 ? 0 : grid.getSelectedItemIndex() == 1 ? 8 : grid.getSelectedItemIndex() == 2 ? 16 : 32;", "1.6.21 Editor: выбор СЕТКИ OFF/1/8/1/16/1/32 вместо SNAP")
    ...
    present(text, "WheelCombo shape, grid;", "1.6.22 Editor: SHAPE/GRID -- колесо")
    present(text, "if(e.x>=end){ // вся правая полоса -- стрелки, список не открыть", "1.6.24 Editor: полоса стрелок забронирована, стрелки в своей рамке")
    ...
    present(text, "BUILD 1.6.27", "1.6.27 Editor: маркер версии сборки обновлен")
    present(text, "Build 1.6.27 (+1 every patch)", "1.6.27 Editor: версия в About")
    ...
    present(text, 'n[]{"KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "MSEG2", "MSEG3", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM"}', "1.6.25 Editor: 18 источников в порядке ModSource (без трёх RANDOM)")
    ...
    present(text, "if(pickMode&&hoverNow){g.setColour(ink);g.drawRect(getLocalBounds().reduced(2),3);} // 1.6.26", "1.6.27 Editor: подсветка прицела -- БЕЛАЯ рамка только на текущей цели")
    present(text, "slider.setVisible(parameter&&!isChoice);repaint();} // 1.6.25", "1.6.25 Editor: ручка не прячется при прицеле")
    ...
    present(text, "machineButton.getScreenBounds().removeFromLeft(machineButton.getWidth()-56)", "1.6.25 Editor: список машин открывается у имени")
    ...
    present(text, "const int oy=(b.getHeight()-18)/2;", "1.6.25 Editor: галочки по центру высоты")
    present(text, "ampModeCombo.setBounds(762,97,91,25);", "1.6.26 Editor: AMP MODE ровно над последней крутилкой; ширина 91")
    ...
    text = read(root / "NovaData.h")
    present(text, '"KEY|VEL|MACRO X|MACRO Y|LFO1|LFO2|LFO3|PITCH WHL|MOD WHL|AFTERTOUCH|MSEG1|MSEG2|MSEG3|STEP VEL|STEP TRANS|ARP GATE|ARP RATE|RANDOM"', "1.6.25 Defaults: src -- 18 источников в порядке enum")
    absent(text, "arp_retrig_sync", "1.6.25 Defaults: arp_retrig_sync удалён")

    # ---- 28. обещания 1.6.26
    text = read(editor)
    present(text, "syntModeCombo.setBounds(363,97,91,25);", "1.6.26 Editor: SYNT MODE над последней крутилкой")
    present(text, "filtModeCombo.setBounds(363,337,91,25);", "1.6.26 Editor: FILT MODE над последней крутилкой")
    present(text, "effxDlyCombo.setBounds(666,337,91,25);", "1.6.26 Editor: второй мод EFFX (DLY) над предпоследней крутилкой")
    present(text, "effxDistCombo.setBounds(762,337,91,25);", "1.6.26 Editor: DIST над последней крутилкой EFFX")
    ...
    present(text, "const bool filled=(down&&outlined)||getToggleState();", "1.6.27 Editor: PixelButton отражает toggle (галка залита белым)")
    text = read(root / "NovaData.h")
    ...
    present(text, 'aux source",0,18,0,1', "1.6.24 Defaults: r*_aux (AUX SOURCE, OFF по умолчанию)")
    text = read(processor)
    ...
    present(text, '"Route target",0,55,0,1', "1.6.22 Defaults: цель маршрута (DEST) -- 0")
