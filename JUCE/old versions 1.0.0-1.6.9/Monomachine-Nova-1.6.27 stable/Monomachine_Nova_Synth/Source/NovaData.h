...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        for(int row=0;row<16;++row)s.push_back({"r"+juce::String(row)+"_aux","Route "+juce::String(row+1)+" aux source",0,18,0,1,
            "OFF|KEY|VEL|MACRO X|MACRO Y|LFO1|LFO2|LFO3|PITCH WHL|MOD WHL|AFTERTOUCH|MSEG1|MSEG2|MSEG3|STEP VEL|STEP TRANS|ARP GATE|ARP RATE|RANDOM"}); // 1.6.24/25: AUX SOURCE -- тот же порядок, что у SOURCE (плюс OFF)
        ...
        for(int row=0;row<16;++row) {
            ...
            s.push_back({prefix+"src","Route source",0,17,static_cast<float>(row%16),1,"KEY|VEL|MACRO X|MACRO Y|LFO1|LFO2|LFO3|PITCH WHL|MOD WHL|AFTERTOUCH|MSEG1|MSEG2|MSEG3|STEP VEL|STEP TRANS|ARP GATE|ARP RATE|RANDOM"}); // 1.6.25: 18 источников без дыры, порядок = ModSource
