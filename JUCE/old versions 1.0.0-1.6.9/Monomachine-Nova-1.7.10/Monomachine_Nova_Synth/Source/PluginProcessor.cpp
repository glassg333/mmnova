...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    for(size_t r=0;r<64;++r){const int dvv=juce::jlimit(0,132,juce::roundToInt(routeRaw[r][2]->load())); // 1.7.10 FIX: было 131 -- LFO FM (значение 132) резалось в ROUTE 64; цель = значение-1 = 0..131
...
void MonomachineNovaAudioProcessor::panic(){
    ...
    envelope.reset(sr);dryDelay.clear();machine.clear();chain.clear();arp.reset(sr,bpm);matrix.reset();msegPhase.fill(0.0);msegValue.fill(0.0f);for(auto& p:msegPhaseUi)p.store(0.0);liveStepVelocity=0.0f;liveStepTranspose=0.0f;liveArpGate=0.0f;selectedStepVelocity=1.0f;selectedStepTranspose=0.0f;selectedStepHold=0.0f;for(auto& l:lfos)l.reset();
}
...
void MonomachineNovaAudioProcessor::render(juce::AudioBuffer<float>& buffer,int start,int length){
    ...
    while(length>0){
        ...
        matrix.evaluate(before,after,msegMod.data(),arpMod.data(),routeExtra.data(),lfoFm.data()); // 1.7.1: +цели ARP; 1.7.7: +ROUTE; 1.7.8: +LFO FM
...
int MonomachineNovaAudioProcessor::addMsegRoute(int msegIndex,uint8_t target){if(msegIndex>=msegPageCount())setMsegPageCount(msegIndex+1); // 1.6.30: бинд активирует страницу; 1.6.34: возвращает слот
    const int destination=juce::jlimit(0,131,static_cast<int>(target)); // 1.7.10 FIX: было 130 -- LFO FM (131) резался в ROUTE 64; цели 0..131
    int slot=-1;
