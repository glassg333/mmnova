...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        for(int row=0;row<64;++row)s.push_back({"r"+juce::String(row)+"_aux","Route "+juce::String(row+1)+" aux source",0,23,0,1, // 1.7.10 FIX: было 16 -- у строк 16..63 AUX молча не работал (модуляция шла как на 100%)
