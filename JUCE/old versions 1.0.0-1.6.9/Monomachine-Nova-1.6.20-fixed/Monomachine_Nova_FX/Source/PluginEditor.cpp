...
public:
    ...
    void paintButton(juce::Graphics& g,bool /*over*/,bool down)override{
        ...
        if(arrows){ // 1.6.20: стрелки ПРИЖАТЫ к правому краю окна и не скачут вместе с текстом
            const int end=getWidth()-40,cy=getHeight()/2;
            for(int i=0;i<5;++i){
    ...
    void mouseDown(const juce::MouseEvent&e)override{
        ...
        if(arrows&&arrowClick){const int end=getWidth()-40; // 1.6.20: фиксированные хитбоксы у правого края
...
public:
    explicit MsegOutputButton(const juce::String& s):PixelButton(s){}
    std::function<void()> beginDrag;std::function<void(juce::Point<int>)> dragMove;std::function<void(juce::Point<int>,bool)> endDrag;
    void mouseDown(const juce::MouseEvent& e) override { PixelButton::mouseDown(e); if(beginDrag)beginDrag(); }
    void mouseDrag(const juce::MouseEvent& e) override { if(dragMove)dragMove(e.getScreenPosition()); }
    void mouseUp(const juce::MouseEvent& e) override { juce::TextButton::mouseUp(e); if(endDrag)endDrag(e.getScreenPosition(),e.mouseWasDraggedSinceMouseDown()); }
};
...
public:
    ...
    bool rightDrag=false,dragStarted=false,dragTint=false; // 1.6.20: состояние драга/прицела (видно Surface)
    // 1.6.20: кнопка выбранной страницы -- БЕЛАЯ; кнопку, которую тянут для
    // прицела, слегка притушить; прицел стартует ТОЛЬКО с реальным движением
    // (6 px) -- простой клик просто выбирает страницу.
    void paintButton(juce::Graphics& g,bool over,bool down) override {
        const bool sel=getToggleState();
        const bool blink=grayBlink&&blinkPhase();
        juce::Colour fill=sel?ink:juce::Colours::black;
        if(dragTint)fill=juce::Colour(0xffc8c8c8);
        if(blink)fill=juce::Colour(0xff8f8f8f);
        if(down&&outlined)fill=ink;
        g.fillAll(fill);
        if(outlined){g.setColour(ink);g.drawRect(getLocalBounds().reduced(1),1);}
        g.setColour((sel||dragTint||blink)&&!(down&&outlined)?knockout:ink);
        pixel::text(g,getButtonText(),getLocalBounds().reduced(5,0),textHeight,textAlign==0);
        juce::ignoreUnused(over);
    }
    void mouseDown(const juce::MouseEvent& e) override { PixelButton::mouseDown(e); dragStarted=false; rightDrag=e.mods.isRightButtonDown(); }
    void mouseDrag(const juce::MouseEvent& e) override {
    ...
    void mouseUp(const juce::MouseEvent& e) override { juce::TextButton::mouseUp(e); if(endDrag) endDrag(e.getScreenPosition(),dragStarted,rightDrag); dragStarted=false; stopTimer(); }
...
public:
    explicit MatrixPage(MonomachineNovaAudioProcessor& p) : processor(p) {
        ...
        clearButton.onClick=[this]{for(int r=0;r<16;++r)if(auto* p=processor.parameters.getParameter("r"+juce::String(r)+"_on")){p->beginChangeGesture();p->setValueNotifyingHost(p->convertTo0to1(0.0f));p->endChangeGesture();}repaint();};
...
public:
    explicit MsegPage(MonomachineNovaAudioProcessor& p) : processor(p) {
        ...
        shift.setTooltip("SHIFT: rotate the curve phase, degrees. Drag edits the shape destructively (double-click = 0).");
        ...
        shift.onValueChange = [this] { const float deg = static_cast<float>(shift.getValue()); restore(base); const float sh = deg / 360.0f;
            for (int i = 0; i < processor.msegPointCount(); ++i) { const float v = sampled(base, std::fmod(base[static_cast<size_t>(i)].first - sh + 1.0f, 1.0f)); processor.setMsegPoint(i, base[static_cast<size_t>(i)].first, v); } shiftLabel = juce::String(deg, 1) + " DEG"; repaint(); };
        ...
        const char* forms[]{"DRAW","FLAT","RISE","FALL","TRI","SINE","SQUARE"};
        for (const char* f : forms) { auto* b = formButtons.add(new PixelButton{f}); b->textHeight = 12; b->setBounds(0,0,10,10); addAndMakeVisible(b);
            b->onClick = [this, f] { if (!juce::String(f).contains("DRAW")) { pushUndo(); generate(f); shape.setText(f, juce::dontSendNotification); } }; }
        clearButton.setButtonText("clear"); clearButton.textHeight = 10; addAndMakeVisible(clearButton);
        clearButton.onClick = [this] { pushUndo(); processor.resetMseg(); shape.setText("CUSTOM", juce::dontSendNotification); };
        snap.setButtonText("SNAP"); snap.setClickingTogglesState(true); addAndMakeVisible(snap);
        snap.setTooltip("Snap point X to the 1/16 grid (hold Alt to bypass).");
        ...
        setSize(1170, 800); startTimerHz(20);
    }
    ...
    void paint(juce::Graphics& g) override {
        ...
        pixel::text(g, juce::String(processor.msegPointCount()) + " / " + juce::String(32) + " POINTS", {470, 10, 220, 18}, 13, false);
        pixel::text(g, "SHIFT", {520, 44, 60, 18}, 13, false);
        pixel::text(g, shiftLabel, {1060, 44, 100, 18}, 13, true);
        // сетка + рельсы с точками
        ...
        // точки: вертикали + ломаная + ручки
        const int n = processor.msegPointCount();
        for (int i = 0; i < n; ++i) { g.setColour(ink.withAlpha(0.30f)); g.drawVerticalLine(juce::roundToInt(pointToX(processor.msegPointX(i))), float(graphY), float(graphY + graphH)); }
        juce::Path path;
        for (int i = 0; i < n; ++i) { const float px = pointToX(processor.msegPointX(i)), py = pointToY(processor.msegPointY(i)); if (i == 0) path.startNewSubPath(px, py); else path.lineTo(px, py); }
        g.setColour(ink); g.strokePath(path, juce::PathStrokeType(2.0f));
        for (int i = 0; i < n; ++i) { const float px = pointToX(processor.msegPointX(i)), py = pointToY(processor.msegPointY(i));
            g.setColour(i == selected ? juce::Colours::black : juce::Colours::black); g.fillEllipse(px - 6.0f, py - 6.0f, 12.0f, 12.0f);
            g.setColour(ink); g.drawEllipse(px - 6.0f, py - 6.0f, 12.0f, 12.0f, i == selected ? 3.0f : 1.6f); }
        // значения выбранной точки
        if (selected >= 0 && selected < n) { pixel::text(g, "X " + juce::String(juce::roundToInt(processor.msegPointX(selected) * 128)), {20, toolY() + 4, 60, 20}, 14, false);
            pixel::text(g, "Y " + juce::String(juce::roundToInt(processor.msegPointY(selected) * 128)), {110, toolY() + 4, 60, 20}, 14, false); }
        ...
        for (int r = 0; r < 16 && row < 6; ++r) {
            ...
            if (!src || !dst || !on || on->load() < 0.5f || juce::roundToInt(src->load()) != static_cast<int>(monomachine::ModSource::MSEG)) continue;
            ...
            pixel::text(g, names[static_cast<size_t>(target)], {54, y, 330, 22}, 15, false);
            auto* depth = processor.parameters.getRawParameterValue("r" + juce::String(r) + "_depth");
            const float raw = depth ? depth->load() : 0.0f; // -64..63
            ...
            g.setColour(ink); g.drawRect(990, y + 2, 20, 18, 1); g.drawLine(995.0f, float(y + 7), 1005.0f, float(y + 15), 2.0f); g.drawLine(1005.0f, float(y + 7), 995.0f, float(y + 15), 2.0f); // [x]
        ...
        pixel::text(g, "DOUBLE CLICK: ADD / REMOVE   DRAG: MOVE / DRAW   ALT: BYPASS SNAP   RIGHT CLICK: REMOVE", {20, getHeight() - 24, 900, 17}, 12, false);
    }
    void resized() override {
        shape.setBounds(20, 40, 210, 26); shift.setBounds(590, 40, 460, 26);
        ...
        snap.setBounds(200, toolY(), 90, 26);
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        ...
        if (e.mods.isPopupMenu()) { if (near > 0 && near < processor.msegPointCount() - 1) { pushUndo(); processor.removeMsegPoint(near); selected = -1; shape.setText("CUSTOM", juce::dontSendNotification); } repaint(); return; }
        if (clicks >= 2) { if (near >= 0) { pushUndo(); if (near > 0 && near < processor.msegPointCount() - 1) processor.removeMsegPoint(near); selected = -1; }
            else { pushUndo(); float px = toX(e.x), py = toY(e.y); if (snap.getToggleState() && !e.mods.isAltDown()) px = std::round(px * 16.0f) / 16.0f; processor.addMsegPoint(px, py); selected = nearestPoint(e.position); } shape.setText("CUSTOM", juce::dontSendNotification); repaint(); return; }
        selected = near;
        if (selected < 0) { pushUndo(); float px = toX(e.x), py = toY(e.y); if (snap.getToggleState() && !e.mods.isAltDown()) px = std::round(px * 16.0f) / 16.0f; processor.addMsegPoint(px, py); selected = nearestPoint(e.position); }
    ...
    void mouseDrag(const juce::MouseEvent& e) override {
        ...
        const float x = toX(e.x), y = toY(e.y); float lo = selected == 0 ? 0.0f : processor.msegPointX(selected - 1) + 0.005f; float hi = selected == processor.msegPointCount() - 1 ? 1.0f : processor.msegPointX(selected + 1) - 0.005f;
        float px = juce::jlimit(lo, hi, x); if (snap.getToggleState() && !e.mods.isAltDown()) px = juce::jlimit(lo, hi, std::round(px * 16.0f) / 16.0f);
        processor.setMsegPoint(selected, px, y); shape.setText("CUSTOM", juce::dontSendNotification); repaint();
    }
    void mouseUp(const juce::MouseEvent&) override { if (draggingPoint) pushUndo(); draggingPoint = false; erasing = false; if (routeDrag >= 0) { if (auto* d = processor.parameters.getParameter("r" + juce::String(routeDrag) + "_depth")) d->endChangeGesture(); routeDrag = -1; } }
    void mouseMove(const juce::MouseEvent& e) override { const int near = nearestPoint(e.position); juce::Component::setMouseCursor(near >= 0 ? juce::MouseCursor::UpDownResizeCursor : juce::MouseCursor::CrosshairCursor); }
private:
    ...
    Points snapshot() const { Points pts; for (int i = 0; i < processor.msegPointCount(); ++i) pts.emplace_back(processor.msegPointX(i), processor.msegPointY(i)); return pts; }
    void restore(const Points& pts) { if (pts.size() < 2) return; processor.setMsegPointCount(static_cast<int>(pts.size())); for (size_t i = 0; i < pts.size(); ++i) processor.setMsegPoint(static_cast<int>(i), pts[i].first, pts[i].second); }
    float sampled(const Points& pts, float phase) const { if (pts.empty()) return 0.0f; if (phase <= pts.front().first) return pts.front().second;
        for (size_t i = 1; i < pts.size(); ++i) if (phase <= pts[i].first) { const float span = std::max(0.000001f, pts[i].first - pts[static_cast<size_t>(i) - 1].first); const float t = (phase - pts[static_cast<size_t>(i) - 1].first) / span; return pts[static_cast<size_t>(i) - 1].second + (pts[i].second - pts[static_cast<size_t>(i) - 1].second) * t; }
    ...
    void generate(const juce::String& what) {
        if (what == "FLAT") { processor.setMsegPointCount(2); processor.setMsegPoint(0, 0.0f, 0.5f); processor.setMsegPoint(1, 1.0f, 0.5f); }
        else if (what == "RISE") { processor.setMsegPointCount(2); processor.setMsegPoint(0, 0.0f, 0.0f); processor.setMsegPoint(1, 1.0f, 1.0f); }
        else if (what == "FALL") { processor.setMsegPointCount(2); processor.setMsegPoint(0, 0.0f, 1.0f); processor.setMsegPoint(1, 1.0f, 0.0f); }
        else if (what == "TRI") { processor.setMsegPointCount(3); processor.setMsegPoint(0, 0.0f, 0.0f); processor.setMsegPoint(1, 0.5f, 1.0f); processor.setMsegPoint(2, 1.0f, 0.0f); }
        else if (what == "SINE") { processor.setMsegPointCount(33); for (int i = 0; i < 33; ++i) { const float t = i / 32.0f; processor.setMsegPoint(i, t, 0.5f - 0.5f * std::cos(2.0f * juce::MathConstants<float>::pi * t)); } }
        else if (what == "SQUARE") { processor.setMsegPointCount(4); processor.setMsegPoint(0, 0.0f, 1.0f); processor.setMsegPoint(1, 0.5f, 1.0f); processor.setMsegPoint(2, 0.5f, 0.0f); processor.setMsegPoint(3, 1.0f, 0.0f); }
        selected = -1; repaint();
    }
    void tool(const juce::String& tl) {
        ...
        else if (tl == "INVERT") { pushUndo(); for (int i = 0; i < processor.msegPointCount(); ++i) processor.setMsegPoint(i, processor.msegPointX(i), 1.0f - processor.msegPointY(i)); shape.setText("CUSTOM", juce::dontSendNotification); }
        else if (tl == "FLIP X") { pushUndo(); const auto pts = snapshot(); for (size_t i = 0; i < pts.size(); ++i) processor.setMsegPoint(static_cast<int>(i), pts[pts.size() - 1 - i].first, pts[pts.size() - 1 - i].second); for (int i = 0; i < processor.msegPointCount(); ++i) processor.setMsegPoint(i, 1.0f - processor.msegPointX(i), processor.msegPointY(i)); shape.setText("CUSTOM", juce::dontSendNotification); }
    ...
    void timerCallback() override { repaint(); }
    MonomachineNovaAudioProcessor& processor; juce::ToggleButton loop, sync, snap; juce::Slider rate, shift; juce::ComboBox shape;
    PixelButton clearButton{"clear"}; juce::OwnedArray<PixelButton> formButtons, toolButtons;
    ...
    int selected = -1, routeDrag = -1, routeY0 = 0; float routeBase = 0; juce::String shiftLabel{"0.0 DEG"};
    bool draggingPoint = false, erasing = false;
    std::vector<Points> undo, redo; Points base, clipboard; std::array<int, 6> routeRows{{-1,-1,-1,-1,-1,-1}};
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    explicit Surface(MonomachineNovaAudioProcessor& p):processor(p),machineButton(""),menuButton("MENU"),tempoButton(""),arpButton("ARP"),gateOuter(p,"arp_length",1,127,64),matrixButton("MATRIX"),lfo1Button("LFO1"),lfo2Button("LFO2"),lfo3Button("LFO3"),msegButton("MSEG"),msegOutputButton("MSEG OUT"),closeButton("BACK"),gateButton(""){
        ...
        for(auto* button:{&machineButton,&menuButton,&tempoButton,&arpButton,&matrixButton,&msegButton})addAndMakeVisible(*button);addAndMakeVisible(gateOuter);gateOuter.setTooltip("GATE: arp note length 1..127 -- how much of each step the note holds. Drag or scroll. RMB on ARP opens the step editor."); // 1.6.20: SET больше не нуженfor(auto* lfo:{&lfo1Button,&lfo2Button,&lfo3Button})addAndMakeVisible(*lfo);addAndMakeVisible(msegOutputButton);msegOutputButton.textHeight=12;msegOutputButton.setTooltip("Drag this external MSEG output point onto a knob: release = matrix route to that target (aim like the LFO buttons)."); // 1.6.19
        msegOutputButton.beginDrag=[this]{for(auto& column:cells)for(auto& cell:column)if(cell)cell->setPickMode(true);}; // 1.6.19: прицел как у LFO
        msegOutputButton.dragMove=[this](juce::Point<int> s){auto* cell=cellAtScreen(s);if(cell!=modHover){if(modHover)modHover->setHover(false);modHover=cell;if(cell)cell->setHover(true);}};
        msegOutputButton.endDrag=[this](juce::Point<int> s,bool dragged){for(auto& column:cells)for(auto& cell:column)if(cell)cell->setPickMode(false);if(modHover)modHover->setHover(false);
            if(dragged){auto* cell=cellAtScreen(s);if(cell&&cell->canBeTarget()){processor.addMsegRoute(cell->targetId());cell->flashPick();}}
            modHover=nullptr;};
        ...
        auto wireLfo=[this](ModSourceButton& b,int i){
            ...
            b.setTooltip("LFO"+juce::String(i+1)+": click = show this LFO panel; drag onto a knob = aiming (starts only when you actually drag): LEFT = this LFO modulates the knob directly (its own PAGE/DEST), RIGHT = a matrix route. While aiming, hovering any LFO button -- including the one you came from -- instantly shows its page, and that button blinks gray.");
        };
    ...
    void timerCallback()override{
        ...
        if(modDragSrc<4){lfoHoldIndex=-1;lfo1Button.grayBlink=lfo2Button.grayBlink=lfo3Button.grayBlink=false;lfo1Button.dragTint=lfo2Button.dragTint=lfo3Button.dragTint=false;return;}
    ...
    void beginModDrag(int src,bool matrix){modDragSrc=src;modDragMatrix=matrix;modHover=nullptr;lfoHoldIndex=-1;
        if(src>=4&&src<=6)lfoButton(static_cast<int>(src)-4).dragTint=true; // 1.6.20: источник прицела чуть серее
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.20 / Synth":"BUILD 1.6.20 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
    ...
    void showMachineMenu(){
        // 1.6.19: список машин в двух НЕВИДИМЫХ колонках: слева генераторы,
        // справа тип генератора -- напротив ПЕРВОГО генератора категории.
        // Категория напротив первого генератора => высота списка почти вдвое меньше.
        struct Picker final : juce::Component {
            ...
            void paint(juce::Graphics& g) override {
                ...
                for(int i=0;i<static_cast<int>(nova::machines().size());++i){
                    ...
                    const bool showCat=firstOfCat||i==hover; // 1.6.20: тип слева от имени; на выделенной строке тоже показываем
                    if(i==hover){g.setColour(ink);g.fillRect(4,y-1,getWidth()-8,22);g.setColour(knockout);}
                    else g.setColour(i==bound?ink:ink);
                    int x=10;
                    if(showCat){ // 1.6.20: тип прижат к имени машины -- читается как одно слово (FM+STAT)
                        const juce::String cat(m.category);
                        pixel::text(g,cat,{x,y,cat.length()*12+4,20},14,false);
                        x+=cat.length()*12+8;
                    }
                    pixel::text(g,shortName(i),{x,y,getWidth()-x-8,20},14,true);
    ...
    void showDspModes(int section,juce::Point<int> screen=juce::Point<int>(-1,-1)){
        ...
        juce::PopupMenu menu; // 1.6.19: меню DSP -- обычный шрифт JUCE (по просьбе), остальные меню пиксельные
    ...
    PixelButton machineButton,menuButton,tempoButton,arpButton,matrixButton,msegButton;DragValueBox gateOuter;ModSourceButton lfo1Button,lfo2Button,lfo3Button;MsegOutputButton msegOutputButton;PixelButton closeButton,gateButton;juce::Slider level;PixelButton dspModeButton{"DSP MODE"};
