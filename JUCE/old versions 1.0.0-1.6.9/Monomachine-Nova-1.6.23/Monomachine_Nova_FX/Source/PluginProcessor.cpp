...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    for(size_t r=0;r<16;++r)matrix.configureRouting(r,routeRaw[r][0]->load()>0.5f,static_cast<monomachine::ModSource>(juce::jlimit(0,17,juce::roundToInt(routeRaw[r][1]->load()))),static_cast<uint8_t>(juce::jlimit(0,55,juce::roundToInt(routeRaw[r][2]->load()))),static_cast<int8_t>(juce::jlimit(-64,63,juce::roundToInt(routeRaw[r][3]->load()))),(juce::roundToInt(routeRaw[r][4]->load())==1?monomachine::ModPolarity::Bipolar:monomachine::ModPolarity::Unipolar));
