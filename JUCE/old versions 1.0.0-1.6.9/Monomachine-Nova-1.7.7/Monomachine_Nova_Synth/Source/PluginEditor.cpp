...
public:
    MsegPage(MonomachineNovaAudioProcessor& p,int startPage) : processor(p), page(juce::jlimit(0,7,startPage)) {
        ...
        for (const char* m : {"RETRIG","ENVELOPE","FREE"}) retrig.addItem(m, retrig.getNumItems()+1);
        retrig.setSelectedItemIndex(0, juce::dontSendNotification); // дефолт RETRIG
        retrig.setTooltip("MSEG mode (this page): RETRIG = every new note restarts the phase. ENVELOPE = one pass per note, then HOLDS the last point value until the next note. FREE = free-run, no restart.");
    ...
    void setPage(int m) {
        ...
        retrigLink = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(processor.parameters, tag + "_retrig", retrig); // 1.7.7: режим постранично (RETRIG|ENVELOPE|FREE)
...
private:
    ...
    juce::ToggleButton loop, sync; juce::Slider rate, shift; DragChoice shape, grid, viewGrid, retrig; PixelButton linkBtn{"LINK"}; // 1.7.7: retrig -- список режимов (DragChoice) // 1.7.1: +RETRIG // 1.6.33: логика DEST-матрицы (клик = список после отпускания, ЛКМ-драг = листать); WheelCombo открывал список по НАЖАТИЮ -- драг триггерил пункт
    ...
    std::unique_ptr<juce::AudioProcessorValueTreeState::ButtonAttachment> loopLink, syncLink; std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment> retrigLink; std::unique_ptr<juce::AudioProcessorValueTreeState::SliderAttachment> rateLink; // 1.7.1: +RETRIG
