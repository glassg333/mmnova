...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        for(int mi=1;mi<=8;++mi){const juce::String n=mi==1?"mseg":"mseg"+juce::String(mi); // 1.7.2: RETRIG постранично (как LOOP/SYNC)
            s.push_back({n+"_retrig","MSEG"+(mi>1?juce::String(mi):juce::String())+" mode",0,2,0,1,"RETRIG|ENVELOPE|FREE"});} // 1.7.7: режим вместо галки: RETRIG (рестарт от ноты), ENVELOPE (один проход и держит последнюю точку), FREE (без рестарта)
        ...
        for(int row=0;row<64;++row) { // 1.7.7: 64 слота матрицы
            ...
            s.push_back({prefix+"dest","Route target",0,131,0,1,targets}); // 1.7.7: цель = значение-1, 0..130 (0 = OFF)
