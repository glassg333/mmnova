...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    for(size_t r=0;r<64;++r){const int dvv=juce::jlimit(0,131,juce::roundToInt(routeRaw[r][2]->load())); // 1.7.7: 64 слота; цель 0..131 (значение-1 = 0..130: PITCH/ROUTE) -- было 66, matrix>dest не доходил до движка
...
void MonomachineNovaAudioProcessor::trigger(int note,float vel,int channel){
    ...
    for(int mi=0;mi<8;++mi){auto* rt=globalRaw[static_cast<size_t>(MsegRetrig+mi)]; // 1.7.7: режим страницы: 0 RETRIG / 1 ENVELOPE / 2 FREE -- рестарт от ноты у первых двух
        const int mode=rt==nullptr?0:juce::jlimit(0,2,juce::roundToInt(rt->load()));
        if(mode<2){msegPhase[static_cast<size_t>(mi)]=0.0;msegValue[static_cast<size_t>(mi)]=mseg[static_cast<size_t>(mi)].value(0.0f);}}
    liveArpGate=1.0f;
...
void MonomachineNovaAudioProcessor::render(juce::AudioBuffer<float>& buffer,int start,int length){
    ...
    while(length>0){
        ...
        matrix.evaluate(before,after,msegMod.data(),arpMod.data(),routeExtra.data()); // 1.7.1: +цели ARP; 1.7.7: +ROUTE
        ...
        for(int mi=0;mi<msegN;++mi){const int b=msegBase[static_cast<size_t>(mi)]; // 1.6.29: rate/sync/loop каждой страницы лежат подряд
            ...
            if(msegPhase[static_cast<size_t>(mi)]>=1.0){const int mmode=[&]{auto* rt=globalRaw[static_cast<size_t>(MsegRetrig+mi)];return rt==nullptr?0:juce::jlimit(0,2,juce::roundToInt(rt->load()));}(); // 1.7.7
                if(mmode==1)msegPhase[static_cast<size_t>(mi)]=1.0; // ENVELOPE: один проход, дальше держит значение последней точки -- не ретригает до следующей ноты
...
int MonomachineNovaAudioProcessor::addMsegRoute(int msegIndex,uint8_t target){if(msegIndex>=msegPageCount())setMsegPageCount(msegIndex+1); // 1.6.30: бинд активирует страницу; 1.6.34: возвращает слот
    const int destination=juce::jlimit(0,130,static_cast<int>(target)); // 1.7.7: цели 0..130 (ROUTE 1..64); 1.7.5: +PITCH/ROUTE
    int slot=-1;
