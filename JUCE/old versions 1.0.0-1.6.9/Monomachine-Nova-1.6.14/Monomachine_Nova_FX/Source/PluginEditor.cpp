...
static const SpeedDef kSpeedDefs[]{
    ...
    {"VALUE BOXES","VALUE BOXES",1.0},{"MODE AMP","AMP MODE",1.0},{"MODE SYNT","SYNT MODE",1.0},{"MODE FILT","FILT MODE",1.0},
    {"MODE DIST","DIST MODE",1.0},{"MODE DLY","DLY MODE",1.0},{"ARP MODE","ARP PLAY",1.0},{"ARP VEL","ARP VEL",1.0},{"ARP PAGE","ARP PAGE",1.0}};
...
public:
    ...
    // 1.6.13: чёрный пиксельный скин для ВСЕХ popup-меню (DSP MODE, матрица,
    // списки ручек, меню машин). Текст по центру -- списки экономнее и ровнее.
    ...
    void drawPopupMenuSectionHeaderWithOptions(juce::Graphics& g,const juce::Rectangle<int>& area,const juce::String& sectionName,const juce::PopupMenu::Options&)override{
        g.fillAll(juce::Colours::black);g.setColour(ink);pixel::text(g,sectionName,area.reduced(6,0),12,false); // 1.6.14: заголовки секций белые
    }
    void drawPopupMenuItemWithOptions(juce::Graphics& g,const juce::Rectangle<int>& area,bool highlighted,const juce::PopupMenu::Item& item,const juce::PopupMenu::Options&)override{
        ...
        g.setColour(item.isEnabled?(highlighted?knockout:ink):dim);
        pixel::text(g,item.text,area.reduced(6,0),13,item.isEnabled);
    }
...
public:
    ...
    void mouseDown(const juce::MouseEvent& e) override { dragY=e.getScreenPosition().y; dragBase=value; }
    void mouseDrag(const juce::MouseEvent& e) override { setValue(dragBase+(dragY-e.getScreenPosition().y)/16.0); }
...
private:
    ...
    juce::String speedKey; double defV=1.0,value=1.0,dragBase=0; int dragY=0;
};
...
private:
    ...
    void applyErase(const juce::MouseEvent& e, const Geo& geo) { // ПКМ стирает velocity и transpose
        if (e.x < geo.x0) return; // 1.6.14: по вертикали клампим -- стирается и у краёв
        const int pg = juce::jlimit(0, 15, pageOf()), st = columnFor(geo, e.x);
        setParam(pg, st, "velocity", 0.0f); setParam(pg, st, "transpose", 0.0f); repaint();
    }
...
public:
    explicit ArpPage(MonomachineNovaAudioProcessor& p) : processor(p), lane(p, [this] { return juce::jlimit(0, 15, juce::roundToInt(pageBox.getValue())); }, [this](int pg, int st, const char* f, float v) { setStepParameter(pg, st, f, v); }),
        pageBox(p, "arp_step_page", 0, 15, 0), pageLimit(p, "arp_step_page_limit", 1, 16, 1), octavesBox(p, "arp_range", 1, 4, 1) {
        ...
        hostTempo.setTooltip("ARP rate mode only: ON = RATE follows host BPM grid, OFF = free ms. Does NOT switch the plugin-wide host sync.");
        rateSync.setButtonText("SYNC");rateSync.textHeight=12;addAndMakeVisible(rateSync);rateSync.onClick=[this]{setClockMode(true);};
        rateTime.setButtonText("TIME");rateTime.textHeight=12;addAndMakeVisible(rateTime);rateTime.onClick=[this]{setClockMode(false);};
        ...
        // 1.6.13: GATE остаётся слайдером со значением; OCT/PAGE LIMIT -- окошки значений.
        ...
        addAndMakeVisible(pageLimit); addAndMakeVisible(octavesBox);
        ...
        // лимит автоматизируемых VST-параметров не тратится).
        for(const auto& def:kSpeedDefs){
            auto label=std::make_unique<PixelLabel>();label->setText(def.label,juce::dontSendNotification);
            label->setBounds(0,0,10,14);label->setJustificationType(juce::Justification::centredLeft);addAndMakeVisible(*label);
            auto box=std::make_unique<FloatBox>(def.key,def.def);addAndMakeVisible(*box);
    ...
    // 1.6.14: колесо страницу не крутит; скролл -- зажать ЛКМ на пустом месте и тащить.
    void mouseWheelMove(const juce::MouseEvent&,const juce::MouseWheelDetails&)override{}
    void mouseDown(const juce::MouseEvent& e)override{scrollBase=static_cast<int>(e.getScreenPosition().y);scrollView=viewPos().y;} // 1.6.14: старт драг-скролла
    void paint(juce::Graphics& g) override {
        g.fillAll(juce::Colours::black); g.setColour(ink);
    ...
    void resized() override {
        ...
        octavesBox.setBounds(925, 76, 56, 28); gate.setBounds(995, 76, 160, 28);
        hostTempo.setBounds(24, 110, 150, 28); rateSync.setBounds(190, 110, 70, 28); rateTime.setBounds(264, 110, 70, 28); // HOST TEMPO + SYNC/TIME (клик = режим)
...
private:
    ...
    juce::Point<int> viewPos() const { if (auto* vp = findParentComponentOfClass<juce::Viewport>()) return vp->getViewPosition(); return {}; }
    ...
    void refreshRate() { // единая кнопка RATE показывает значение текущего режима
        ...
        rateSync.setToggleState(isSyncMode(), juce::dontSendNotification);
        rateTime.setToggleState(!isSyncMode(), juce::dontSendNotification);
        rateButton.repaint();
    }
    ...
    void randomizeRange(int pg0, int pg1, bool doTr, bool doVel, bool doHold) {
        ...
        for (int pg = pg0; pg <= pg1; ++pg) for (int st = 0; st < 8; ++st) {
            const auto put = [this](int pg2, int st2, const char* f, float v) { if (auto* p = processor.parameters.getParameter("arp_s" + juce::String(pg2) + "_" + juce::String(st2) + "_" + f)) p->setValueNotifyingHost(p->convertTo0to1(v)); };
    ...
    void mouseDrag(const juce::MouseEvent& e)override{ // 1.6.14: драг страницы по пустому месту
        if (auto* vp = findParentComponentOfClass<juce::Viewport>()) vp->setViewPosition({vp->getViewPositionX(), juce::jmax(0, scrollView - (static_cast<int>(e.getScreenPosition().y) - scrollBase))});
    }
    ...
    DragValueBox pageBox, pageLimit, octavesBox;
    juce::Slider gate, wrap;
    PixelButton rateSync{"SYNC"}, rateTime{"TIME"}; float rateAccum = 0; int seenPage = -1, lastStepEcho = -1, scrollBase = 0, scrollView = 0;
    std::vector<std::unique_ptr<PixelLabel>> speedLabels; std::vector<std::unique_ptr<FloatBox>> speedBoxes; // 1.6.14: GUI DRAG SPEED
    std::vector<std::unique_ptr<juce::AudioProcessorValueTreeState::ButtonAttachment>> toggleLinks; std::unique_ptr<ComboAttachment> modeLink, velocityModeLink; std::unique_ptr<SliderAttachment> gateLink, wrapLink;
};
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.14 / Synth":"BUILD 1.6.14 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
    ...
    void showDspModes(int section,juce::Point<int> screen=juce::Point<int>(-1,-1)){
        ...
        juce::Component::SafePointer<Surface> safe(this);
        juce::PopupMenu::Options options=juce::PopupMenu::Options().withMinimumWidth(560).withTargetComponent(this); // 1.6.13: чёрный скин гарантирован
