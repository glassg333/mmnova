...
public:
    ...
    std::function<void()> openSamples;std::function<void(int)> openEnvelope;std::function<void(uint8_t)> addMsegModulation;
...
public:
    explicit MatrixPage(MonomachineNovaAudioProcessor& p) : processor(p) {
        ...
        for (int r = 0; r < 16; ++r)
            for (int c = 0; c < 5; ++c)
                cells[static_cast<size_t>(r * 5 + c)] = p.parameters.getParameter("r" + juce::String(r) + "_" + fields[c]);
        setSize(1170, 650);
        startTimerHz(15);
    }
    ~MatrixPage() override { stopTimer(); }

    static int percent(int raw) { return juce::jlimit(0, 100, juce::roundToInt(std::abs(raw) * 100.0f / 64.0f)); }
    static juce::String signedPercent(int raw) { return juce::String(raw < 0 ? "-" : "+") + juce::String(percent(raw)) + "%"; }

    void paint(juce::Graphics& g) override {
        g.fillAll(juce::Colours::black);
        g.setColour(ink);
        pixel::text(g, "MODULATION MATRIX / ROUTING", {8, 2, 800, 24}, 20);
        pixel::text(g, "MSEG / LFO / MIDI / ARP STEP SOURCES", {650, 5, 500, 20}, 13, false);
        const char* headers[]{"SORT", "ON", "SOURCE", "DESTINATION", "DEPTH", "MODE"};
        for (int c = 0; c < 6; ++c) pixel::text(g, headers[c], {colX(c), 30, colWidth(c), 20}, 14, true);
        for (int r = 0; r < 16; ++r) {
            const int y = rowY(r);
            pixel::text(g, juce::String(r + 1).paddedLeft('0', 2), {colX(0), y, colWidth(0), 25}, 17, true);
            for (int c = 1; c <= 5; ++c) {
                auto* par = cell(r, c);
                if (!par) continue;
                const int value = juce::roundToInt(par->convertFrom0to1(par->getValue()));
                juce::String text;
                if (c == 1) text = value > 0 ? "ON" : "OFF";
                else if (c == 2) text = sourceName(value);
                else if (c == 3) text = targetName(processor, juce::jlimit(0, 55, value));
                else if (c == 4) {
                    text = signedPercent(value);
                    const float x = static_cast<float>(colX(c) + 8), w = static_cast<float>(colWidth(c) - 16);
                    const float mid = x + w * 0.5f;
                    const float pos = mid + static_cast<float>(value) / 64.0f * w * 0.5f;
                    g.setColour(ink.withAlpha(0.28f)); g.drawHorizontalLine(y + 28, x, x + w);
                    g.setColour(ink); g.fillRect(std::min(mid, pos), static_cast<float>(y + 26), std::max(1.0f, std::abs(pos - mid)), 3.0f);
                } else text = polarityName(value);
                pixel::text(g, text, {colX(c), y, colWidth(c), 25}, c == 3 ? 15 : 17, true);
            }
            for (int c = 1; c <= 5; ++c) pixel::dotted(g, colX(c) - 3, y - 3, colX(c) - 3, y + 31);
        }
        g.setColour(ink.withAlpha(0.25f)); g.drawRect(6, 26, getWidth() - 14, 16 * 34 + 36, 1);
        g.setColour(dim);
        pixel::text(g, "DRAG DEPTH: 0..100%   SHIFT: fine   DRAG SORT: reorder routes   CLICK: list / toggle", {8, getHeight() - 25, 1120, 18}, 13, false);
    }

    void mouseWheelMove(const juce::MouseEvent& e, const juce::MouseWheelDetails& w) override {
        const int r = rowAt(e.y), c = colAt(e.x); if (r < 0 || c < 1 || std::abs(w.deltaY) < 0.0001f) return;
        auto* p = cell(r, c); if (!p) return;
        const int old = juce::roundToInt(p->convertFrom0to1(p->getValue()));
        const int lo = c == 4 ? -64 : 0, hi = c == 4 ? 63 : (c == 5 ? 2 : choiceCount(p) - 1);
        p->beginChangeGesture(); p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(juce::jlimit(lo, hi, old + (w.deltaY > 0 ? 1 : -1))))); p->endChangeGesture();
        repaint();
    }
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        ...
        hitRow = r; hitCol = c; hitY = static_cast<int>(e.getScreenPosition().y);
        hitBase = cell(r, c) ? juce::roundToInt(cell(r, c)->convertFrom0to1(cell(r, c)->getValue())) : 0;
        dragged = false; if (auto* p = cell(r, c)) { gesture = true; p->beginChangeGesture(); }
    }
    void mouseDrag(const juce::MouseEvent& e) override {
        if (dragRow >= 0) { dragTarget = juce::jlimit(0, 15, rowAt(e.y)); repaint(); return; }
        if (hitRow < 0) return; auto* p = cell(hitRow, hitCol); if (!p) return;
        const int dy = hitY - static_cast<int>(e.getScreenPosition().y); if (std::abs(dy) > 2) dragged = true;
        if (hitCol == 4) {
            const int amount = e.mods.isShiftDown() ? dy / 10 : dy;
            p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(juce::jlimit(-64, 63, hitBase + amount))));
        } else {
            const int hi = hitCol == 5 ? 2 : choiceCount(p) - 1;
            p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(juce::jlimit(0, std::max(0, hi), hitBase + dy / 8))));
        }
        repaint();
    }
    void mouseUp(const juce::MouseEvent&) override {
        ...
        if (wasDrag || r < 0 || c < 2) return;
        auto* par = cell(r, c); if (!par) return;
        juce::PopupMenu menu;
        if (c == 4) {
            const int values[]{0, 16, 32, 48, 63, -16, -32, -48, -64};
            for (int i = 0; i < 9; ++i) menu.addItem(i + 1, signedPercent(values[i]), true, values[i] == juce::roundToInt(par->convertFrom0to1(par->getValue())));
        } else {
            const int n = c == 5 ? 3 : choiceCount(par); const int selected = juce::roundToInt(par->convertFrom0to1(par->getValue()));
            const auto names = par->getAllValueStrings();
            for (int i = 0; i < n; ++i) menu.addItem(i + 1, c == 2 ? sourceName(i) : c == 3 ? targetName(processor, i) : (names.size() > i ? names[i] : polarityName(i)), true, i == selected);
        }
        const juce::Component::SafePointer<MatrixPage> safe(this);
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this), [safe, r, c](int result) {
            if (!safe || result <= 0) return; auto* p = safe->cell(r, c); if (!p) return;
            if (c == 4) { const int values[]{0, 16, 32, 48, 63, -16, -32, -48, -64}; p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(values[result - 1]))); }
...
private:
    ...
    static juce::String polarityName(int v) { const char* n[]{"AUTO", "ONE SIDE", "TWO SIDE"}; return n[juce::jlimit(0, 2, v)]; }
    int colX(int c) const { const int x[]{10, 66, 164, 350, 740, 910}; return x[juce::jlimit(0, 5, c)]; }
    int colWidth(int c) const { const int w[]{45, 90, 180, 380, 165, 220}; return w[juce::jlimit(0, 5, c)]; }
    ...
    std::array<juce::RangedAudioParameter*, 80> cells{};
    int hitRow = -1, hitCol = -1, hitY = 0, hitBase = 0, dragRow = -1, dragTarget = -1; bool gesture = false, dragged = false;
};

// The actual MSEG page: points are editable, serialised with the plug-in state,
// and its output port can be dragged to a destination row or any main-screen Cell.
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        pixel::text(g, "OUTPUT", {696, 40, 78, 18}, 13, true);
        ...
        g.fillEllipse(outputX - 8.0f, outputY - 8.0f, 16.0f, 16.0f); g.drawEllipse(outputX - 8.0f, outputY - 8.0f, 16.0f, 16.0f, 2.0f);
        pixel::text(g, "DOUBLE CLICK: ADD POINT   RIGHT CLICK: REMOVE   DRAG: EDIT", {20, 358, 720, 20}, 13, false);
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        if (e.x >= outputX - 15 && e.x <= outputX + 15 && e.y >= outputY - 15 && e.y <= outputY + 15) {
            if (auto* d = juce::DragAndDropContainer::findParentDragContainerFor(this)) d->startDragging("MSEG", this, juce::ScaledImage(), true);
            return;
        }
        selected = nearestPoint(e.position);
        if (e.mods.isPopupMenu() && selected > 0 && selected < processor.msegPointCount() - 1) { processor.removeMsegPoint(selected); selected = -1; repaint(); return; }
        if (selected >= 0) { draggingPoint = true; return; }
    }
    void mouseDoubleClick(const juce::MouseEvent& e) override { if (e.x >= graphX && e.x <= graphX + graphW && e.y >= graphY && e.y <= graphY + graphH) { processor.addMsegPoint(toX(e.x), toY(e.y)); repaint(); } }
    void mouseDrag(const juce::MouseEvent& e) override {
    ...
    void mouseUp(const juce::MouseEvent&) override { draggingPoint = false; }
private:
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer {
    explicit Surface(MonomachineNovaAudioProcessor& p):processor(p),machineButton(""),menuButton("MENU"),tempoButton(""),arpButton("ARP"),matrixButton("MATRIX"),lfoButton("LFO2"),lfo3Button("LFO3"),msegButton("MSEG"),closeButton("BACK"),gateButton(""){
        setLookAndFeel(&theme);
        for(auto& panel:cells)for(auto& cell:panel){cell=std::make_unique<Cell>(p);cell->openSamples=[this]{showSamples();};cell->openEnvelope=[this](int knob){showEnvelope(knob);};cell->addMsegModulation=[this](uint8_t target){processor.addMsegRoute(target);};addAndMakeVisible(*cell);}
        for(auto* button:{&machineButton,&menuButton,&tempoButton,&arpButton,&matrixButton,&lfoButton,&lfo3Button,&msegButton})addAndMakeVisible(*button);
        ...
        closeButton.textHeight=20;closeButton.onClick=[this]{viewport.setViewedComponent(nullptr);settings.reset();matrixPage.reset();samplePage.reset();msegPage.reset();envelopePage.reset();viewport.setVisible(false);closeButton.setVisible(false);repaint();};
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.5.0 / Synth / DSP1":"BUILD 1.5.0 / FX / DSP1",850,570,370,13,juce::Justification::right);
    }
    void resized()override{
        ...
        lfoButton.setBounds(870,334,120,30);lfo3Button.setBounds(995,334,120,30);msegButton.setBounds(1120,334,137,30);
    ...
    PixelButton machineButton,menuButton,tempoButton,arpButton,matrixButton,lfoButton,lfo3Button,msegButton,closeButton,gateButton;juce::Slider level;
