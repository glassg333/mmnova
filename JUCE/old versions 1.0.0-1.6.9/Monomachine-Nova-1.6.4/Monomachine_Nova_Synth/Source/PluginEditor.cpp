...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer {
    ...
    // DSP mode list - fixed: erase extra m letters, show all 8 modes with explanations
    void showDspModes(int section){
        ...
        auto addSection=[&](int index){
            ...
            sub.addItem(index*100+primary+1,
                        "mnm - ОСНОВНОЙ: "+juce::String(monomachine::dspModeProvenanceForMode(index,primary)),
            ...
            if(monomachine::kShowBackupModes){
                ...
                if(backups.size()>0){
                    sub.addSeparator();
                    sub.addItem(-1,"РЕЗЕРВ (бэкап, не основной):",false,false);
                    for(auto& name:backups){
                        const int m=monomachine::dspModeIndexByName(name.toRawUTF8());
                        if(m<0)continue;
                        sub.addItem(index*100+m+1,
                                    "   "+name+" - "+juce::String(monomachine::dspModeProvenanceForMode(index,m)),
                                    true, current==m);
                    }
                }
            }
            ...
            menu.addSubMenu(juce::String(monomachine::dspSectionLabel(index))+"   ["+juce::String(monomachine::dspModeLabel(current))+"]",sub);
        };
    ...
    void updateDspModeLabel(){
        ...
        juce::String tip="DSP MODE -- в каждом разделе свой список, основной режим mnm:\n";
        for(int i=0;i<monomachine::DspSectionCount;++i){
            const int mode=processor.dspMode(i);
            tip+=juce::String(monomachine::dspSectionLabel(i))+": "+juce::String(monomachine::dspModeLabel(mode));
