...
public:
    ...
    void drawLinearSlider(juce::Graphics& g,int x,int y,int width,int height,float pos,float min,float max,const juce::Slider::SliderStyle style,juce::Slider& slider)override{
        if(style==juce::Slider::LinearVertical){g.setColour(ink);g.fillRect(x+width/2-6,juce::roundToInt(pos),12,std::max(1,y+height-juce::roundToInt(pos)));
            const auto rng=slider.getRange(); // 1.6.28: риски -- максимальный уровень и 100 (видно, что опустил ниже дефолта)
            if(rng.getLength()>0){g.setColour(juce::Colours::white);g.drawHorizontalLine(y+1,static_cast<float>(x)+2,static_cast<float>(x+width)-2);
                if(100>=rng.getStart()&&100<=rng.getEnd()){const int ty=juce::roundToInt(y+height*static_cast<float>(rng.getEnd()-100)/static_cast<float>(rng.getLength()));g.drawHorizontalLine(ty,x+width/2-8.0f,x+width/2+6.0f);}}}
...
public:
    ...
    void paintButton(juce::Graphics& g,bool /*over*/,bool down)override{
        const bool filled=(down&&outlined)||getToggleState(); // 1.6.26: PixelButton умеет быть галкой (MAP FLASH)
...
public:
    ...
    void openValueMenu(int r, int c, juce::Point<int> screen) { // 1.6.24: единый список значений ячейки
        ...
        if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; for (int i = 0; i < 9; ++i) menu.addItem(i + 1, signedPercent(values[i])); } // -100% ... 0% ... +100%
        ...
        else if (c == 3) { // 1.6.28: ПЛОСКИЙ список с заголовками папок -- всё видно сразу, без задержки подменю
            ...
            for (int g = 0; g < 7; ++g) {
                menu.addSectionHeader(groups[g]);
                for (int k = 0; k < 8; ++k) { const int target = g * 8 + k; menu.addItem(target + 1, targetName(processor, target), true, target == selected); }
            }
        }
        else if (c == 2) { // 1.6.28: RANDOM первым, папки MIDI/LFO/MSEG/ARP (плоско, без задержки подменю)
            static const int vals[]{17, 2, 3, 0, 1, 9, 8, 7, 4, 5, 6, 10, 11, 12, 13, 14, 15, 16};
            const int selected = juce::roundToInt(par->convertFrom0to1(par->getValue()));
    ...
    void openAuxMenu(int r, juce::Point<int> screen) { // 1.6.24: AUX SOURCE -- второй источник решает, сколько модуляции пройдёт
        ...
        static const char* auxNames[]{"OFF", "KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "MSEG2", "MSEG3", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM"}; // 1.6.25: порядок = ModSource
        auto* par = processor.parameters.getParameter("r" + juce::String(r) + "_aux"); const int cur = par ? juce::jlimit(0, 18, juce::roundToInt(par->convertFrom0to1(par->getValue()))) : 0;
        for (int i = 0; i < 19; ++i) m.addItem(i + 1, auxNames[i], true, i == cur);
        const juce::Component::SafePointer<MatrixPage> safe(this);
...
private:
    ...
    static juce::String sourceName(int v) { const char* n[]{"KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "MSEG2", "MSEG3", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM"}; return n[juce::jlimit(0, 17, v)]; } // 1.6.25: без трёх RANDOM -- MSEG2/3 и шаги арпа на месте
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        for (int r = 0; r < 16 && row < 6; ++r) {
            ...
            const int mIdx = sv == 10 ? 0 : sv == 11 ? 1 : sv == 12 ? 2 : -1; // 1.6.25: MSEG2=11, MSEG3=12 (дыра в enum убрана)
            if (mIdx < 0) continue;
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.4f));g.drawRect(8,94,55,512,2); // 1.6.28: низ пунктира LEV совпадает с низом секций
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.28 / Synth":"BUILD 1.6.28 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
        ...
        arpEnabled.setBounds(471,58,26,30);arpButton.setBounds(595,58,120,30);gateOuter.setBounds(719,58,34,30);trigToggle.setBounds(757,58,105,30);matrixButton.setBounds(870,58,140,30); // 1.6.28: галка на линии AMP, шаги по 124 как ARP->GATE
    ...
    void showMachineMenu(){
        ...
        juce::CallOutBox::launchAsynchronously(std::move(panel),machineButton.getScreenBounds().removeFromLeft(machineButton.getWidth()-56),nullptr); // 1.6.25: список открывается у имени машины, а не сбоку от стрелок
    }
    ...
    void showMenu(){juce::PopupMenu menu;menu.setLookAndFeel(&getLookAndFeel()); // 1.6.14: чёрный скин
        ...
        juce::Component::SafePointer<Surface> safe(this);menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&menuButton),[safe](int result){if(!safe)return;
            ...
            if(result==6)juce::AlertWindow::showMessageBoxAsync(juce::MessageBoxIconType::InfoIcon,"Monomachine Nova Synth / BUILD 1.6.28","Build 1.6.28 (+1 every patch):\n1.6.28: while AIMING, a knob can no longer be turned by LMB -- LMB maps, ALT+LMB turns. Matrix SOURCE/DEST lists are flat with folder headers (RANDOM first in SOURCE; no more submenu delay), DEST shows --- when nothing is assigned; DEPTH returns the gray FILL BAR (with white edge + outline, no full-cell gray). The picker uses the full-size font (SWAVE is no longer smaller) and its width hugs the text. MODE windows are 92 px (right edge almost touches the page divider); category headers (SYNT/LFO/FILT/EFFX/MSEG/AMP) are now buttons sized from the panel line to the first knob -- MSEG opens the curve editor, the rest are reserved for future functionality; smaller category font. Front row: the ARP checkbox square is full-height and sits on the AMP line, ARP and GATE follow with even 124 px steps. LEV shows a tick at maximum and a micro-tick at 100, its dotted frame now ends flush with the sections. ARP window: the PLAY caption is gone and the top rows are compacted. Drag on the machine name never opens the list; arrows are fully isolated gestures with extra margin on the left arrow. MSEG page amount hitbox is wider.\n1.6.27: the aim highlight is WHITE again (a wrong colour constant had turned it gray/invisible), and the post-confirm flash is GONE by default. MENU > GUI OPTIONS has a new MAP FLASH toggle: when ON, dropping a modulation on a knob blinks the cell pale-white ONCE as confirmation. The highlight also works when picking a target from the matrix.\n1.6.26: the MODE windows sit EXACTLY above the last knob column of their page (AMP mode included); a page with two modes puts the second one above the second-to-last knob (EFFX: DLY over the 7th knob column, DIST over the 8th). The machine arrows strip got its own black background -- pressing the machine name (the list) no longer floods the arrows with white. The synth picker is now exactly the first black-interface layout with the two columns SWAPPED: the machine TYPE sits on the left shown ONCE per category (and on the hovered row), and the machine short NAMES are the right list.\nFixed in 1.6.25: the PAGE RND checkbox works again (a swallowed line had disabled the engine PAGE/LIMIT wiring); source lists show all 18 sources (MSEG2/3 and the step/arp sources are back, no triple RANDOM); the AUX SRC column uses the same order as SOURCE. Matrix: DEPTH shows only the cell outline + the white fill bar (gray background removed), the matrix frame is WHITE, lists have a full WHITE border, SOURCE/DEST/MODE/AUX open on click and scroll with LMB drag without conflicts, the lock hitbox is wider toward the DEST list. Aim highlight is back to the old look: only the aimed cell gets a white frame -- no blinking, knobs and values stay visible. Machine arrows: each arrow has its own hitbox and only the pressed one lights up, the synth list opens by the NAME again (not beside the arrows), the list columns are swapped (type on the left, family on the right, synth machines first, FX at the bottom). ARP: rate list opens on RIGHT-click; PAGE and PAGE LIMIT sit side by side, double-click types a value, hover shows the tooltip; RMB on PAGE/PITCH/VEL/HOLD randomizers runs them on ALL pages (LMB = current page); RETRIG SYNC removed as requested. Engine: polarity fixed per Serum (unipolar = one side 0..+depth, bipolar = centred -depth..+depth regardless of the native source range). Front panel: GATE box border is pixel-aligned with ARP, checkboxes are centred, all MODE windows are 91 px wide (last-knob width) with centred text, AMP mode moved right. LFO lock panel now follows page swaps live.\n"
    "Patch cords: the experimental persistent cords are REMOVED (they doubled); the live drag cord stays. NEW: releasing the LFO cord on empty space CANCELS the attempt -- PAGE/DEST snap back to what they were before the modulation.\nMachine picker: names stay the FIRST (left) list, the category column moved to the right edge, the window width fits the widest category+name; the preset arrows now sit in their own bordered box and own the whole right strip (no more list invocation under them); preset wheel steps every tick and LMB drag on the name scrolls UP/DOWN (arrows stay horizontal).\nWhile aiming, EVERY valid target cell blinks and hides its contents (MULT included). Releasing the cord on nothing restores the pre-aim values.\nMatrix: SOURCE/DESTINATION lists are back on click; DEPTH popup on RMB only, double LMB-click resets to 0, RMB never resets; the DEPTH value is centred and shows a gray fill + bright white edge line at the reached amount; POLARITY is a plain UNI<->BI toggle (Sylenth: bipolar = centred +-100, unipolar = one side 0..+100); NEW AUX SRC column -- a secondary source scales the amount (OFF by default).\nARP: RETRIG SYNC toggle -- the first note after silence starts exactly on the next host-grid beat; velocity mode default is STEP and STEP modes are first in the list.\nFixed state saves: the plugin always wrote the old schema tag, so legacy migrations (DSP remap) re-ran on EVERY preset load and could clobber settings; VST3 presets now reload as saved.\nMisc: the false pointing-finger cursor over knobs is gone and RMB works anywhere on a knob (PORT panel included); LEV scale shows a hollow white outline where unfilled; the junk text file is gone; the archive folder is renamed per version.\nARP gate verified numerically: note-off lands exactly at GATE/128 of the step; the amp envelope release still shapes the tail (per Sound On Sound gate semantics) -- hold-less steps with short DEC/REL show the clean gate effect; steps with HOLD=ON sustain to the next step by design.\nPersistent patch cords (an experimental feature) were REMOVED in 1.6.24 -- they doubled the live cable; only the drag-time cord remains.\nMachine picker is back to the 1.6.20 two-column look: names in the left column, the generator TYPE in the right column on the first row of the category (and on the hovered row), WHITE row highlight.\nMatrix: the DEPTH list is ordered -100% ... 0% ... +100%; value popups open on RIGHT-click only (LMB is drag), a double LEFT-click resets a cell to 0; CLEAR writes exact zeros with a single conversion and without host notification -- no more 2% ghosts after clearing.\nARP: HOLD now sustains the note until the NEXT step (per the Monomachine manual, HOLD removes the need for NOTE OFF); the step/page randomizers write straight into the parameter atomics (the old suspend-processing stall is gone) and PAGE RND picks any of the 16 pages each pass.\nThe junk text file is gone and the archive folder is renamed to match the version.\nREMOVE BUG: the matrix CLEAR button (LMB) crashed the plugin -- a lambda stored in onClick held a dangling reference to a constructor-local helper.\nMatrix: the ON checkboxes can be PAINTED by dragging (LMB paints on, RMB erases); SRC/DEST/DEPTH/MODE cells react to the LEFT button only (right button is reserved for alt modes/lists).\nPatch cords REMAIN visible where attached (dim gray); RIGHT-click an LFO 1/2/3 or MSEG 1/2/3 button to spotlight only its cords (cheap: a few arcs, redrawn only when routes change).\nMSEG: RIGHT-click STEPS bakes the stairs into the curve (press again to bring the smooth shape back); the DRAW button works again (freehand) and DRAW STEP draws square stairs without the STEPS display mode; the SHAPE and GRID lists scroll with the wheel.\n"
