...
def main() -> int:
    ...
    present(text, "BUILD 1.6.28", "1.6.28 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.28", "1.6.28 Editor: версия в About (титул окна)")
    ...
    present(text, "BUILD 1.6.28", "1.6.28 Editor: маркер версии сборки обновлен")
    present(text, "Build 1.6.28 (+1 every patch)", "1.6.28 Editor: версия в About")
    ...
    present(text, "static const int vals[]{17, 2, 3, 0, 1, 9, 8, 7, 4, 5, 6, 10, 11, 12, 13, 14, 15, 16};", "1.6.28 Editor: RANDOM первым, папки MIDI/LFO/MSEG/ARP")
    present(text, 'menu.addSectionHeader(groups[g]);', "1.6.28 Editor: DEST -- плоский список с заголовками")
    ...
    present(text, "const int s=std::min(b.getHeight()-2,28),oy=(b.getHeight()-s)/2;", "1.6.28 Editor: квадрат галки на всю высоту кнопки")
    present(text, "arpButton.setBounds(595,58,120,30);gateOuter.setBounds(719,58,34,30);", "1.6.28 Editor: ARP/GATE ровными шагами 124 от линии AMP")
    present(text, "g.drawHorizontalLine(ty,x+width/2-8.0f,x+width/2+6.0f);", "1.6.28 Editor: LEV -- микрориска на 100")
    present(text, "g.drawRect(8,94,55,512,2);", "1.6.28 Editor: пунктир LEV до низа секций")
