...
public:
    explicit MatrixPage(MonomachineNovaAudioProcessor& p) : processor(p) { clearButton.setComponentID("clear"); // 1.6.38: LAYOUT EDIT
        ...
        auto wipeField=[this](const char* f){for(int r=0;r<64;++r)if(auto* p=processor.parameters.getParameter("r"+juce::String(r)+"_"+f)){p->setValue(p->convertTo0to1(0.0f));}}; // 1.7.7: 64 слота // 1.6.23: ровно ноль (getDefaultValue у float уже нормализован -- была двойная конвертация), без спама хосту
        ...
        setSize(1145, 58 + 64 * 34 + 40); startTimerHz(15); // 1.7.7: высота на 64 строки
    }
    ...
    void paint(juce::Graphics& g) override {
        ...
        for (int r = 0; r < 64; ++r) { // 1.7.7: 64 строки
        ...
        g.setColour(juce::Colours::white); g.drawRect(8, 26, 1120, 64 * 34 + 36, 1); // 1.6.25/1.7.7: рамка матрицы на 64 строки
        ...
        if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; for (int i = 0; i < 9; ++i) menu.addItem(i + 1, signedPercent(values[i])); } // -100% ... 0% ... +100%
            ...
            { juce::PopupMenu mxSub; // 1.7.5/1.7.7: цели-маршруты 1..64: номер строки и то, что она модулирует (модулируют её ГЛУБИНУ)
              for (int n = 0; n < 64; ++n) { auto* dp = processor.parameters.getRawParameterValue("r" + juce::String(n) + "_dest");
                  ...
                  mxSub.addItem(69 + n, juce::String(n + 1) + " - " + (dv > 1 ? targetName(processor, juce::jlimit(0, 130, dv - 1)) : juce::String("OFF")), true, selected == 68 + n); }
...
private:
    juce::String destName(int id) const { if (id >= 67 && id < 131) { auto* dp = processor.parameters.getRawParameterValue("r" + juce::String(id - 67) + "_dest"); const int dv = dp ? juce::roundToInt(dp->load()) : 0; return juce::String(id - 66) + " - " + (dv > 1 ? targetName(processor, juce::jlimit(0, 130, dv - 1)) : juce::String("OFF")); } return targetName(processor, id); } // 1.7.7: строки-маршруты 1..64 в DEST -- номер и то, что модулирует
    ...
    int rowAt(int y) const { if (y < 58) return -1; const int r = (y - 58) / 34; return r >= 0 && r < 64 ? r : -1; } // 1.7.7
    ...
    void timerCallback() override { repaint(); }
    MonomachineNovaAudioProcessor& processor; std::array<juce::RangedAudioParameter*, 320> cells{}; int hitRow = -1, hitCol = -1, hitY = 0, hitX = 0, hitBase = 0, dragRow = -1, dragTarget = -1, sortColumn = -1; bool gesture = false, dragged = false, sortDescending = false; // 1.7.7: cells 64x5
...
public:
    MsegPage(MonomachineNovaAudioProcessor& p,int startPage) : processor(p), page(juce::jlimit(0,7,startPage)) {
        ...
        for (const char* m : {"RETRIG","VITAL","FREE","SUSTAIN","LOOP POINT","LOOP HOLD"}) retrig.addItem(m, retrig.getNumItems()+1); // 1.7.8: как в Vital sync-type
        retrig.setSelectedItemIndex(0, juce::dontSendNotification); // дефолт RETRIG
        rmodeBox.speedKey="LFO RATE MODE"; addAndMakeVisible(rmodeBox); // 1.7.8: семья шкалы RATE
        for (const char* m : {"TEMPO","TRIPLET","DOTTED","TIME","HZ"}) rmodeBox.addItem(m, rmodeBox.getNumItems()+1);
        rmodeBox.setSelectedItemIndex(0, juce::dontSendNotification);
        rmodeBox.setTooltip("RATE MODE (as in Vital): TEMPO = divisions 32 BAR..1/256 + FAST synced to tempo. TRIPLET/DOTTED = same scale with triplets/dotted. TIME = period 1 ms..60 s. HZ = 0.001 Hz..20 kHz log curve (SHIFT = slower).");
        ...
        retrig.setTooltip("MSEG mode (this page, as in Vital): RETRIG = every new note restarts. VITAL (was ENVELOPE) = one pass per note, then HOLDS the last point value. FREE = free-run, no restart.");
        ...
        stepsButton.rightClick = [this] { processor.setMsegSteps(page, !stepsButton.getToggleState()); stepsButton.setToggleState(!stepsButton.getToggleState(), juce::dontSendNotification); repaint(); }; // 1.7.7 FIX: ПКМ = тот же режим, что ЛКМ (старый ПКМ «запекал» квадраты, съедая лимит точек, и не дорисовывал до конца)
        ...
        rate.setTooltip("RATE -- position inside the selected R MODE: TEMPO = 32 BAR..1/256 + FAST (tempo sync), TRIPLET/DOTTED = same but triplets/dotted, TIME = period 1 ms..60 s, HZ = 0.001 Hz to 20 kHz (x10 per quarter turn; SHIFT = slower). LFO FM target adds octaves (up to ~20 kHz)."); // 1.7.8
        ...
        const char* forms[]{"DRAW","DRAW STEP","RAND DRAW 1","RAND DRAW 2","VITAL"}; // 1.7.8: vital = сглаженное перо до 256 точек (старые кнопки на месте) // 1.6.30: плоские формы убраны (они в списке SHAPE)
    ...
    void setPage(int m) {
        ...
        rmodeLink = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(processor.parameters, tag + "_rmode", rmodeBox); // 1.7.8
    ...
    void resized() override {
        ...
        { const int ay = routeY() + 3 * 34 + 30; // 1.7.8: R MODE / KEY / LOOP POINT -- свободная зона под маршрутами
          rmodeBox.setBounds(16, ay, 150, 26); keyBtn.setBounds(174, ay + 3, 52, 20); lpoint.setBounds(232, ay, 140, 26); }
...
private:
    ...
    static const std::array<juce::String, 131>& targetNames() { static const auto names = [] { std::array<juce::String, 131> a{}; for (int i = 0; i < 131; ++i) a[static_cast<size_t>(i)] = juce::String(monomachine::ModulationMatrix::getDestinationName(static_cast<uint8_t>(i))); return a; }(); return names; } // 1.7.7: ROUTE 1..64; 1.6.29: +MSEG OUT; 1.7.1: +ARP RATE/GATE
    ...
    void vitalStroke() { auto pts = stroke; if (pts.size() < 2) return; // 1.7.8: vital-перо -- сглаженная кривая до 256 точек (без прореживания до 32)
        ...
        processor.setMsegPointCount(page, static_cast<int>(merged.size()));
        for (size_t i = 0; i < merged.size(); ++i) { processor.setMsegPoint(page, static_cast<int>(i), merged[i].x, merged[i].y); processor.setMsegSegK(page, static_cast<int>(i), 0.0f); }
        markCustom(); }
    ...
    void filesDropped(const juce::StringArray& files, int, int) override {
        ...
        for (auto& f : files) { if (!(f.endsWithIgnoreCase(".wav") || f.endsWithIgnoreCase(".wave") || f.endsWithIgnoreCase(".aif") || f.endsWithIgnoreCase(".aiff"))) continue;
            ...
            processor.setMsegPointCount(page, n);
    ...
    juce::ToggleButton loop, sync, keyBtn; juce::Slider rate, shift, lpoint; DragChoice shape, grid, viewGrid, retrig, rmodeBox; PixelButton linkBtn{"LINK"}; // 1.7.8: +RMODE/KEYTRACK/LOOP POINT // 1.7.7: retrig -- список режимов (DragChoice) // 1.7.1: +RETRIG // 1.6.33: логика DEST-матрицы (клик = список после отпускания, ЛКМ-драг = листать); WheelCombo открывал список по НАЖАТИЮ -- драг триггерил пункт
    ...
    std::unique_ptr<juce::AudioProcessorValueTreeState::ButtonAttachment> loopLink, syncLink, keyLink; std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment> retrigLink, rmodeLink; std::unique_ptr<juce::AudioProcessorValueTreeState::SliderAttachment> rateLink, lpointLink; // 1.7.8: +RMODE/KEY/LPOINT
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    juce::String matrixRouteName(int r) const { // 1.7.5/1.7.7: динамическое имя маршрута (0..63)
        ...
        return v > 0 ? targetName(processor, juce::jlimit(0, 130, v - 1)) : juce::String("OFF"); }
    void showMsegDestList(int pg, int row) { // 1.7.5: UNASSIGNED-строка MSEG -- список целей (папки как в P-LIST), выбор создаёт маршрут
        ...
        m.showMenuAsync(juce::PopupMenu::Options(), [safe, pg, row](int res) { if (!safe || res <= 0) return; const int tg = juce::jlimit(0, 130, res - 10); // 1.7.7
