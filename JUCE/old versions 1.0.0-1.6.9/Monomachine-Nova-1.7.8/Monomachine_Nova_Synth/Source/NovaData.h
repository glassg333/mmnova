...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        for(int mi=1;mi<=8;++mi){const juce::String n=mi==1?"mseg":"mseg"+juce::String(mi); // 1.7.2: RETRIG постранично (как LOOP/SYNC)
            s.push_back({n+"_retrig","MSEG"+(mi>1?juce::String(mi):juce::String())+" mode",0,5,0,1,"RETRIG|VITAL|FREE|SUSTAIN|LOOP POINT|LOOP HOLD"}); // 1.7.8: как в Vital: VITAL = один проход и держит (был ENVELOPE); SUSTAIN = идёт до LOOP POINT и держит, после note-off доигрывает до конца; LOOP POINT = после первого прохода крутит хвост от точки; LOOP HOLD = держит ноту -- крутит 0..точку, отпустил -- доигрывает хвост
        ...
        for(int row=0;row<64;++row) { // 1.7.7: 64 слота матрицы
            ...
            s.push_back({prefix+"dest","Route target",0,132,0,1,targets}); // 1.7.8: цель = значение-1, 0..132 (0 = OFF); 131 = LFO FM
