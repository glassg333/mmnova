...
public:
    ...
    void paintButton(juce::Graphics& g,bool /*over*/,bool down)override{
        ...
        if(arrows){ // 1.6.24: стрелки в ОТДЕЛЬНОЙ рамке у правого края (не пересекаются со списком)
...
public:
    ...
    void paint(juce::Graphics& g)override{
        if(pickMode){ // 1.6.24: цель прицела -- ТОЛЬКО мигающая рамка (крутилки/значения скрыты, даже choice-ячейки вроде MULT)
            const bool on=(juce::Time::getMillisecondCounter()/280)&1;
            g.setColour(hoverNow||on?ink:ink.withAlpha(0.30f));g.drawRect(getLocalBounds().reduced(2),hoverNow?3:2);return;}
    ...
    void mouseDown(const juce::MouseEvent&e)override{
        ...
        if(e.mods.isPopupMenu()&&parameter&&!isChoice){ // 1.6.24: ПКМ в любой точке ручки = её меню (раньше зона значения была мёртвой)
...
public:
    explicit ArpPage(MonomachineNovaAudioProcessor& p) : processor(p), lane(p, [this] { return juce::jlimit(0, 15, juce::roundToInt(pageBox.getValue())); }, [this](int pg, int st, const char* f, float v) { setStepParameter(pg, st, f, v); }),
        ...
        retrigSync.setButtonText("RETRIG SYNC");retrigSync.setClickingTogglesState(true);addAndMakeVisible(retrigSync); // 1.6.24: старт арпа по сетке проекта
        retrigSync.setTooltip("ARP RETRIG SYNC: the first arp note after silence starts exactly on the next BEAT of the host grid.");
        retrigLink=std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment>(processor.parameters,"arp_retrig_sync",retrigSync);
    ...
    void resized() override {
        ...
        hostTempo.setBounds(24, 110, 150, 28); retrigSync.setBounds(180, 110, 150, 28); // 1.6.16/1.6.24
...
private:
    ...
    juce::ToggleButton pageRndToggle; std::unique_ptr<juce::AudioProcessorValueTreeState::ButtonAttachment> pageRndLink; juce::ToggleButton retrigSync; std::unique_ptr<juce::AudioProcessorValueTreeState::ButtonAttachment> retrigLink; // 1.6.21/1.6.24
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        for (int r = 0; r < 16; ++r) {
            ...
            for (int c = 1; c <= 5; ++c) {
                ...
                if (c == 4) { // 1.6.24: визуал глубины -- серая заливка + яркая белая обводка на границе заполнения
                    ...
                    g.setColour(ink.withAlpha(0.15f)); g.fillRect(bx, by, bw, bh);
                    if (fw > 0) { g.setColour(ink.withAlpha(0.45f)); if (value >= 0) g.fillRect(mid, by, fw, bh); else g.fillRect(mid - fw, by, fw, bh);
                        g.setColour(juce::Colours::white); const int edge = value >= 0 ? mid + fw : mid - fw;
                        g.drawVerticalLine(edge, float(by + 1), float(by + bh - 1)); g.drawVerticalLine(mid, float(by + 1), float(by + bh - 1)); } }
            ...
            { // 1.6.24: AUX SOURCE -- вторичный источник масштабирует глубину (Sylenth-style)
                static const char* auxNames[]{"OFF", "KEY", "VEL", "MCR X", "MCR Y", "LFO1", "LFO2", "LFO3", "PTCH", "MODW", "ATH", "MSEG1", "ST VEL", "ST TR", "ARP GT", "ARP RT", "RND", "MSEG2", "MSEG3"};
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        ...
        if (c == 2 || c == 3) { openValueMenu(r, c, e.getScreenPosition()); return; } // 1.6.24: списки SOURCE/DESTINATION -- клик (вернули)
        if (c == 5) { auto* p = cell(r, 5); if (p) { p->beginChangeGesture(); p->setValueNotifyingHost(p->convertTo0to1(p->convertFrom0to1(p->getValue()) > 0.5f ? 0.0f : 1.0f)); p->endChangeGesture(); } return; } // 1.6.24: полярность -- тумблер
        if (c == 6) { openAuxMenu(r, e.getScreenPosition()); return; } // 1.6.24: AUX SOURCE
    ...
    void mouseUp(const juce::MouseEvent& e) override {
        ...
        if (c == 4 && hitWasRmb) openValueMenu(r, 4, e.getScreenPosition()); // 1.6.24: ПКМ по DEPTH = список значений
    }
    void openValueMenu(int r, int c, juce::Point<int> screen) { // 1.6.24: единый список значений ячейки
        ...
        else { const int n = choiceCount(par); const int selected = juce::roundToInt(par->convertFrom0to1(par->getValue())); for (int i = 0; i < n; ++i) menu.addItem(i + 1, sourceName(i), true, i == selected); }
        const juce::Component::SafePointer<MatrixPage> safe(this);
    ...
    void openAuxMenu(int r, juce::Point<int> screen) { // 1.6.24: AUX SOURCE -- второй источник решает, сколько модуляции пройдёт
        ...
        static const char* auxNames[]{"OFF", "KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM", "MSEG2", "MSEG3"};
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.24 / Synth":"BUILD 1.6.24 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
    ...
    void showMachineMenu(){
        ...
        struct Picker final : juce::Component {
            ...
            explicit Picker(MonomachineNovaAudioProcessor& p,int current):processor(p),bound(current){
                int w=300; // 1.6.24: ширина по самому длинному "категория + машина", зазор между колонками
                for(int i=0;i<static_cast<int>(nova::machines().size());++i){const int need=260+static_cast<int>(juce::String(nova::machines()[static_cast<size_t>(i)].category).length())*12+24;w=std::max(w,need);}
                setSize(std::min(640,w),std::min<int>(464,static_cast<int>(nova::machines().size())*24+10)); setWantsKeyboardFocus(false); }
            ...
            void paint(juce::Graphics& g) override {
                ...
                for(int i=0;i<static_cast<int>(nova::machines().size());++i){
                    ...
                    if(firstOfCat||i==hover)pixel::text(g,cat,{getWidth()-184,y,174,20},14,false); // 1.6.24: тип в правой колонке (имена -- первым списком)
