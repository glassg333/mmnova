...
public:
    ...
    void addMsegRoute(int msegIndex, uint8_t target); // 1.6.21: какая из трёх кривых
    ...
    void addRouteFromSource(int src, uint8_t target);
...
private:
    ...
    std::array<double,3> msegPhase{}; // 1.6.21
    std::array<float,3> msegValue{};
    ...
    monomachine::MSEG mseg[3]; // 1.6.21: MSEG1..MSEG3
