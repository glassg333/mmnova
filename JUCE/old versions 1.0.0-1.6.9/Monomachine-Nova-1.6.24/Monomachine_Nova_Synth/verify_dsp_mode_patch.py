...
def main() -> int:
    ...
    present(text, "BUILD 1.6.24", "1.6.20 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.24", "1.6.23 Editor: версия в About (титул окна)")
    ...
    present(text, "pixel::text(g,cat,{getWidth()-184,y,174,20},14,false);", "1.6.24 Editor: пикер -- имена первым списком, тип у правого края, ширина по контенту")
    ...
    present(text, "BUILD 1.6.24", "1.6.24 Editor: маркер версии сборки обновлен")
    present(text, "Build 1.6.24 (+1 every patch)", "1.6.24 Editor: версия в About")
    ...
    present(text, "if (c == 4 && hitWasRmb) openValueMenu(r, 4, e.getScreenPosition());", "1.6.24 Editor: DEPTH -- список по ПКМ, двойной ЛКМ -- сброс, ПКМ никогда не сбрасывает")
    ...
    present(text, "if (c == 2 || c == 3) { openValueMenu(r, c, e.getScreenPosition()); return; }", "1.6.24 Editor: списки SOURCE/DESTINATION вернулись по клику")
    ...
    present(text, "if (c == 5) { auto* p = cell(r, 5);", "1.6.24 Editor: полярность -- тумблер UNI<->BI без списка")
    ...
    present(text, "retrigSync.setButtonText(\"RETRIG SYNC\")", "1.6.24 Editor: галка RETRIG SYNC на странице ARP")
    present(text, '"arp_retrig_sync",retrigSync', "1.6.24 Editor: RETRIG SYNC -- параметр arp_retrig_sync")
    ...
    present(text, "if(pickMode){ // 1.6.24", "1.6.24 Editor: при прицеле ячейка -- только мигающая рамка (MULT тоже)")
    ...
    present(text, "sourceValue(static_cast<ModSource>(route.auxSource - 1))", "1.6.24 Matrix: AUX SOURCE масштабирует Amount")
    text = read(processor)
    ...
    present(text, "velMap", "1.6.24 Processor: порядок velocity-режимов STEP-первыми отображён в движок")
    present(text, "arp.advance(-static_cast<int>(juce::jlimit(1,static_cast<int>(beatSamples)", "1.6.24 Processor: RETRIG SYNC -- старт арпа по следующей доле хоста")
    text = read(root / "NovaData.h")
    present(text, '"STEP|STEP+KEY|STEP+HOLD|KEY|HOLD"', "1.6.24 Defaults: velocity -- STEP, первые в списке")
    present(text, '"arp_retrig_sync","ARP Retrig sync"', "1.6.24 Defaults: параметр arp_retrig_sync")
