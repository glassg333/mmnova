...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        s.push_back({"arp_retrig_sync","ARP Retrig sync",0,1,0,1,"OFF|ON"}); // 1.6.24: старт арпа по сетке проекта
        for(int row=0;row<16;++row)s.push_back({"r"+juce::String(row)+"_aux","Route "+juce::String(row+1)+" aux source",0,18,0,1,
            "OFF|KEY|VEL|MACRO X|MACRO Y|LFO1|LFO2|LFO3|PITCH WHL|MOD WHL|AFTERTOUCH|MSEG1|STEP VEL|STEP TRANS|ARP GATE|ARP RATE|RANDOM|MSEG2|MSEG3"}); // 1.6.24: AUX SOURCE
        ...
        for(int row=0;row<16;++row) {
            ...
            s.push_back({prefix+"src","Route source",0,15,static_cast<float>(row%16),1,"KEY|VEL|MACRO X|MACRO Y|LFO1|LFO2|LFO3|PITCH WHL|MOD WHL|AFTERTOUCH|MSEG1|STEP VEL|STEP TRANS|ARP GATE|ARP RATE|RANDOM|MSEG2|MSEG3"});
            s.push_back({prefix+"dest","Route target",0,55,0,1,targets}); // 1.6.22: дефолт цели = 0
