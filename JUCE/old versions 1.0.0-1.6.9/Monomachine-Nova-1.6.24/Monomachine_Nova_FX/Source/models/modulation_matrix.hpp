...
enum class ModSource : uint8_t {
    ...
    Aftertouch,
    MSEG,
    MSEG2 = 16,   // 1.6.21: appended (indices of the old sources stay stable)
    MSEG3 = 17,
    StepVelocity,
    StepTranspose,
    ArpGate,
...
public:
    ...
    void evaluate(const std::array<T, N>& baseParams, std::array<T, N>& modulatedParams) const noexcept {
        ...
        for (const auto& route : m_routings) {
            ...
            float source = sourceValue(route.source);
            if (route.polarity == ModPolarity::Unipolar) source = source * 0.5f + 0.5f;
            else if (route.polarity == ModPolarity::Bipolar) source = source * 2.0f - 1.0f;
            if (route.auxSource > 0) source *= sourceValue(static_cast<ModSource>(route.auxSource - 1)); // 1.6.24: AUX SOURCE масштабирует Amount
