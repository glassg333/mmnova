...
// 1.6.5: одинаковые кнопки LFO1/LFO2/LFO3. Клик (без движения) выбирает,
// какой LFO показан в нижней правой панели; пресс-и-тащи в любую ручку и
// отпусти -- назначит маршрут модуляции на эту цель («прицел» как в матрице,
// но без второго клика: отпустил там, где хочешь модулировать).
...
public:
    ...
    std::function<void(juce::Point<int>,bool)> endDrag;
    void mouseDown(const juce::MouseEvent& e) override { PixelButton::mouseDown(e); dragged=false; if(beginDrag) beginDrag(); }
    void mouseDrag(const juce::MouseEvent& e) override { dragged=true; if(dragMove) dragMove(e.getScreenPosition()); }
    void mouseUp(const juce::MouseEvent& e) override { juce::TextButton::mouseUp(e); if(endDrag) endDrag(e.getScreenPosition(),dragged); dragged=false; }
...
public:
    ...
    std::function<void()> openSamples;std::function<void(int)> openEnvelope;std::function<void(int)> openModes;std::function<void(uint8_t)> addMsegModulation;std::function<void(uint8_t)> pickTarget;std::function<void()> openRepitch; // 1.6.6
    ...
    void bind(const juce::String& parameterId,const juce::String& name,int page=-1,int index=-1,int engine=-1,bool modTargetEnabled=true){
        ...
        // 1.6.6: RMB on DTIM opens the delay repitch-speed list (OFF/FAST/MED/SLOW).
        if(pageIndex==2&&knob==3)slider.modeMenu=[this]{if(openRepitch)openRepitch();};
    ...
    void mouseDown(const juce::MouseEvent&e)override{
        ...
        if(pageIndex==2&&knob==3&&e.mods.isPopupMenu()){popupHandled=true;if(openRepitch)openRepitch();return;} // 1.6.6 DTIM repitch list
        if(!parameter)return;
...
// 1.6.6: horizontal step sequencer lane. Left-drag paints transpose (top area)
// or velocity (bottom area) across all steps in one gesture; RMB toggles hold.
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        const int w = getWidth() / 8;
        const int tTop = 4, tH = getHeight() - 108, vTop = getHeight() - 96, vH = 88;
        g.setColour(ink.withAlpha(0.35f)); g.drawRect(0, 0, getWidth(), getHeight(), 1);
        g.drawHorizontalLine(tTop + tH / 2, 0, getWidth()); g.drawHorizontalLine(vTop - 4, 0, getWidth());
        for (int i = 0; i < 8; ++i) {
            ...
            const int x = i * w;
            g.setColour(ink.withAlpha(0.25f)); for (int c = 1; c < 8; ++c) g.drawVerticalLine(c * w, 0, getHeight());
            g.setColour(ink);
            const int mid = tTop + tH / 2;
            const int hgt = juce::roundToInt(tr / 24.0f * (tH / 2 - 4));
            if (hgt >= 0) g.fillRect(x + w / 2 - 4, mid - hgt, 8, hgt + 1); else g.fillRect(x + w / 2 - 4, mid, 8, -hgt + 1);
            const int vh = juce::roundToInt(ve / 127.0f * (vH - 6));
            g.fillRect(x + w / 2 - 4, vTop + vH - vh, 8, vh);
            if (ho > 0.5f) g.fillRect(x + w / 2 - 6, tTop - 2, 12, 3);
            pixel::text(g, juce::String(i + 1), {x, getHeight() - 14, w, 14}, 11, true);
            pixel::text(g, juce::String(static_cast<int>(tr)), {x, tTop, w, 12}, 10, true);
        }
    }
    void mouseDown(const juce::MouseEvent& e) override {
        if (e.mods.isPopupMenu()) { toggleHold(e); return; }
        painting = true; apply(e);
    }
    void mouseDrag(const juce::MouseEvent& e) override { if (painting) apply(e); }
    void mouseUp(const juce::MouseEvent&) override { painting = false; }
private:
    int columnFor(int x) const { return juce::jlimit(0, 7, x / std::max(1, getWidth() / 8)); }
    void toggleHold(const juce::MouseEvent& e) {
        const int pg = juce::jlimit(0, 15, pageOf()), st = columnFor(e.x);
        const auto pre = "arp_s" + juce::String(pg) + "_" + juce::String(st) + "_";
        const float cur = processor.parameters.getRawParameterValue(pre + "hold")->load();
        setParam(pg, st, "hold", cur > 0.5f ? 0.0f : 1.0f); repaint();
    }
    void apply(const juce::MouseEvent& e) {
        const int pg = juce::jlimit(0, 15, pageOf()), st = columnFor(e.x);
        const int tTop = 4, tH = getHeight() - 108, vTop = getHeight() - 96, vH = 88;
        if (e.y < vTop - 4) {
            const float rel = juce::jlimit(0.0f, 1.0f, static_cast<float>(e.y - tTop) / static_cast<float>(tH));
            setParam(pg, st, "transpose", std::round((0.5f - rel) * 48.0f));
        } else {
            const float rel = juce::jlimit(0.0f, 1.0f, 1.0f - static_cast<float>(e.y - vTop) / static_cast<float>(vH));
...
public:
    explicit ArpPage(MonomachineNovaAudioProcessor& p) : processor(p), lane(p, [this] { return page.getSelectedItemIndex(); }, [this](int pg, int st, const char* f, float v) { setStepParameter(pg, st, f, v); }) {
        ...
        addAndMakeVisible(randomButton); randomButton.setButtonText("RANDOM"); randomButton.onClick = [this] { for (int i = 0; i < 8; ++i) randomizeStep(page.getSelectedItemIndex(), i); }; // 1.6.6: instant randomize, RMB keeps the menu
        randomButton.rmb = [this] { showRandomMenu(); };
        ...
        addAndMakeVisible(lane); // 1.6.6: horizontal sequencer replaces the vertical fader list
    ...
    void paint(juce::Graphics& g) override {
        ...
        pixel::text(g, "STEPS", {24, 294, 80, 20}, 15, true); pixel::text(g, "DRAG = PAINT TRANSPOSE (TOP) / VELOCITY (BOTTOM), RMB STEP = HOLD", {120, 296, 700, 18}, 12, false);
...
private:
    ...
    void randomizeStep(int pg, int st) { pg = juce::jlimit(0, 15, pg); st = juce::jlimit(0, 7, st); auto& r = juce::Random::getSystemRandom(); setStepParameter(pg, st, "hold", r.nextInt(2)); setStepParameter(pg, st, "transpose", r.nextInt(49) - 24); setStepParameter(pg, st, "velocity", r.nextInt(128)); lane.repaint(); repaint(); }
    ...
    MonomachineNovaAudioProcessor& processor; juce::ToggleButton arpEnable, arpHold, hostTempo; RndButton randomButton; StepLane lane; juce::ComboBox clock, mode, rateGrid, velocityMode, page; int selectedStep = 0; juce::Slider timeRate, octaves, gate, wrap, pageLimit, random; 
    std::vector<std::unique_ptr<juce::AudioProcessorValueTreeState::ButtonAttachment>> toggleLinks; std::unique_ptr<ComboAttachment> clockLink, modeLink, rateGridLink, velocityModeLink; std::unique_ptr<SliderAttachment> timeRateLink, octavesLink, gateLink, wrapLink, pageLimitLink, randomLink;
};
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        if (sortColumn >= 2 && sortColumn <= 5) { g.setColour(ink.withAlpha(0.5f)); pixel::text(g, sortDescending ? "\xe2\x96\xbc" : "\xe2\x96\xb2", {colX(sortColumn) + colWidth(sortColumn) - 19, 30, 16, 18}, 12, true); }
        for (int r = 0; r < 16; ++r) {
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer {
    explicit Surface(MonomachineNovaAudioProcessor& p):processor(p),machineButton(""),menuButton("MENU"),tempoButton(""),arpButton("ARP"),arpSettingsButton("SET"),matrixButton("MATRIX"),lfo1Button("LFO1"),lfo2Button("LFO2"),lfo3Button("LFO3"),msegButton("MSEG"),msegOutputButton("MSEG OUT"),closeButton("BACK"),gateButton(""){
        ...
        auto wireLfo=[this](ModSourceButton& b,int i){
            b.textHeight=24;
            b.beginDrag=[this,i]{beginModDrag(4+i);};
            b.dragMove=[this](juce::Point<int> s){modDragMove(s);};
            b.endDrag=[this,i](juce::Point<int> s,bool dragged){endModDrag(s,dragged,i);};
            b.setTooltip("LFO"+juce::String(i+1)+": click = show this LFO panel; press, drag to any knob and release = assign LFO"+juce::String(i+1)+" modulation to that target.");
        };
    ...
    void bind(){
        ...
        // 1.6.5: верхняя правая панель -- быстрые настройки MSEG (не цели
        // модуляции), правая нижняя -- выбранный LFO (LFO1/2/3 кнопками).
        cells[2][0]->bind("mseg_rate","RATE",-1,0,-1,false);
        cells[2][1]->bind("mseg_sync","SYNC",-1,1,-1,false);
        cells[2][2]->bind("mseg_loop","LOOP",-1,2,-1,false);
        for(int i=3;i<8;++i)cells[2][static_cast<size_t>(i)]->bind(juce::String(),"---",-1,i,-1,false);
        const int pageForPanel[]={-1,0,-1,1,2,3+lfoPageSel};
        for(int panel=1;panel<6;++panel){if(panel==2)continue;for(int i=0;i<8;++i){int page=pageForPanel[panel];cells[static_cast<size_t>(panel)][static_cast<size_t>(i)]->bind(nova::pageParam(page,i),nova::pageLabel(page,i),page,i);}}
        rebuildModeControls(); // 1.6.6
        lfo1Button.setButtonText(lfoPageSel==0?"[LFO1]":"LFO1");
        lfo2Button.setButtonText(lfoPageSel==1?"[LFO2]":"LFO2");
        lfo3Button.setButtonText(lfoPageSel==2?"[LFO3]":"LFO3");
    ...
    void beginModDrag(int src){modDragSrc=src;modHover=nullptr;for(auto& column:cells)for(auto& cell:column)if(cell)cell->setPickMode(true);}
    void modDragMove(juce::Point<int> screen){auto* cell=cellAtScreen(screen);if(cell!=modHover){modHover=cell;if(cell)cell->flashPick();}}
    void endModDrag(juce::Point<int> screen,bool dragged,int lfoIndex){
        for(auto& column:cells)for(auto& cell:column)if(cell)cell->setPickMode(false);
        const int src=modDragSrc;modDragSrc=-1;modHover=nullptr;
        if(src<0)return;
        if(dragged){auto* cell=cellAtScreen(screen);if(cell&&cell->canBeTarget()){processor.addRouteFromSource(src,cell->targetId());cell->flashPick();}}
        else{lfoPageSel=juce::jlimit(0,2,lfoIndex);bind();}
        repaint();
    }
    ...
    void paint(juce::Graphics& g)override{
        ...
        const juce::String lfoPanelName="LFO"+juce::String(lfoPageSel+1); // 1.6.5
        const juce::String names[]{"SYNT","","MSEG","FILT","EFFX",lfoPanelName};
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.6 / Synth / 1.6.6 fixes":"BUILD 1.6.6 / FX / 1.6.6 fixes",850,570,370,13,juce::Justification::right);
    }
    void resized()override{
        ...
        lfo1Button.setBounds(995,334,80,30);lfo2Button.setBounds(1080,334,80,30);lfo3Button.setBounds(1165,334,80,30);
        msegButton.setBounds(1000,97,90,24);msegOutputButton.setBounds(1096,97,110,24);
    ...
    void showDspModes(int section){
        ...
        auto addSection=[&](int index){
            ...
            juce::PopupMenu sub;
            // папка/категория: к чему относится раздел (машины, ручки, страницы)
            sub.addItem(-1,juce::String("\xd0\x9f\xd0\x90\xd0\x9f\xd0\x9a\xd0\x90: ")+monomachine::dspSectionCategory(index),false,false);
            // 1) основной режим раздела -- всегда первым пунктом
            sub.addItem(index*100+primary+1,
                        juce::String(monomachine::dspModeProvenanceForMode(index,primary)),
                        true, current==primary);
            // 2) резервные режимы -- только те, что имеют смысл в ЭТОМ разделе
            if(monomachine::kShowBackupModes){
                ...
                for(int m=0;m<monomachine::dspModeCount;++m){
                    ...
                    backups.add(juce::String(monomachine::dspModeName(m)));
                }
                if(backups.size()>0){
                    sub.addSeparator();
                    sub.addItem(-1,"\xd0\xa0\xd0\x95\xd0\x97\xd0\x95\xd0\xa0\xd0\x92 (\xd0\xbd\xd0\xb5 \xd0\xbe\xd1\x81\xd0\xbd\xd0\xbe\xd0\xb2\xd0\xbd\xd0\xbe\xd0\xb9):",false,false);
                    for(auto& name:backups){
                        const int m=monomachine::dspModeIndexByName(name.toRawUTF8());
                        if(m<0)continue;
            ...
            menu.addSubMenu(juce::String(monomachine::dspSectionCategory(index))+" / "+juce::String(monomachine::dspSectionLabel(index))+"   ["+juce::String(monomachine::dspModeLabel(current))+"]",sub);
        };
        ...
        // 1.6.5: справка по цепочке (ROUTE) -- описание при наведении, без переключения
        if(section<0||section==monomachine::DspRouting){
            juce::PopupMenu route;
            route.addItem(-1,"\xd0\xa6\xd0\x95\xd0\x9f\xd0\xac \xd0\x9e\xd0\x94\xd0\x9d\xd0\x90 (\xd0\x9f\xd0\xa0\xd0\x9e\xd0\x92\xd0\x95\xd0\xa0\xd0\x95\xd0\x9d\xd0\x90), \xd0\x9f\xd0\x95\xd0\xa0\xd0\x95\xd0\x9a\xd0\x9b\xd0\xae\xd0\xa7\xd0\x90\xd0\xa2\xd0\x95\xd0\x9b\xd0\xac \xd0\xa3\xd0\x91\xd0\xa0\xd0\x90\xd0\x9d \xd0\x92 1.6.5",false,false);
            ...
            menu.addSubMenu("\xd0\xa6\xd0\x95\xd0\x9f\xd0\xac / ROUTE   [fixed]",route);
        }
        juce::Component::SafePointer<Surface> safe(this);
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&dspModeButton).withMinimumWidth(560),[safe](int result){
    ...
    void updateDspModeLabel(){
        ...
        // как она влияет на звук и почему при дефолте FX не меняет источник.
        juce::String tip="DSP MODE -- \xd0\xbf\xd0\xb0\xd0\xbf\xd0\xba\xd0\xb8 \xd1\x80\xd0\xb0\xd0\xb7\xd0\xb4\xd0\xb5\xd0\xbb\xd0\xbe\xd0\xb2, \xd0\xbe\xd1\x81\xd0\xbd\xd0\xbe\xd0\xb2\xd0\xbd\xd0\xbe\xd0\xb9 \xd1\x80\xd0\xb5\xd0\xb6\xd0\xb8\xd0\xbc mnm:\n";
        for(int i=0;i<monomachine::DspSectionCount;++i){
            ...
            tip+= (mode==monomachine::dspModeMnm) ? " (\xd0\xbe\xd1\x81\xd0\xbd\xd0\xbe\xd0\xb2\xd0\xbd\xd0\xbe\xd0\xb9)" : " (\xd1\x80\xd0\xb5\xd0\xb7\xd0\xb5\xd1\x80\xd0\xb2)";
    ...
    // 1.6.6: RMB on DTIM -- how hard repeats repitch while sweeping delay time.
    void showRepitchMenu(){
        auto* p=processor.parameters.getParameter("dly_repitch");if(!p)return;
        const int current=juce::roundToInt(p->convertFrom0to1(p->getValue()));
        juce::PopupMenu menu;
        menu.addItem(-1,"REPITCH: \xd0\xba\xd0\xb0\xd0\xba \xd1\x81\xd0\xb8\xd0\xbb\xd1\x8c\xd0\xbd\xd0\xbe \xd0\xbc\xd0\xb5\xd0\xbd\xd1\x8f\xd0\xb5\xd1\x82\xd1\x81\xd1\x8f \xd0\xbf\xd0\xb8\xd1\x82\xd1\x87 \xd0\xbf\xd0\xbe\xd0\xb2\xd1\x82\xd0\xbe\xd1\x80\xd0\xbe\xd0\xb2 \xd0\xbf\xd1\x80\xd0\xb8 \xd0\xbf\xd1\x80\xd0\xbe\xd0\xba\xd1\x80\xd1\x83\xd1\x82\xd0\xba\xd0\xb5 DTIM",false,false);
        const char* names[4]={"OFF (\xd0\xbc\xd0\xb3\xd0\xbd\xd0\xbe\xd0\xb2\xd0\xb5\xd0\xbd\xd0\xbd\xd0\xbe, \xd0\xba\xd0\xbb\xd0\xb8\xd0\xba\xd0\xb8 \xd0\xb2\xd0\xbe\xd0\xb7\xd0\xbc\xd0\xbe\xd0\xb6\xd0\xbd\xd1\x8b)","FAST","MED","SLOW"};
        for(int i=0;i<4;++i)menu.addItem(i+1,names[i],true,i==current);
        juce::Component::SafePointer<Surface> safe(this);
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this).withMinimumWidth(420),[safe](int result){
            if(!safe||result<=0)return;
            auto* par=safe->processor.parameters.getParameter("dly_repitch");if(!par)return;
            par->beginChangeGesture();par->setValueNotifyingHost(par->convertTo0to1(static_cast<float>(result-1)));par->endChangeGesture();});
    }
    // 1.6.6: MODE selectors on page headers (like the envelope REFERENCE FIT):
    void rebuildModeControls(){
        auto wire=[this](juce::ComboBox& c,const char* paramId,std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment>& link,const char* a,const char* b){
        ...
        else wire(syntModeCombo,"mode_synt",syntModeLink,"mnm","old");
        ...
        syntModeCombo.setTooltip("SYNT mode (mnm = firmware DSP, old = reserve). For SWAVE-SAW: oscillator algorithm.");
    ...
    PixelButton ampButton{"AMP"};juce::ComboBox ampMode;juce::ComboBox syntModeCombo,filtModeCombo,effxDistCombo,effxDlyCombo;std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment> syntModeLink,filtModeLink,effxDistLink,effxDlyLink; // 1.6.6
    ...
    juce::TooltipWindow tooltips{this,800};int lfoPageSel=1;int modDragSrc=-1;Cell* modHover=nullptr;int boundMachine=-1;bool pickingTarget=false;int pickRoute=-1,pickHoverTarget=-1;
};
