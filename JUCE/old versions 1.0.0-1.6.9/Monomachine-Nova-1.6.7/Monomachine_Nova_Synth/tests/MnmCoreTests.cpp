...
int main() {
    ...
    // ---------------------------------------------------------------- delay
    {
        ...
        // выход -- чистый wet.
        delay.process(127.0f, 0.0f, 1.0f, false, 0.0f, 1.0f, 1.0f, wl, wr);
        int samples = 1;
        for (; samples < 44100 * 2; ++samples) {
            delay.process(127.0f, 0.0f, 0.0f, false, 0.0f, 0.0f, 0.0f, wl, wr);
        ...
        shortDelay.process(0.0f, 0.0f, 1.0f, false, 0.0f, 1.0f, 1.0f, wl, wr);
        samples = 1;
        for (; samples < 44100; ++samples) {
            shortDelay.process(0.0f, 0.0f, 0.0f, false, 0.0f, 0.0f, 0.0f, wl, wr);
            if (std::fabs(wl) > 1.0e-5f) break;
        }
        ...
        for (int i = 0; i < 2048; ++i) unity.process(64.0f, 0.5f, 0.0f, false, 0.03f, 0.7f, -0.7f, ul, ur);
        ...
        auto fillRamp = [](DelayCore& d) {
            ...
            for (int i = 0; i < 44104; ++i) {
                ...
                d.process(127.0f, 0.0f, 1.0f, false, 0.0f, v, v, a, b);
            }
        };
        DelayCore noslew;
        ...
        for (int i = 0; i < 64; ++i) { // резкая смена DTIM 127->40 без сглаживания
            noslew.process(40.0f, 0.0f, 0.0f, false, 0.0f, 0.0f, 0.0f, nl, nr);
        ...
        for (int i = 0; i < 4410; ++i) { // та же смена, но slew=MED (0.03 с)
            slewed.process(40.0f, 0.0f, 0.0f, false, 0.03f, 0.0f, 0.0f, sl, srOut);
    ...
    // ---------------------------------------------------------------- 1.6.6 laws
    {
        ...
        d.process(0.0f, 0.5f, 0.0f, true, 0.0f, 0.0f, 1.0f, ol, orr);
        ...
        for (int i = 0; i < 1200; ++i) { const float inR = (i == 0) ? 1.0f : 0.0f; d2.process(0.0f, 0.0f, 1.0f, false, 0.0f, 0.0f, inR, wl, wr); maxR = std::max(maxR, wr); }
        checkTrue(maxR > 0.9f, "DLY 1.6.6 stereo send=1 returns the written impulse");
        monomachine::mnm::DelayCore d3; d3.prepare(44100.0);
        float maxL = 0.0f;
        for (int i = 0; i < 1200; ++i) { const float inR = (i == 0) ? 1.0f : 0.0f; d3.process(0.0f, 0.0f, 1.0f, true, 0.0f, 0.0f, inR, wl, wr); maxL = std::max(maxL, wl); }
