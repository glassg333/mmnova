...
public:
    ...
    void refreshNames(){
        ...
        for(int k=0;k<8;++k){
            if(cur<=1)choices.add(mi>=0?juce::String(nova::machines()[static_cast<size_t>(mi)].synthParams[static_cast<size_t>(k)].name):juce::String(k)); // 1.7.6 FIX: PTCH/SYNT -- ручки машины
            else{juce::String lbl=nova::pageLabel(cur-2,k);if(cur-2>=3)lbl="LFO"+juce::String(cur-4)+" "+lbl;choices.add(lbl);} // 1.7.6 FIX: строка таблицы = cur-2 (было cur-1 -- сдвиг на одну, LFO3 читала ЗА таблицей: мусорные TOM/CLAP/RIM/COW)
        }
...
public:
    ...
    void resized() override {
        ...
        slotBtn.setBounds(670, 312, 44, 26); detachBtn.setBounds(1098, 12, 52, 21); // 1.7.6: DETACH чуть выше -- справа в углу страницы
...
private:
    ...
    void randomizePlock(int pg0, int pg1, bool doVals, bool doSlide) { // 1.7.5 FIX: текущий слот, если пуст -- первый занятый (кнопки PLK/SLIDE снова работают)
        ...
        if (processor.plockTarget(sl) < 0) { sl = -1; for (int s = 0; s < processor.kPlockSlots; ++s) if (processor.plockTarget(s) >= 0) { sl = s; break; }
            if (sl < 0) return; } // 1.7.6: фолбэк на первый занятый
    ...
    void setCurSlot(int s) { plockSlot = juce::jlimit(0, processor.kPlockSlots - 1, s); // 1.7.6: текст кнопки не трогаем -- она теперь P-LIST
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        { const float ph = static_cast<float>(juce::jlimit(0.0, 1.0, processor.msegPhaseUi[static_cast<size_t>(page)].load())); const int px = juce::roundToInt(pointToX(ph)); // 1.7.6: PLAY FOLLOWER -- где сейчас играет MSEG
          if (px >= graphX && px <= graphX + graphW) { g.setColour(juce::Colours::white); g.drawVerticalLine(px, float(graphY), float(graphY + graphH)); g.fillRect(float(px) - 3.0f, float(graphY - 8), 6.0f, 6.0f); } }
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.7.6 / Synth":"BUILD 1.7.6 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
    ...
    void showEnvelope(int knob){dismissDock();viewport.setViewedComponent(nullptr,false);settings.reset();matrixPage.reset();samplePage.reset();msegPage.reset();envelopePage=std::make_unique<EnvelopePage>(processor,false);
    ...
    struct ArpDialog final : juce::DialogWindow { std::function<void()> onX; ArpDialog():juce::DialogWindow("ARP",juce::Colours::black,true,true){} void closeButtonPressed() override { if(onX)onX(); } }; // 1.7.6: крестик окна работает
    void detachArp(){ // 1.7.6: окно = ТОТ ЖЕ класс ArpPage (копия раскладки), списки/галки в стиле мода; исходная страница закрывается -- панель свободна
        ...
        auto* win = new ArpDialog(); win->onX = [this] { if (arpWindow) arpWindow->setVisible(false); };
        arpWindow.reset(win);
        arpWindow->setLookAndFeel(&getLookAndFeel()); // 1.7.6: заголовок чёрный, MODE:STEP и списки не джус-дефолт
        arpWindow->setContentOwned(page, true);
        arpWindow->centreWithSize(page->getWidth(), page->getHeight() + 28); arpWindow->setVisible(true); arpWindow->toFront(true);
        closeOverlay(); } // 1.7.6: страница ARP в оверлее закрывается, ручки лицевой панели доступны
 ...
 // 1.6.30: новый стандарт -- список НИЖЕ своего хитбокса
    ...
    void toggleEnvMini(){if(dockPanel!=nullptr){dismissDock();return;}if(envelopePage!=nullptr)return; // 1.6.44: мини-ENV НЕ поверх большого окна
        showDocked(std::make_unique<EnvelopePage>(processor,true),juce::Rectangle<int>(471,95,387,236),false);
    ...
    void showSettings(bool matrix){if(!matrix&&arpWindow){arpWindow->setVisible(true);arpWindow->toFront(true);return;} setViewportMode(matrix);} // 1.7.6: ARP при детаче = показать окно, страницу заново не создавать
