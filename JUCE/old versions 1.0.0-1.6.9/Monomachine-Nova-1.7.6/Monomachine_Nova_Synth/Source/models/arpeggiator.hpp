...
public:
    ...
    ArpNoteEvent poll() {
        ...
        if (settings.play == Play::Step || settings.play == Play::StepChord) {
            ...
            if (newPass && settings.stepRandom > 0.5f) { // 1.7.6 FIX: как в старой версии (STEP RND до решафла) -- случайная страница из ВСЕХ 16; PAGE LIMIT остаётся длиной цикла AUTO SWAP
                rng ^= rng << 13; rng ^= rng >> 17; rng ^= rng << 5;
                randomPage = static_cast<uint8_t>(rng % 16u); }
