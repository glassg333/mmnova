...
public:
    ...
    static constexpr size_t kTargetCount = 83; // 1.7.5: +PITCH (66) + ROUTE 1..16 (67..82) -- модуляция глубины соседних маршрутов
    ...
    void evaluate(const std::array<T, N>& baseParams, std::array<T, N>& modulatedParams, float* msegDests = nullptr, float* arpDests = nullptr, float* routeExtra = nullptr) const noexcept { // 1.7.1: +arpDests (0=RATE,1=GATE,2=PITCH); 1.7.5: +routeExtra (прибавки к глубинам маршрутов)
        ...
        if (routeExtra != nullptr) // 1.7.5: pass 1 -- источники, целящиеся в маршруты 67..82, прибавляются к ГЛУБИНЕ той строки
            for (const auto& route : m_routings) {
                if (!route.enabled || route.depth == 0 || route.targetParamId < 67 || route.targetParamId >= kTargetCount) continue;
        ...
        for (const auto& route : m_routings) {
            if (!route.enabled || route.depth == 0 || route.targetParamId >= 67) continue; // 1.7.5 FIX: ROUTE-цели обработаны в pass 1 -- попадание в ветку arpDests[67-64] было записью ЗА ГРАНИЦЕЙ массива
            const float depth = route.targetParamId < 67 && routeExtra != nullptr && route.targetParamId < 16
