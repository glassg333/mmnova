...
public:
    ...
    void mouseWheelMove(const juce::MouseEvent&,const juce::MouseWheelDetails& w)override{if(getNumItems()<2||std::abs(w.deltaY)<0.01f)return;setSelectedItemIndex(juce::jlimit(0,getNumItems()-1,getSelectedItemIndex()+(w.deltaY>0?-1:1)),juce::sendNotificationSync);} // 1.6.33: колесо листает свой список, не страницу
...
public:
    explicit ArpPage(MonomachineNovaAudioProcessor& p) : processor(p), lane(p, [this] { return pageDisplay(); }, [this](int pg, int st, const char* f, float v) { setStepParameter(pg, st, f, v); }), // 1.6.42: FOLLOW PLAY
        ...
        const std::pair<const char*,juce::Rectangle<int>> heads[]{{"hMODE",{127,25,66,14}},{"hOCT",{198,25,60,14}},{"hRATE",{237,25,60,14}},{"hGATE",{302,25,60,14}},{"hVEL",{352,25,95,14}},{"hWRAP",{454,25,52,14}},{"hASWAP",{517,21,70,13}},{"hFOLLOW",{586,28,86,13}},{"hPAGE",{695,25,52,14}},{"hPAGERND",{795,25,90,14}},{"hRANDOM",{893,25,110,14}}}; // 1.6.43: заголовки ARP -- теги LAYOUT EDIT
    ...
    void paint(juce::Graphics& g) override {
        ...
        drawHead("hPAGE","PAGE",{695,25,52,14},14); drawHead("hPAGERND","PAGE RND",{795,25,90,14},14);
    ...
    void resized() override {
        ...
        pageRndToggle.setBounds(808, 45, 26, 26);
        pageRndButton.setBounds(869, 45, 63, 26); pitchRndButton.setBounds(937, 45, 63, 26); velRndButton.setBounds(1005, 45, 63, 26); holdRndButton.setBounds(1073, 45, 63, 26); // 1.6.42: без ALL -- ПКМ на PAGE = ALL (было и в 1.6.25)
...
private:
    ...
    MonomachineNovaAudioProcessor& processor; juce::ToggleButton arpEnable, arpHold, hostTempo, autoSwapToggle, followPlayToggle; PixelButton rateButton{""}, pageRndButton{"PAGE"}, pitchRndButton{"PITCH"}, velRndButton{"VEL"}, holdRndButton{"HOLD"}; StepLane lane; // 1.6.42: STEP RND и ALL убраны (AUTO SWAP и ПКМ на PAGE)
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        // 1.7.0: описание ROUTING / SIGNED AMOUNT удалено (просьба)
        int row = 0;
        for (int r = 0; r < 16 && row < 8; ++r) { // 1.6.29: тот же порядок и размер строк, что в матрице
            ...
            const int target = juce::jlimit(0, 63, dRaw - 1);
            const int y = routeY() + row * 34;
            g.setColour(ink); g.drawRect(16, y + 2, 34, 26, 1); // 1.6.31/34: гнездо прицела -- клик = прицел (как DEST в матрице)
            ...
            g.setColour(ink.withAlpha(0.25f)); g.drawHorizontalLine(y + 14, 395.0f, 585.0f); // 1.6.35: ампаунт -- бар 395..585, значение и стирание ПРИДВИНУТЫ
            g.setColour(ink); g.fillEllipse(395.0f + 190.0f * frac - 5.0f, float(y + 9), 10.0f, 10.0f);
            pixel::text(g, (raw < 0 ? "-" : raw > 0 ? "+" : juce::String()) + juce::String(juce::roundToInt(std::abs(raw) * 100.0f / (raw < 0 ? 64.0f : 63.0f))) + "%", {592, y, 60, 22}, 14, false);
            g.setColour(ink); g.drawRect(656, y + 2, 20, 18, 1); g.drawLine(661.0f, float(y + 7), 671.0f, float(y + 15), 2.0f); g.drawLine(671.0f, float(y + 7), 661.0f, float(y + 15), 2.0f);
        ...
        for (int rest = row; rest < 3; ++rest) { const int y2 = routeY() + rest * 34; // 1.6.34: UNASSIGNED -- гнездо прицела как у активных строк
            g.setColour(ink); g.drawRect(16, y2 + 2, 34, 26, 1);
            const int cx2 = 33, cy2 = y2 + 15;
    ...
    void resized() override {
        ...
        { static const int tx[]{336, 468, 608, 743, 892, 1024}; // 1.7.0: из JSON пользователя
          for (int i = 0; i < toolButtons.size() && i < 6; ++i) toolButtons.getUnchecked(i)->setBounds(tx[i], 420, 132, 26); }
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        for (int row = 0; row < 8; ++row) { const int rr = routeRows[static_cast<size_t>(row)]; if (rr < 0) continue;
            const int y = routeY() + row * 34;
            if (e.x >= 654 && e.x <= 678 && e.y >= y && e.y <= y + 30) { if (auto* on = processor.parameters.getParameter("r" + juce::String(rr) + "_on")) { on->beginChangeGesture(); on->setValueNotifyingHost(on->convertTo0to1(0.0f)); on->endChangeGesture(); } repaint(); return; } // 1.6.35: кнопка стирания рядом с баром
            if (e.x >= 393 && e.x <= 587 && e.y >= y - 3 && e.y <= y + 28) { routeDrag = rr; routeBase = processor.parameters.getRawParameterValue("r" + juce::String(rr) + "_depth")->load(); routeY0 = static_cast<int>(e.x); if (auto* d = processor.parameters.getParameter("r" + juce::String(rr) + "_depth")) d->beginChangeGesture(); return; } // 1.6.35: зона драга = бар 395..585
        }
        for (int row = 0; row < 8; ++row) { const int rr = routeRows[static_cast<size_t>(row)]; if (rr < 0) continue; // 1.6.31: прицел с гнезда-номера строки маршрута -- как в основной матрице
            const int y = routeY() + row * 34;
            if (e.x >= 14 && e.x <= 52 && e.y >= y && e.y <= y + 30 && !e.mods.isPopupMenu()) { if (pickStart) pickStart(page, row); return; } } // 1.6.34: гнездо = ПРИЦЕЛ (клик-клик), не провод
        for (int rest = 0; rest < 3; ++rest) { const int y2 = routeY() + rest * 34; // 1.6.34: прицел из UNASSIGNED-строки
            if (e.x >= 14 && e.x <= 52 && e.y >= y2 && e.y <= y2 + 30 && !e.mods.isPopupMenu()) { if (pickStart) pickStart(page, rest); return; } }
...
private:
    ...
    static const std::array<juce::String, 64>& targetNames() { static const auto names = [] { std::array<juce::String, 64> a{}; for (int i = 0; i < 64; ++i) a[static_cast<size_t>(i)] = juce::String(monomachine::ModulationMatrix::getDestinationName(static_cast<uint8_t>(i))); return a; }(); return names; } // 1.6.29: +MSEG OUT
    ...
    juce::ToggleButton loop, sync; juce::Slider rate, shift; DragChoice shape, grid, viewGrid; PixelButton linkBtn{"LINK"}; // 1.6.33: логика DEST-матрицы (клик = список после отпускания, ЛКМ-драг = листать); WheelCombo открывал список по НАЖАТИЮ -- драг триггерил пункт
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.7.0 / Synth":"BUILD 1.7.0 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
    ...
    void setViewportMode(bool matrix){
        ...
        else{matrixPage.reset();settings=std::make_unique<ArpPage>(processor);settings->pushEdit=[this]{pushUndoPoint();};viewport.setViewedComponent(settings.get(),false);}
 ...
 // 1.6.30: новый стандарт -- список НИЖЕ своего хитбокса
    ...
    juce::TooltipWindow tooltips{this,800};int lfoPageSel=0;int modDragSrc=-1;bool modDragMatrix=false;Cell* modHover=nullptr;int boundMachine=-1;bool pickingTarget=false;int pickRoute=-1,pickHoverTarget=-1;float machineAccum=0,arpAccum=0;juce::uint32 lastMachineCycleMs=0;std::unique_ptr<PopupDock> dock;std::unique_ptr<juce::Component> dockPanel; // 1.6.13: дробные аккумуляторы скорости драга
