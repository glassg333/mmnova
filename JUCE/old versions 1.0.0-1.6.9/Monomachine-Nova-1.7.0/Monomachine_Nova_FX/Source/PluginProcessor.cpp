...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    for(size_t r=0;r<16;++r){const int dvv=juce::jlimit(0,64,juce::roundToInt(routeRaw[r][2]->load())); // 1.6.32: DEST 0 = OFF, цель = значение-1
    ...
    settings.stepPage=static_cast<uint8_t>(juce::jlimit(0,15,juce::roundToInt(global[ArpStepPage])));settings.stepPageLimit=static_cast<uint8_t>(juce::jlimit(1,16,juce::roundToInt(global[ArpStepPageLimit])));settings.stepRandom=juce::jlimit(0.0f,1.0f,global[ArpStepRandom]/100.0f);settings.autoSwap=arpAutoSwap.load(); // 1.6.42: AUTO SWAP PAGE (галка ARP-страницы)
