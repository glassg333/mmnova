...
public:
    ...
    ArpNoteEvent poll() {
        ...
        if (settings.play == Play::Step || settings.play == Play::StepChord) {
            ...
            if (newPass && settings.stepRandom > 0.5f) { // 1.7.5 FIX: случайная страница В ПРЕДЕЛАХ лимита от стартовой (rng%16 выбирал из всех 16 -- часто пустые, казалось что не рандомит)
                rng ^= rng << 13; rng ^= rng >> 17; rng ^= rng << 5;
                const size_t span = std::max<size_t>(1, std::min<size_t>(settings.stepPageLimit, 16u - settings.stepPage));
                randomPage = static_cast<uint8_t>(settings.stepPage + rng % span); }
            uint8_t playPage = settings.stepRandom > 0.5f ? randomPage : settings.stepPage;
    ...
    uint8_t stepEcho = 0; uint8_t randomPage = 0; uint8_t playingPage = 0; // 1.6.42: страница, играющая СЕЙЧАС (FOLLOW PLAY)
