...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        targets+="|PITCH";for(int rn=0;rn<16;++rn){targets+="|ROUTE "+juce::String(rn+1);} // 1.7.5: PITCH (66) + ROUTE 1..16 (глубина соседних маршрутов) // 1.6.29: +MSEG OUT; 1.7.1: +папка ARP (64, 65)G1..8 OUT (56..63)
        for(int row=0;row<16;++row) {
            ...
            s.push_back({prefix+"dest","Route target",0,82,0,1,targets}); // 1.6.22: дефолт цели = 0; 1.6.29: 64 цели
