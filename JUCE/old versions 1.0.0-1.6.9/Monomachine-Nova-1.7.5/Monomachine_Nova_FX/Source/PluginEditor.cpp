...
static juce::String targetName(MonomachineNovaAudioProcessor& p,int target){
    ...
    if(target==66)return "PITCH"; // 1.7.5: высота тона как цель
...
public:
    explicit DsndPanel(MonomachineNovaAudioProcessor& p) : processor(p), pp(p, "dly_ppmode", 0.0, 1.0, 0.0) {
        addAndMakeVisible(pp); setSize(238, 90); // 1.7.5: окно по содержимому, не 320x96
    }
    void paint(juce::Graphics& g) override {
        g.fillAll(juce::Colours::black); g.setColour(ink);
        g.drawRect(0,0,getWidth(),getHeight(),1); // 1.6.42: белая обводка доп-панели
        drawPin(g); // 1.6.44: булавка
        pixel::text(g,"DSND / DELAY SEND",{6,3,226,18},13,false);
        pixel::text(g,"PP MODE",{6,36,86,18},13,false);
        auto* ppm=processor.parameters.getParameter("dly_ppmode");
        pixel::text(g,ppm?(juce::roundToInt(ppm->convertFrom0to1(ppm->getValue()))==1?"MID SAFE":"CLASSIC"):juce::String("---"),{96,36,136,18},13,true);
    }
...
public:
    explicit ArpPage(MonomachineNovaAudioProcessor& p) : processor(p), lane(p, [this] { return pageDisplay(); }, [this](int pg, int st, const char* f, float v) { setStepParameter(pg, st, f, v); }), // 1.6.42: FOLLOW PLAY
        ...
        detachBtn.setButtonText("DETACH"); detachBtn.textHeight = 12; addAndMakeVisible(detachBtn); // 1.7.5: ARP -- отдельным окном
        ...
        slotBtn.setButtonText("P-LIST"); slotBtn.textHeight = 12; addAndMakeVisible(slotBtn); // 1.7.5: P-LIST -- клик = список, драг = листать слоты
        slotBtn.setTooltip("P-LIST: click = list of bound parameters (N - name; ALL keep playing). LMB drag = cycle slots without the list. RMB = list.");
        ...
        slotBtn.onClick = [this] { if (juce::Time::getMillisecondCounter() - slotDraggedMs < 400) return; showDestList(); }; // 1.7.5: после драга список НЕ вызывается
    ...
    void resized() override {
        ...
        slotBtn.setBounds(670, 312, 44, 26); detachBtn.setBounds(1098, 20, 52, 21); // 1.7.5: DETACH -- справа в углу страницы
...
private:
    ...
    void showDestList() { // список параметров: автоматизированные (слоты) сверху с индикацией, остальные по папкам матрицы
        ...
        { juce::PopupMenu sub; sub.addItem(10 + 64, "ARP RATE", true, processor.plockTarget(curSlot()) == 64);
          ...
          sub.addItem(10 + 66, "PITCH", true, processor.plockTarget(curSlot()) == 66); m.addSubMenu("ARP", sub); } // 1.7.5: PITCH -- полутоны ±24
        const juce::Component::SafePointer<ArpPage> safe(this);
    ...
    std::unique_ptr<PlockDestBox> destBox; PixelButton slotBtn{"S1"}, detachBtn{"DETACH"}; int plockSlot = 0; public: std::function<void()> detachRequest; // 1.7.2/5: public -- замыкает Surface
...
public:
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        if (e.y < 56) { const int c = colAt(e.x);
            if (c >= 1 && std::abs(e.x - (colX(c) - 3)) <= 4) { colDrag = c; colDragX = e.x; return; } // 1.7.5 FIX: хитбокс = видимая линия разделителя (она на colX-3) полосу номеров строк = сдвиг старта колонок от начала матрицы) (1.6.41: ресайз колонки -- тянуть границу в шапке)
    ...
    void mouseDrag(const juce::MouseEvent& e) override {
        ...
        if (colDrag > 0) { const int nx = static_cast<int>(e.getScreenPosition().x); colw[(size_t)colDrag - 1] = juce::jlimit(colMin(colDrag - 1), 400, colw[(size_t)colDrag - 1] + nx - colDragX); colDragX = nx; normalizeCols(); repaint(); return; } // 1.7.5: текст колонки не схлопывается (минимум по колонке) // 1.6.41: ресайз колонки, правый край неподвижен
    ...
    void openValueMenu(int r, int c, juce::Point<int> screen) { // 1.6.24: единый список значений ячейки
        ...
        if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; for (int i = 0; i < 9; ++i) menu.addItem(i + 1, signedPercent(values[i])); } // -100% ... 0% ... +100%
            ...
            { juce::PopupMenu arpSub; // 1.7.1: папка ARP -- rate и gate как цели маршрутов
              ...
              arpSub.addItem(68, "PITCH", true, selected == 67); // 1.7.5: высота тона как цель
              menu.addSubMenu("ARP", arpSub); }
            { juce::PopupMenu mxSub; // 1.7.5: цели-маршруты: номер строки и то, что она модулирует (модулируют её ГЛУБИНУ)
              for (int n = 0; n < 16; ++n) { auto* dp = processor.parameters.getRawParameterValue("r" + juce::String(n) + "_dest");
                  const int dv = dp ? juce::roundToInt(dp->load()) : 0;
                  mxSub.addItem(69 + n, juce::String(n + 1) + " - " + (dv > 1 ? targetName(processor, juce::jlimit(0, 82, dv - 1)) : juce::String("OFF")), true, selected == 68 + n); }
...
private:
    juce::String destName(int id) const { if (id >= 67 && id < 83) { auto* dp = processor.parameters.getRawParameterValue("r" + juce::String(id - 67) + "_dest"); const int dv = dp ? juce::roundToInt(dp->load()) : 0; return juce::String(id - 66) + " - " + (dv > 1 ? targetName(processor, juce::jlimit(0, 82, dv - 1)) : juce::String("OFF")); } return targetName(processor, id); } // 1.7.5: строки-маршруты в DEST -- номер и то, что модулирует
...
public:
    ...
    void resized() override {
        ...
        retrig.setBounds(920, 70, 72, 26); // 1.7.5 FIX: 150/600 -- обе точки перекрывались соседними комбами (SHAPE, GRID/LINK); свободная щель после SHIFT
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        ...
        for (int rest = 0; rest < 3; ++rest) { const int y2 = routeY() + rest * 34; // 1.6.34: прицел из UNASSIGNED-строки
            if (e.x >= rowX - 2 && e.x <= rowX + 36 && e.y >= y2 && e.y <= y2 + 30 && !e.mods.isPopupMenu()) { if (destListRequest) destListRequest(page, rest); return; } } // 1.7.5: UNASSIGNED -- список целей (папки как в P-LIST)
...
private:
    ...
    static const std::array<juce::String, 83>& targetNames() { static const auto names = [] { std::array<juce::String, 83> a{}; for (int i = 0; i < 83; ++i) a[static_cast<size_t>(i)] = juce::String(monomachine::ModulationMatrix::getDestinationName(static_cast<uint8_t>(i))); return a; }(); return names; } // 1.6.29: +MSEG OUT; 1.7.1: +ARP RATE/GATE
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.7.5 / Synth":"BUILD 1.7.5 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
    ...
    bool envMini=false; // 1.7.5
    void showEnvelope(int knob,bool mini=false){envMini=mini;dismissDock();viewport.setViewedComponent(nullptr,false);settings.reset();matrixPage.reset();samplePage.reset();msegPage.reset();envelopePage=std::make_unique<EnvelopePage>(processor,mini);envelopePage->toggleSize=[this]{showEnvelope(-1,!envMini);};viewport.setViewedComponent(envelopePage.get(),false);viewport.setVisible(true);viewport.toFront(false);closeButton.setVisible(true);closeButton.toFront(false);if(knob>=0)envelopePage->focusCurve(knob);} // 1.7.5: ПКМ -- малое/большое
    ...
    void detachArp(){ // 1.7.5: ARP-страница отдельным окном (два окна: ARP + лицевая панель)
        if (arpWindow) { arpWindow->setVisible(!arpWindow->isVisible()); if (arpWindow->isVisible()) arpWindow->toFront(true); return; }
        auto* page = new ArpPage(processor);
        page->pushEdit = [this] { pushUndoPoint(); }; page->armRequest = [this](int slot) { beginPlockPick(slot); };
        page->detachRequest = [this] { if (arpWindow) arpWindow->setVisible(false); };
        arpWindow = std::make_unique<juce::DialogWindow>("ARP", juce::Colours::black, true, true);
        arpWindow->setContentOwned(page, true); arpWindow->centreWithSize(1160, 478); arpWindow->setVisible(true); arpWindow->toFront(true); }
    ...
    juce::String matrixRouteName(int r) const { // 1.7.5: динамическое имя маршрута -- номер строки и то, что она модулирует
        auto* d = processor.parameters.getRawParameterValue("r" + juce::String(r) + "_dest"); const int v = d ? juce::roundToInt(d->load()) : 0;
        return v > 0 ? targetName(processor, juce::jlimit(0, 82, v - 1)) : juce::String("OFF"); }
    void showMsegDestList(int pg, int row) { // 1.7.5: UNASSIGNED-строка MSEG -- список целей (папки как в P-LIST), выбор создаёт маршрут
        juce::PopupMenu m; m.setLookAndFeel(&getLookAndFeel());
        static const char* groups[]{"MACHINE","AMP","FILTER","EFFX","LFO1","LFO2","LFO3"};
        for (int g = 0; g < 7; ++g) { juce::PopupMenu sub; for (int k = 0; k < 8; ++k) sub.addItem(10 + g * 8 + k, targetName(processor, g * 8 + k), true, false); m.addSubMenu(groups[g], sub); }
        { juce::PopupMenu sub; for (int p2 = 0; p2 < 8; ++p2) sub.addItem(10 + 56 + p2, "MSEG" + juce::String(p2 + 1) + " OUT", true, false); m.addSubMenu("MSEG", sub); }
        { juce::PopupMenu sub; sub.addItem(10 + 64, "ARP RATE", true, false); sub.addItem(10 + 65, "ARP GATE", true, false); sub.addItem(10 + 66, "PITCH", true, false); m.addSubMenu("ARP", sub); }
        { juce::PopupMenu sub; for (int n = 0; n < 16; ++n) sub.addItem(10 + 67 + n, juce::String(n + 1) + " - " + matrixRouteName(n), true, false); m.addSubMenu("MATRIX", sub); } // 1.7.5: модулировать соседние модуляции
        const juce::Component::SafePointer<Surface> safe(this);
        m.showMenuAsync(juce::PopupMenu::Options(), [safe, pg, row](int res) { if (!safe || res <= 0) return; const int tg = juce::jlimit(0, 82, res - 10);
