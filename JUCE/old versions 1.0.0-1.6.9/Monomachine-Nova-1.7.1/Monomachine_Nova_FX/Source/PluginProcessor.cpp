...
void MonomachineNovaAudioProcessor::trigger(int note,float vel,int channel){
    ...
    if(globalRaw[static_cast<size_t>(MsegRetrig)]==nullptr||globalRaw[static_cast<size_t>(MsegRetrig)]->load()>0.5f){msegPhase.fill(0.0); for(int mi=0;mi<8;++mi)msegValue[static_cast<size_t>(mi)]=mseg[mi].value(0.0f);} // 1.7.1: галка RETRIG -- нота MIDI/арп перезапускает фазы (дефолт ON = как было)
    liveArpGate=1.0f;
...
void MonomachineNovaAudioProcessor::render(juce::AudioBuffer<float>& buffer,int start,int length){
    ...
    while(length>0){
        ...
        std::array<float,8> msegMod{};std::array<float,2> arpMod{};matrix.evaluate(before,after,msegMod.data(),arpMod.data()); // 1.7.1: +цели ARP RATE/GATE
...
void MonomachineNovaAudioProcessor::getStateInformation(juce::MemoryBlock& out){
    ...
    { auto pl=juce::ValueTree("PLOCK"); // 1.7.1: слоты P-LOCK в состояние
        for(int s=0;s<kPlockSlots;++s){auto n=juce::ValueTree("S");n.setProperty("slot",s,nullptr);n.setProperty("target",plockTarget(s),nullptr);n.setProperty("pol",plockPolarity(s),nullptr);
...
void MonomachineNovaAudioProcessor::setStateInformation(const void* data,int size){
    ...
    if(auto xml=getXmlFromBinary(data,size))if(xml->hasTagName(parameters.state.getType().toString())){
        ...
        { auto pl=state.getChildWithName("PLOCK"); // 1.7.1: восстановление P-LOCK
          if(pl.isValid())for(int s=0;s<pl.getNumChildren()&&s<kPlockSlots;++s){auto n=pl.getChild(s);
              const int slot=juce::jlimit(0,kPlockSlots-1,static_cast<int>(n.getProperty("slot",s)));
              plockTargetI[static_cast<size_t>(slot)].store(static_cast<int>(n.getProperty("target",-1)));
              plockPolI[static_cast<size_t>(slot)].store(static_cast<int>(n.getProperty("pol",0)));
              auto fill=[this,slot](const juce::String& csv,bool slide){int i=0;
                  for(const auto& tok:juce::StringArray::fromTokens(csv,",","")){if(i>=128)break;
                      (slide?plockSlideV:plockVal)[static_cast<size_t>(slot*128+i)].store(juce::jlimit(0.0f,127.0f,tok.getFloatValue()));++i;}};
...
int MonomachineNovaAudioProcessor::addMsegRoute(int msegIndex,uint8_t target){if(msegIndex>=msegPageCount())setMsegPageCount(msegIndex+1); // 1.6.30: бинд активирует страницу; 1.6.34: возвращает слот
    const int destination=juce::jlimit(0,63,static_cast<int>(target)); // 1.7.1 FIX: было 55 -- прицел не доставал MSEG OUT (56..63)
    int slot=-1;
...
// ---- 1.7.1: P-LOCK ---------------------------------------------------------
void MonomachineNovaAudioProcessor::plockSetTarget(int slot,int target){if(slot<0||slot>=kPlockSlots)return;plockTargetI[static_cast<size_t>(slot)].store(juce::jlimit(-1,65,target));}
...
void MonomachineNovaAudioProcessor::plockStep(int page,int step){
    ...
    for(int s=0;s<kPlockSlots;++s){
        const int t=plockTargetI[static_cast<size_t>(s)].load();if(t<0||t>65)continue;
