...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        for(const auto& m:machines()) {if(names.isNotEmpty()) names+="|";names+=juce::String(m.name);if(m.id==10) defaultMachine=index;++index;} // 1.6.6: FM DYN default on open
        ...
        s.push_back({"amp_mode","AMP DSP",0,1,1,1,"old|mnm"});
        ...
        for(int row=0;row<16;++row)s.push_back({"r"+juce::String(row)+"_aux","Route "+juce::String(row+1)+" aux source",0,23,0,1,
            "OFF|KEY|VEL|MACRO X|MACRO Y|LFO1|LFO2|LFO3|PITCH WHL|MOD WHL|AFTERTOUCH|MSEG1|MSEG2|MSEG3|STEP VEL|STEP TRANS|ARP GATE|ARP RATE|RANDOM|MSEG4|MSEG5|MSEG6|MSEG7|MSEG8"}); // 1.6.24/25: AUX SOURCE = OFF + порядок ModSource; 1.6.29: MSEG4..8 = 19..23
        for(int row=0;row<16;++row)s.push_back({"r"+juce::String(row)+"_aux_depth","Route "+juce::String(row+1)+" aux depth",-64,63,0,1,{}}); // 1.6.32: вклад AUX, 0 = не влияет
        ...
        s.push_back({"mseg2_rate","MSEG2 rate",0.05f,20.0f,1.0f,0.01f,{}}); // 1.6.21: три кривых MSEG
        s.push_back({"mseg2_sync","MSEG2 host sync",0,1,1,1,"OFF|ON"});
        s.push_back({"mseg2_loop","MSEG2 loop",0,1,1,1,"OFF|ON"});
        s.push_back({"mseg3_rate","MSEG3 rate",0.05f,20.0f,1.0f,0.01f,{}}); // 1.6.21: три кривых MSEG
        ...
        for(int mi=4;mi<=8;++mi){const juce::String n="mseg"+juce::String(mi); // 1.6.29: страницы 4..8
            s.push_back({n+"_rate","MSEG"+juce::String(mi)+" rate",0.05f,20.0f,1.0f,0.01f,{}});
        ...
        s.push_back({"mseg_retrig","MSEG retrig",0,1,1,1,"FREE|RETRIG"}); // 1.7.1: перезапуск фаз MSEG от ноты MIDI/арпа (дефолт ON = как было)
        ...
        for(int i=0;i<64;++i) {targets+="|";targets+=i<8?"SYN "+juce::String(i+1):i<56?juce::String(sections[(i-8)/8])+" "+pageLabel((i-8)/8,(i-8)%8):"MSEG"+juce::String(i-55)+" OUT";}targets+="|ARP RATE|ARP GATE"; // 1.6.29: +MSEG OUT; 1.7.1: +папка ARP (64, 65)G1..8 OUT (56..63)
        for(int row=0;row<16;++row) {
            ...
            s.push_back({prefix+"src","Route source",0,22,static_cast<float>(row%16),1,"KEY|VEL|MACRO X|MACRO Y|LFO1|LFO2|LFO3|PITCH WHL|MOD WHL|AFTERTOUCH|MSEG1|MSEG2|MSEG3|STEP VEL|STEP TRANS|ARP GATE|ARP RATE|RANDOM|MSEG4|MSEG5|MSEG6|MSEG7|MSEG8"}); // 1.6.25: порядок = ModSource; 1.6.29: 23 источника, MSEG4..8 = 18..22
            s.push_back({prefix+"dest","Route target",0,66,0,1,targets}); // 1.6.22: дефолт цели = 0; 1.6.29: 64 цели
