...
private:
    void applyZoom(float z,float fx,float fy){ zoom=z; root.setTransform(baseT.followedBy(juce::AffineTransform().scaled(z,z,fx,fy))); } // 1.6.43: зум вокруг курсора
    ...
    float zoom=1.0f; juce::AffineTransform baseT; int lastFid=-1; float lastFs=1.0f; juce::Colour pickColour{juce::Colours::white}; std::unique_ptr<ColorWin> colorWin; // 1.6.43
...
public:
    ...
    void mouseUp(const juce::MouseEvent&)override{
        ...
        for(int i=0;i<n;++i)menu.addItem(i+1,choiceName(i),lfoLockAxis()<0||!lfoTargetBlocked(i),i==juce::roundToInt(parameter->convertFrom0to1(parameter->getValue()))); // 1.6.30: чужие замки -- пункт недоступен
        ...
        const int panelRight=72+((pageIndex<0?0:pageIndex%3))*399+387; const int lx=std::max(0,std::min(getScreenX(),panelRight-estW)); // 1.6.44: не выпирает за правую стенку категории
    ...
    int lfoLockAxis() const { return (pageIndex>=3&&pageIndex<=5&&(knob==0||knob==1))?(knob==0?0:1):-1; } // 1.6.30: PAGE(0)/DEST(1) LFO-страниц
    bool lfoTargetBlocked(int v) const {
        ...
        for(int y=0;y<3;++y){ if(y==me)continue;
            if(auto* m=processor.parameters.getParameter("lfo"+juce::String(y+1)+"_"+mk)) if((((juce::roundToInt(m->convertFrom0to1(m->getValue())))>>v)&1)!=0) return true;
            if(auto* s=processor.parameters.getParameter("lfo"+juce::String(y+1)+"_"+sk)) if(juce::roundToInt(s->convertFrom0to1(s->getValue()))==v+1) return true; }
        return false;
...
private:
    ...
    MonomachineNovaAudioProcessor& processor;juce::String id,label;int pageIndex=-1,knob=-1,machine=-1;bool isChoice=false,modTarget=true;
...
public:
    ...
    void mouseDown(const juce::MouseEvent& e) override { dragOk=false;
        if(pinRect().contains(e.position.toInt())){ pinned=!pinned; if(pinned)toFront(true); repaint(); if(auto* pr=getParentComponent())pr->repaint(); return; } // 1.7.0 FIX: пин = toFront панели (setAlwaysOnTop на child в JUCE поднимал РОДИТЕЛЯ, а не панель)
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        pixel::text(g,repitchText(rep),{320-160,3,140,18},13,true); // 1.6.44: правее -- булавка
...
public:
    LfoLockPanel(MonomachineNovaAudioProcessor& p,int lfoIndex,bool):processor(p),lfo(lfoIndex){ // 1.6.30: ЕДИНОЕ окно -- секции PAGE и DEST вместе
        refreshNames();
        setSize(470,44+8*30+46); startTimerHz(10); // 1.6.33: ДВЕ КОЛОНКИ (PAGE|DEST), строки 30px, шрифты крупнее
    }
    ...
    void refreshNames(){
        ...
        for(int k=0;k<8;++k)pageNames[static_cast<size_t>(k)]=pageChoices[static_cast<int>(juce::jlimit(0,pageChoices.size()-1,k))];
        auto* pg=processor.parameters.getParameter(nova::pageParam(page,0));const int cur=juce::jlimit(0,7,juce::roundToInt(pg->convertFrom0to1(pg->getValue())));
        const int mi=processor.machineIndex();
        juce::StringArray choices;
        for(int k=0;k<8;++k){
            if(cur==0)choices.add(mi>=0?juce::String(nova::machines()[static_cast<size_t>(mi)].synthParams[static_cast<size_t>(k)].name):juce::String(k)); // PTCH -- ручки машины
    ...
    void timerCallback() override { repaint(); }
    static int rowsTop(){return 44;} static int rowH(){return 30;} static int colSplit(){return 235;} // 1.6.33: сетка двух колонок
    void paint(juce::Graphics& g)override{
        refreshNames(); // живые имена каждый кадр
        g.fillAll(juce::Colours::black);g.setColour(ink);
        pixel::text(g,juce::String("LFO")+juce::String(lfo+1)+" LOCKS",{6,4,208,18},13,false);
        pixel::text(g,"PAGE",{46,rowsTop()-16,120,14},11,false);pixel::text(g,"DEST",{colSplit()+16,rowsTop()-16,120,14},11,false);
        for(int s=0;s<2;++s){const bool pg=(s==0);const int mask=maskValue(pg),solo=soloValue(pg);const int knob=knobValue(pg);const int bx=pg?6:colSplit();
            for(int k=0;k<8;++k){const int y=rowsTop()+k*rowH();
                const bool locked=((mask>>k)&1)!=0;const bool soloed=(solo-1)==k;
                ...
                g.setColour(ink);
                pixel::text(g,pg?pageNames[static_cast<size_t>(k)]:rowNames[static_cast<size_t>(k)],{bx+42,y+3,176,22},14,false);
                if(knob==k)g.drawRect(bx+2,y,222,rowH(),1);}}
        g.setColour(ink);pixel::text(g,"CLEAR LOCKS",{6,getHeight()-30,120,20},12,false);
        pixel::text(g,"LMB LOCK (drag to paint) / RMB SOLO / click value = set",{6,getHeight()-16,452,14},9,false);
    }
    void mouseDown(const juce::MouseEvent& e)override{
        ...
        const int hit=rowAt(e.x,e.y);if(hit<0)return;const bool pg=(e.x<colSplit());const int row=hit; // 1.6.33: колонка = PAGE слева / DEST справа
        if(e.x<(pg?40:colSplit()+34)){const bool soloHere=(soloValue(pg)-1)==row;
            ...
            paintValue=was?0:1;lockPainting=true;paintSection=pg;
    ...
    void mouseDrag(const juce::MouseEvent& e)override{
        ...
        const int hit=rowAt(e.x,e.y);if(hit<0)return;const int row=hit;const bool pg=(e.x<colSplit()); // 1.6.33: красим ТА СЕКЦИЯ, где курсор (не где начали)
...
private:
    int rowAt(int x,int y)const{if(y<rowsTop()||y>=rowsTop()+8*rowH())return -1;if(x<6||x>=colSplit()+224)return -1;return juce::jlimit(0,7,(y-rowsTop())/rowH());} // 1.6.33: строка 0..7 в колонке по X
    ...
    int knobValue(bool pg)const{auto* p=processor.parameters.getParameter(nova::pageParam(3+lfo,pg?0:1));return p?juce::jlimit(0,7,juce::roundToInt(p->convertFrom0to1(p->getValue()))):0;}
    bool otherLocksBlock(bool pg,int row)const{const juce::String mk=pg?"page_locks":"dest_locks";const juce::String sk=pg?"page_solo":"dest_solo";for(int y=0;y<3;++y){if(y==lfo)continue;if(auto* m=processor.parameters.getParameter("lfo"+juce::String(y+1)+"_"+mk))if((((juce::roundToInt(m->convertFrom0to1(m->getValue())))>>row)&1)!=0)return true;if(auto* s=processor.parameters.getParameter("lfo"+juce::String(y+1)+"_"+sk))if(juce::roundToInt(s->convertFrom0to1(s->getValue()))==row+1)return true;}return false;} // 1.6.30: enforcement чужих замков
    void setMask(bool pg,int m){if(auto* p=processor.parameters.getParameter(id(pg?"page_locks":"dest_locks"))){p->beginChangeGesture();p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(juce::jlimit(0,255,m))));p->endChangeGesture();}repaint();}
    void setSolo(bool pg,int s){if(auto* p=processor.parameters.getParameter(id(pg?"page_solo":"dest_solo"))){p->beginChangeGesture();p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(juce::jlimit(0,8,s))));p->endChangeGesture();}repaint();}
    ...
    std::array<juce::String,8> rowNames{},pageNames{};bool lockPainting=false;int paintValue=0;bool paintSection=true;
};
...
public:
    ...
    struct Geo { int pad = 0, gap = 0, x0 = 0, yVal = 0, ySlide = 0, slideH = 14; };
    Geo geo() const {
        const int gap = 8, slideH = 14, margin = 6, left = 36;
        int pad = (getWidth() - left - margin - 7 * gap) / 8;
    ...
    void paint(juce::Graphics& g) override {
        ...
        const Geo geo = this->geo();
        g.setColour(ink.withAlpha(0.45f));
        pixel::text(g, "P" + juce::String(sl + 1), {2, geo.yVal + geo.pad / 2 - 7, 30, 14}, 12, true);
        for (int i = 0; i < 8; ++i) {
...
private:
    void paintAt(const juce::MouseEvent& e, const Geo& geo) {
        ...
        if (e.y >= geo.ySlide && e.y < geo.ySlide + geo.slideH) processor.plockSetSlide(sl, pg, st, processor.plockSlideValue(sl, pg, st) < 0.5f);
        else if (e.y >= geo.yVal && e.y <= geo.yVal + geo.pad) {
            const float rel = juce::jlimit(0.0f, 1.0f, static_cast<float>(geo.yVal + geo.pad - e.y) / static_cast<float>(geo.pad));
            processor.plockSetValue(sl, pg, st, std::round(rel * 127.0f)); }
        repaint(); }
    void applyErase(const juce::MouseEvent& e, const Geo& geo) {
        const int pg = juce::jlimit(0, 15, pageOf()), sl = juce::jlimit(0, processor.kPlockSlots - 1, slotOf());
        if (e.x < geo.x0) return; const int st = juce::jlimit(0, 7, static_cast<int>(e.x - geo.x0) / (geo.pad + geo.gap));
        processor.plockSetValue(sl, pg, st, 0.0f); processor.plockSetSlide(sl, pg, st, false); repaint(); }
...
public:
    ...
    void mouseDown(const juce::MouseEvent& e) override { dragY = e.getScreenPosition().y; lastApplied = 0; moved = false;
    ...
    void mouseDrag(const juce::MouseEvent& e) override {
        ...
        while (std::abs(dy - lastApplied) >= 8) { const int dir = dy > lastApplied ? 1 : -1; lastApplied += dir * 8;
            int t = processor.plockTarget(curSlot()); t = t < 0 ? 0 : juce::jlimit(0, 65, t + dir);
...
public:
    explicit ArpPage(MonomachineNovaAudioProcessor& p) : processor(p), lane(p, [this] { return pageDisplay(); }, [this](int pg, int st, const char* f, float v) { setStepParameter(pg, st, f, v); }), // 1.6.42: FOLLOW PLAY
        ...
        armBtn.setButtonText("P-LOCK"); armBtn.textHeight = 12; addAndMakeVisible(armBtn);
        armBtn.setTooltip("P-LOCK: LMB = crosshair, then click a knob on the face panel -- the steps of the current page get that parameter ( Elektron-style parameter lock). RMB = parameter list (bound slots on top).");
        armBtn.onClick = [this] { if (armRequest) armRequest(curSlot()); };
        armBtn.rightClick = [this] { showDestList(); };
        for (int s = 0; s < 4; ++s) { auto* b = slotTabs.add(new PixelButton{"P" + juce::String(s + 1)}); b->textHeight = 12;
            b->setClickingTogglesState(true); b->setTriggeredOnMouseDown(true); addAndMakeVisible(b);
            b->setTooltip("P-LOCK slot " + juce::String(s + 1) + ". A bound slot shows its lane below; the list marks bound parameters.");
            b->onClick = [this, s] { setCurSlot(s); }; }
        ...
        const std::pair<juce::Component*,const char*> layIds[]{{&arpEnable,"enable"},{&arpHold,"hold"},{&mode,"mode"},{&rateButton,"rate"},{&octavesBox,"oct"},{&gateBox,"gate"},{&hostTempo,"hosttempo"},{&wrapBox,"wrap"},{&velocityMode,"velmode"},{&pageRndButton,"rndpage"},{&pitchRndButton,"rndpitch"},{&velRndButton,"rndvel"},{&holdRndButton,"rndhold"},{&autoSwapToggle,"autoswap"},{&followPlayToggle,"follow"},{&pageBox,"page"},{&pageLimit,"pagelimit"},{&wrapBox,"wrap"},{&lane,"lane"},{&stepRndToggle,"steprnd"},{&plck,"plocklane"},{&plkRndButton,"rndplk"},{&slideRndButton,"rndslide"},{&armBtn,"plock"},{&polBtn,"plockpol"}}; // 1.7.1: +P-LOCK/STEP RND в LAYOUT EDIT
        ...
        plkRndButton.onClick=[this]{randomizePlock(curPage(),curPage(),true,false);};
        plkRndButton.rightClick=[this]{randomizePlock(0,15,true,false);};
        slideRndButton.onClick=[this]{randomizePlock(curPage(),curPage(),false,true);};
    ...
    void plockAfterBind(int slot) { setCurSlot(juce::jlimit(0, processor.kPlockSlots - 1, slot)); processor.plockFillPage(slot, curPage()); plck.repaint(); repaint(); } // 1.7.1: возврат из прицела -- слот назначен, степы страницы заполнены
    ...
    void resized() override {
        ...
        for (int s = 0; s < slotTabs.size(); ++s) slotTabs.getUnchecked(s)->setBounds(670 + s * 34, 312, 30, 26);
        destBox->setBounds(670, 342, 160, 26); polBtn.setBounds(834, 342, 54, 26); armBtn.setBounds(892, 342, 66, 26);
...
private:
    ...
    // ---- 1.7.1: P-LOCK -------------------------------------------------------
    void randomizePlock(int pg0, int pg1, bool doVals, bool doSlide) { // рандом P-LOCK/SLIDE текущего слота
        const int sl = juce::jlimit(0, processor.kPlockSlots - 1, curSlot());
        if (processor.plockTarget(sl) < 0) return;
    ...
    void showDestList() { // список параметров: автоматизированные (слоты) сверху с индикацией, остальные по папкам матрицы
        ...
        for (int s = 0; s < processor.kPlockSlots; ++s) { const int tg = processor.plockTarget(s); if (tg < 0) continue;
            m.addItem(s + 1, "P" + juce::String(s + 1) + " -- " + targetName(processor, tg), true, curSlot() == s); ++used; }
        if (used > 0) m.addSeparator();
        m.addItem(90, "-- clear slot --", true, processor.plockTarget(curSlot()) < 0);
        m.addSeparator();
        static const char* groups[]{"MACHINE", "AMP", "FILTER", "EFFX", "LFO1", "LFO2", "LFO3"};
        for (int g = 0; g < 7; ++g) { juce::PopupMenu sub;
            for (int k = 0; k < 8; ++k) { const int tg = g * 8 + k; sub.addItem(10 + tg, targetName(processor, tg), true, processor.plockTarget(curSlot()) == tg); }
        ...
        { juce::PopupMenu sub; sub.addItem(10 + 64, "ARP RATE", true, processor.plockTarget(curSlot()) == 64);
          sub.addItem(10 + 65, "ARP GATE", true, processor.plockTarget(curSlot()) == 65); m.addSubMenu("ARP", sub); }
        const juce::Component::SafePointer<ArpPage> safe(this);
        m.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&armBtn), [safe](int res) { if (!safe || res <= 0) return;
            if (res <= 4) { safe->setCurSlot(res - 1); return; } // повторный бинд = снова показать строку слота
            if (res == 90) { safe->processor.plockSetTarget(safe->curSlot(), -1); safe->repaint(); return; }
            const int tg = res - 10; if (tg >= 0 && tg <= 65) safe->bindPlockTarget(safe->curSlot(), tg); }); }
    void bindPlockTarget(int slot, int tg) { // бинд цели: степы страницы получают текущее значение цели
        processor.plockSetTarget(slot, tg); processor.plockFillPage(slot, curPage());
        polBtn.setButtonText(processor.plockPolarity(slot) == 0 ? "UNI" : "BIP"); plck.repaint(); repaint(); }
    int curSlot() const { return plockSlot; } // 1.7.1
    void setCurSlot(int s) { plockSlot = juce::jlimit(0, processor.kPlockSlots - 1, s);
        for (int i = 0; i < slotTabs.size(); ++i) slotTabs.getUnchecked(i)->setToggleState(i == curSlot(), juce::dontSendNotification);
    ...
    MonomachineNovaAudioProcessor& processor; juce::ToggleButton arpEnable, arpHold, hostTempo, autoSwapToggle, followPlayToggle; PixelButton rateButton{""}, pageRndButton{"PAGE"}, pitchRndButton{"PITCH"}, velRndButton{"VEL"}, holdRndButton{"HOLD"}, plkRndButton{"PLK"}, slideRndButton{"SLIDE"}, polBtn{"UNI"}, armBtn{"P-LOCK"}; StepLane lane; PlockLane plck; // 1.7.1: +P-LOCK/SLIDE рандом, полярность, прицел
    std::unique_ptr<PlockDestBox> destBox; juce::OwnedArray<PixelButton> slotTabs; int plockSlot = 0; // 1.7.1: P-LOCK
    ...
    float rateAccum = 0; int seenPage = -1;
...
public:
    explicit MatrixPage(MonomachineNovaAudioProcessor& p) : processor(p) { clearButton.setComponentID("clear"); // 1.6.38: LAYOUT EDIT
        ...
        clearButton.rightClick=[this,wipeField]{juce::PopupMenu m;m.setLookAndFeel(&smallMenuLaf()); // 1.6.33: компактный список
            m.addItem(1,"ERASE EVERYTHING (defaults)");m.addItem(2,"ONLY ROUTE ON/OFF");m.addItem(3,"ONLY DEPTH VALUES");m.addItem(4,"ONLY LOCKS");m.addItem(5,"ONLY POLARITY MODE");m.addItem(6,"ONLY AUX SOURCE");m.addItem(7,"ONLY AUX DEPTH"); // 1.6.41
        ...
        if(uiLayout().has("matrix.colw")){ const auto cw=uiLayout().getArr("matrix.colw"); if((int)cw.size()==8){ for(int i=0;i<8;++i)colw[(size_t)i]=juce::jlimit(18,400,cw[(size_t)i]); normalizeCols(); } } // 1.6.41: ширины колонок из layout.json
    ...
    void paint(juce::Graphics& g) override {
        ...
        for (int r = 0; r < 16; ++r) {
            const int y = rowY(r); g.setColour(ink.withAlpha(0.5f)); g.drawHorizontalLine(y + 31, 8.0f, static_cast<float>(colX(7) + colWidth(7) + 2)); // 1.6.46: строка кончается у правой границы AUX DEPTH (1.6.44: 987)
            for (int c = 1; c <= 5; ++c) {
                ...
                juce::String text = c == 2 ? sourceName(value) : c == 3 ? (value == 0 ? juce::String("---") : targetName(processor, value - 1)) : c == 4 ? signedPercent(value) : polarityName(value); // 1.6.32: 0 = OFF
            ...
            { // 1.6.24: AUX SOURCE -- вторичный источник масштабирует глубину (Sylenth-style)
                static const char* auxNames[]{"OFF", "KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "MSEG2", "MSEG3", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM"}; // 1.6.25: порядок = ModSource
                auto* ap = processor.parameters.getRawParameterValue("r" + juce::String(r) + "_aux");
                const int aux = ap ? juce::jlimit(0, 18, juce::roundToInt(ap->load())) : 0;
        ...
        g.setColour(juce::Colours::white); g.drawRect(8, 26, 1120, 16 * 34 + 36, 1); // 1.6.25: рамка матрицы -- белая
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        if (e.y < 56) { const int c = colAt(e.x);
            if (c >= 1 && std::abs(e.x - colX(c)) <= 3) { colDrag = c; colDragX = e.x; return; } // 1.6.46: c>=1 -- и ПЕРЕДНЯЯ граница ON (тянет полосу номеров строк = сдвиг старта колонок от начала матрицы) (1.6.41: ресайз колонки -- тянуть границу в шапке)
    ...
    void mouseDrag(const juce::MouseEvent& e) override {
        if (colDrag > 0) { const int nx = static_cast<int>(e.getScreenPosition().x); colw[(size_t)colDrag - 1] = juce::jlimit(18, 400, colw[(size_t)colDrag - 1] + nx - colDragX); colDragX = nx; normalizeCols(); repaint(); return; } // 1.6.41: ресайз колонки, правый край неподвижен
        ...
        else { const int hi = hitCol == 3 ? 64 : hitCol == 5 ? 1 : hitCol == 6 ? 18 : 17; p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(juce::jlimit(0, hi, hitBase + dy / 8)))); } // 1.6.32: DEST -- 0..64 (0 = OFF)
        repaint();
    }
    void mouseUp(const juce::MouseEvent& e) override {
    ...
    void openValueMenu(int r, int c, juce::Point<int> screen) { // 1.6.24: единый список значений ячейки
        ...
        if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; for (int i = 0; i < 9; ++i) menu.addItem(i + 1, signedPercent(values[i])); } // -100% ... 0% ... +100%
            ...
            for (int g = 0; g < 7; ++g) {
                juce::PopupMenu sub;
                for (int k = 0; k < 8; ++k) { const int target = g * 8 + k; sub.addItem(target + 2, targetName(processor, target), true, selected == target + 1); }
            ...
            static const int vals[]{17, 0, 1, 7, 8, 9, 4, 5, 6, 13, 14, 15, 16, 10, 11, 12, 18, 19, 20, 21, 22, 2, 3}; // 1.6.34: порядок = RANDOM, MIDI, LFO, ARP, MSEG, MACRO (позиции = id)
            const int selected = juce::roundToInt(par->convertFrom0to1(par->getValue()));
            ...
            juce::PopupMenu lfom; for (int i = 0; i < 3; ++i) lfom.addItem(7 + i, sourceName(vals[6 + i]), true, selected == vals[6 + i]);
            juce::PopupMenu arp; for (int i = 0; i < 4; ++i) arp.addItem(10 + i, sourceName(vals[9 + i]), true, selected == vals[9 + i]);
            juce::PopupMenu msegm; for (int i = 0; i < 8; ++i) msegm.addItem(14 + i, sourceName(vals[13 + i]), true, selected == vals[13 + i]);
            juce::PopupMenu mac; for (int i = 0; i < 2; ++i) mac.addItem(22 + i, sourceName(vals[21 + i]), true, selected == vals[21 + i]);
            menu.addSubMenu("MIDI", midi); menu.addSubMenu("LFO", lfom); menu.addSubMenu("ARP", arp); menu.addSubMenu("MSEG", msegm); menu.addSubMenu("MACRO", mac);
        }
        const juce::Component::SafePointer<MatrixPage> safe(this);
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this).withTargetScreenArea(localAreaToGlobal(juce::Rectangle<int>(120, 227, 1, 1))), [safe, r, c](int result) { if (!safe || result <= 0) return; auto* p = safe->cell(r, c); if (!p) return; if (safe->pushEdit) safe->pushEdit(); if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(values[result - 1]))); } else if (c == 2) { static const int vals[]{17, 0, 1, 7, 8, 9, 4, 5, 6, 13, 14, 15, 16, 10, 11, 12, 18, 19, 20, 21, 22, 2, 3}; p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(vals[result - 1]))); } else p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(result - 1))); }); // 1.6.43: список всегда у 120,227 (просьба), ширины колонок не тронуты // 1.6.34: порядок RANDOM/MIDI/LFO/ARP/MSEG/MACRO; 1.6.28/31: порядок списка SOURCE != порядка enum; страницы фиксированы
    }
    void openAuxMenu(int r, juce::Point<int> screen) { // 1.6.31: AUX = копия меню SOURCE + OFF первым (папки, тот же порядок)
        juce::PopupMenu m; m.setLookAndFeel(&getLookAndFeel());
        static const char* auxNames[]{"OFF", "KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "MSEG2", "MSEG3", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM", "MSEG4", "MSEG5", "MSEG6", "MSEG7", "MSEG8"};
        auto* par = processor.parameters.getParameter("r" + juce::String(r) + "_aux"); const int cur = par ? juce::jlimit(0, 23, juce::roundToInt(par->convertFrom0to1(par->getValue()))) : 0;
        m.addItem(1, "OFF", true, cur == 0); m.addItem(2, "RANDOM", true, cur == 18); // 1.6.34: сепаратор-«пустая ячейка» после RANDOM убран
        static const int vals[]{17, 0, 1, 7, 8, 9, 4, 5, 6, 13, 14, 15, 16, 10, 11, 12, 18, 19, 20, 21, 22, 2, 3}; // 1.6.34: MIDI, LFO, ARP, MSEG, MACRO
        juce::PopupMenu midi; for (int i = 0; i < 5; ++i) midi.addItem(3 + i, auxNames[vals[1 + i] + 1], true, cur == vals[1 + i] + 1);
        juce::PopupMenu lfom; for (int i = 0; i < 3; ++i) lfom.addItem(8 + i, auxNames[vals[6 + i] + 1], true, cur == vals[6 + i] + 1);
        juce::PopupMenu arp; for (int i = 0; i < 4; ++i) arp.addItem(11 + i, auxNames[vals[9 + i] + 1], true, cur == vals[9 + i] + 1);
        juce::PopupMenu msegm; for (int i = 0; i < 8; ++i) msegm.addItem(15 + i, auxNames[vals[13 + i] + 1], true, cur == vals[13 + i] + 1);
        juce::PopupMenu mac; for (int i = 0; i < 2; ++i) mac.addItem(23 + i, auxNames[vals[21 + i] + 1], true, cur == vals[21 + i] + 1);
        m.addSubMenu("MIDI", midi); m.addSubMenu("LFO", lfom); m.addSubMenu("ARP", arp); m.addSubMenu("MSEG", msegm); m.addSubMenu("MACRO", mac);
        const juce::Component::SafePointer<MatrixPage> safe(this);
        m.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this).withTargetScreenArea(juce::Rectangle<int>(screen.x, screen.y, 1, 1)), [safe, r](int result) { if (!safe || result <= 0) return; if (safe->pushEdit) safe->pushEdit(); if (auto* p = safe->processor.parameters.getParameter("r" + juce::String(r) + "_aux")) { static const int vals2[]{17, 0, 1, 7, 8, 9, 4, 5, 6, 13, 14, 15, 16, 10, 11, 12, 18, 19, 20, 21, 22, 2, 3}; const int v = result == 1 ? 0 : result == 2 ? 18 : vals2[result - 2] + 1; p->beginChangeGesture(); p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(v))); p->endChangeGesture(); } }); // 1.6.43 FIX: был result-3 -- всё смещалось на один (KEY -> RANDOM, VEL -> KEY)
    }
...
private:
    ...
    static juce::String sourceName(int v) { const char* n[]{"KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "MSEG2", "MSEG3", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM", "MSEG4", "MSEG5", "MSEG6", "MSEG7", "MSEG8"}; return n[juce::jlimit(0, 22, v)]; } // 1.6.25/29: 23 источника
    ...
    void normalizeCols(){ int w7=939; for(int i=0;i<7;++i)w7-=colw[(size_t)i]; int over=w7<30?30-w7:0; colw[7]=juce::jlimit(30,400,w7); // 1.6.41/43: последняя >= 30, лишнее поджимает предыдущие (не выталкиваются за край)
    ...
    void timerCallback() override { repaint(); }
    MonomachineNovaAudioProcessor& processor; std::array<juce::RangedAudioParameter*, 80> cells{}; int hitRow = -1, hitCol = -1, hitY = 0, hitX = 0, hitBase = 0, dragRow = -1, dragTarget = -1, sortColumn = -1; bool gesture = false, dragged = false, sortDescending = false;
};
...
public:
    MsegPage(MonomachineNovaAudioProcessor& p,int startPage) : processor(p), page(juce::jlimit(0,7,startPage)) {
        ...
        retrig.setButtonText("RETRIG"); retrig.setClickingTogglesState(true); addAndMakeVisible(retrig); // 1.7.1: перезапуск MSEG от ноты MIDI/арпа
        retrig.setTooltip("RETRIG: ON = every new note (incoming MIDI or the arp) restarts the MSEG phases. OFF = the curves free-run. Default ON.");
        ...
        for (const char* g : {"GRID OFF","GRID 1/1","GRID 1/2","GRID 1/4","GRID 1/8","GRID 1/16","GRID 1/32","GRID 1/64","GRID 1/4T","GRID 1/8T","GRID 1/16T","GRID 1/32T"}) grid.addItem(g, grid.getNumItems()+1);
        grid.setSelectedItemIndex(5, juce::dontSendNotification); // 1/16
        ...
        for (const char* g : {"VIEW OFF","VIEW 1/1","VIEW 1/2","VIEW 1/4","VIEW 1/8","VIEW 1/16","VIEW 1/32","VIEW 1/64","VIEW 1/4T","VIEW 1/8T","VIEW 1/16T","VIEW 1/32T"}) viewGrid.addItem(g, viewGrid.getNumItems()+1);
        viewGrid.setSelectedItemIndex(5, juce::dontSendNotification); // 1.6.33: старт = 1/16 как GRID -- LINK включён, видовая сетка не расходится с рисованием до первой смены
        ...
        const char* forms[]{"DRAW","DRAW STEP","RAND DRAW 1","RAND DRAW 2"}; // 1.6.30: плоские формы убраны (они в списке SHAPE)
        for (const char* f : forms) { auto* b = formButtons.add(new PixelButton{f}); b->textHeight = 14; b->setBounds(0,0,10,10); addAndMakeVisible(b); b->setComponentID(juce::String("form")+juce::String(formButtons.size()-1)); // 1.6.32/41: доступны в LAYOUT EDIT
        ...
        clearButton.onClick = [this] { pushUndo(); processor.setMsegPointCount(page, 2); processor.setMsegPoint(page, 0, 0.0f, 0.0f); processor.setMsegPoint(page, 1, 0.0f, 0.0f); markCustom(); repaint(); }; // 1.6.30: только одна точка слева внизу
        ...
        retrigLink = std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment>(processor.parameters, "mseg_retrig", retrig); // 1.7.1: глобальная галка
        ...
        tagRows=std::make_unique<TextTag>(*this,"rownames",juce::Rectangle<int>(54,routeY(),330,22),true); // 1.6.42: имена строк двигаются в LAYOUT EDIT (вертикаль привязана к строкам)
    }
    ~MsegPage() override { stopTimer(); }

    int curPage() const { return page; } // 1.6.31: для прицела из редактора
    std::function<void(int,int)> pickStart; // 1.6.34: прицел из строки маршрутов -- клик по гнезду, клик по ручке (как DEST в матрице); pg, displayRow
    ...
    void setPage(int m) {
        ...
        undo.clear(); redo.clear(); selected = -1; hoverSeg = -1; bendSeg = -1; drawing = false; stepsBaked = false; stepsPrimed = false; bakedBackup.clear(); repaint();
    }
    void paint(juce::Graphics& g) override {
        g.fillAll(juce::Colours::black); g.setColour(ink);
        pixel::text(g, "LFO CURVE EDITOR", {12, 4, 420, 26}, 22);
        pixel::text(g, juce::String(processor.msegPointCount(page)) + " / 32 POINTS", {450, 6, 260, 22}, 16, false); // 1.7.0: крупнее (было 13)
        ...
        { static const int steps[]{0,1,2,4,8,16,32,64,3,6,12,24}; const int den = steps[juce::jlimit(0, 11, viewGrid.getSelectedItemIndex())]; // 1.6.29: GRID VIEW
        ...
        if (drawing && (drawMode == 1 || drawMode == 3 || drawMode == 4) && stroke.size() >= 2) { // 1.6.32: живое превью штриха -- сама кривая не трогается
        ...
        const int rowX = tagRows->getX() - 38, barX = tagRows->getRight() + 11, eraseX = barX + 261;
        int row = 0;
        for (int r = 0; r < 16 && row < 8; ++r) { // 1.6.29: тот же порядок и размер строк, что в матрице
            ...
            const int target = juce::jlimit(0, 65, dRaw - 1); // 1.7.1: +ARP
    ...
    void resized() override {
        graphW = juce::jlimit(600, 2400, 984 + (getWidth() - 1170)); graphH = juce::jlimit(150, 1200, 292 + (getHeight() - 800)); // 1.7.1: граф тянется за окно от заводского 1170x800
        retrig.setBounds(150, 70, 72, 26); // 1.7.1
        shape.setBounds(146, 70, 182, 26); // 1.7.0: из JSON пользователя
        if (pageButtons.size() > 0) pageButtons.getUnchecked(0)->setBounds(328, 70, 64, 26); // 1.7.0: из JSON пользователя
        stepsButton.setBounds(452, 70, 90, 26); grid.setBounds(549, 70, 94, 26); linkBtn.setBounds(643, 70, 54, 26); viewGrid.setBounds(697, 70, 108, 26); // 1.7.0: из JSON пользователя: плотный ряд, зазоры 8px, без наложений
        shift.setBounds(814, 70, 104, 26); // 1.7.0: из JSON пользователя
        ...
        { static const int tx[]{336, 468, 608, 743, 892, 1024}; // 1.7.0: из JSON пользователя
          for (int i = 0; i < toolButtons.size() && i < 6; ++i) toolButtons.getUnchecked(i)->setBounds(tx[i], toolY(), 132, 26); } // 1.7.1: инструменты едут с графом
        loop.setBounds(17, 70, 62, 26); sync.setBounds(79, 70, 67, 26); rate.setBounds(225, 70, 360, 26); // 1.7.0: loop/sync из JSON (rate к ряду)
        uiLayoutApply(*this,"mseg"); // 1.6.38: сохранённая раскладка MSEG-страницы
    }
    void mouseDown(const juce::MouseEvent& e) override {
        const int rowX = tagRows->getX() - 38, barX = tagRows->getRight() + 11, eraseX = barX + 261; // 1.7.1: зоны -- от тега rownames
        ...
        for (int rest = 0; rest < 3; ++rest) { const int y2 = routeY() + rest * 34; // 1.6.34: прицел из UNASSIGNED-строки
            if (e.x >= rowX - 2 && e.x <= rowX + 36 && e.y >= y2 && e.y <= y2 + 30 && !e.mods.isPopupMenu()) { if (pickStart) pickStart(page, rest); return; } }
        ...
        if (e.getNumberOfClicks() >= 2) { if (near >= 0) { pushUndo(); if (near > 0 && near < processor.msegPointCount(page) - 1) processor.removeMsegPoint(page, near); selected = -1; }
            else { pushUndo(); float px = snapX(toX(e.x)), py = toY(e.y); processor.addMsegPoint(page, px, py); selected = nearestPoint(e.position); } markCustom(); repaint(); return; }
        selected = near;
    ...
    void mouseUp(const juce::MouseEvent& e) override {
        ...
        if (draggingPoint) pushUndo(); draggingPoint = false; erasing = false; bendSeg = -1; if (drawing && drawMode > 0 && drawMode != 2) writeStroke(); stroke.clear(); drawing = false; if (routeDrag >= 0) { if (auto* d = processor.parameters.getParameter("r" + juce::String(routeDrag) + "_depth")) d->endChangeGesture(); routeDrag = -1; } } // 1.6.32: штрих пишется ОДИН раз на отпускании; 1.6.29: фиксация
...
private:
    ...
    float snapX(float x) const { static const int steps[]{0,1,2,4,8,16,32,64,3,6,12,24}; const int den = steps[juce::jlimit(0, 11, grid.getSelectedItemIndex())]; // 1.6.29: полная сетка + тройки
        return den > 0 ? std::round(x * den) / den : x; }
    void gridStep(int delta) { const int idx = juce::jlimit(1, 11, grid.getSelectedItemIndex() + delta); grid.setSelectedItemIndex(idx, juce::sendNotificationSync); } // 1.6.29: ALT+1/2
    void gridTriplet() { if (grid.getSelectedItemIndex() < 8) { straightIdx = grid.getSelectedItemIndex(); grid.setSelectedItemIndex(10, juce::sendNotificationSync); } // 1.6.29: ALT+3
        else grid.setSelectedItemIndex(juce::jlimit(1, 7, straightIdx), juce::sendNotificationSync); }
    void gridSnapToggle() { if (grid.getSelectedItemIndex() > 0) { snapSavedIdx = grid.getSelectedItemIndex(); grid.setSelectedItemIndex(0, juce::sendNotificationSync); } // 1.6.29: ALT+4
        else grid.setSelectedItemIndex(juce::jlimit(1, 11, snapSavedIdx), juce::sendNotificationSync); }
    ...
    static const std::array<juce::String, 66>& targetNames() { static const auto names = [] { std::array<juce::String, 66> a{}; for (int i = 0; i < 66; ++i) a[static_cast<size_t>(i)] = juce::String(monomachine::ModulationMatrix::getDestinationName(static_cast<uint8_t>(i))); return a; }(); return names; } // 1.6.29: +MSEG OUT; 1.7.1: +ARP RATE/GATE
    ...
    void syncDrawButtons() { for (int i = 0; i < formButtons.size(); ++i) { auto* b = formButtons.getUnchecked(i); const juce::String s = b->getButtonText();
        if (s == "DRAW") b->setToggleState(drawMode == 1, juce::dontSendNotification); else if (s == "DRAW STEP") b->setToggleState(drawMode == 2, juce::dontSendNotification); else if (s == "RAND DRAW 1") b->setToggleState(drawMode == 3, juce::dontSendNotification); else if (s == "RAND DRAW 2") b->setToggleState(drawMode == 4, juce::dontSendNotification); } }
    ...
    void writeStroke() { auto pts = stroke; if (pts.empty()) return; // 1.6.31: штрих ДОБАВЛЯЕТСЯ к кривой -- точки вне штриха сохраняются (кривую больше не стирает)
        ...
        for (size_t i = 0; i < merged.size(); ++i) processor.setMsegPoint(page, static_cast<int>(i), merged[i].x, merged[i].y); }
    std::vector<Pt> stroke; juce::Random rng; float randPhase = 0.0f; bool snapStroke = true, stepsPrimed = false; // 1.6.30
    int gridDen() const { static const int steps[]{0, 1, 2, 4, 8, 16, 32, 64, 3, 6, 12, 24}; const int den = steps[juce::jlimit(0, 11, grid.getSelectedItemIndex())]; return den > 0 ? den : 16; } // 1.6.30: квадраты -- по сетке GRID
    void rebuildStairs() { const int n = gridDen(); int k0 = -1, k1 = -1; // 1.6.31: ступени пишутся только в тронутом диапазоне колонок, остальная кривая жива
        ...
        std::sort(pts.begin(), pts.end(), [](const Pt& a, const Pt& b) { return a.x < b.x; });
        if (static_cast<int>(pts.size()) > 32) { std::vector<Pt> kept; for (int i = 0; i < 32; ++i) kept.push_back(pts[static_cast<size_t>(i * (static_cast<int>(pts.size()) - 1) / 31)]); pts.swap(kept); }
    ...
    juce::ToggleButton loop, sync, retrig; juce::Slider rate, shift; DragChoice shape, grid, viewGrid; PixelButton linkBtn{"LINK"}; // 1.7.1: +RETRIG // 1.6.33: логика DEST-матрицы (клик = список после отпускания, ЛКМ-драг = листать); WheelCombo открывал список по НАЖАТИЮ -- драг триггерил пункт
    int straightIdx = 5, snapSavedIdx = 5; std::vector<Pt> customBackup; bool lastShapeCustom = true; // 1.6.29: память CUSTOM-волны
    ...
    std::unique_ptr<TextTag> tagRows; // 1.6.42
    std::unique_ptr<juce::AudioProcessorValueTreeState::ButtonAttachment> loopLink, syncLink, retrigLink; std::unique_ptr<juce::AudioProcessorValueTreeState::SliderAttachment> rateLink; // 1.7.1: +RETRIG
    ...
    int graphX = 16, graphY = 118, graphW = 984, graphH = 292; // 1.7.1: окно MSEG редактируемого размера (грип справа внизу, размер в layout.json)
...
public:
    explicit EnvelopePage(MonomachineNovaAudioProcessor& p,bool compact=false):processor(p),mini(compact){
        mode.setScrollWheelEnabled(true);mode.smallPopup=true;mode.addItem("old",1);mode.addItem("mnm",2);addAndMakeVisible(mode); // 1.6.35: режим огибающей -- компактный список по ширине кнопки
        ...
        if(mini){mode.setVisible(false);gx=8;gy=32;gw=371;gh=114; // 1.6.42: режим -- НАСТОЯЩИМ MODE AMP (шапка окна)
    ...
    void paint(juce::Graphics& g)override{
        ...
        if(!mini){g.setColour(ink);g.drawText(mode.getSelectedId()==1?"old = legacy 1.4 envelope. Curves apply only in mnm.":"mnm = firmware kernel ADSR. Drag graph vertically: decay before NOTE OFF, release after. Preview uses the DSP envelope; display limited to 20 s.",20,307,1130,20,juce::Justification::left);} // 1.6.30: в мини-окне подсказка не влезает
        if(selected>=4){auto r=sliders[static_cast<size_t>(selected)].getBounds().expanded(2);g.drawRect(r);}
    }
    void mouseDown(const juce::MouseEvent& e)override{if(e.y<gy||e.y>gy+gh||mode.getSelectedId()!=2)return;dragId=(e.x-gx)/static_cast<double>(gw)*duration<gate.getValue()/1000?"amp_curve_d":"amp_curve_r";dragParameter=processor.parameters.getParameter(dragId);origin=e.y;start=dragParameter->convertFrom0to1(dragParameter->getValue());dragParameter->beginChangeGesture();}
...
private:
    ...
    juce::Slider gate;juce::Label gateLabel;std::array<double,9> previous{};std::vector<float> trace;double duration=1;int selected=-1,origin=0;float start=0;juce::String dragId;juce::RangedAudioParameter* dragParameter=nullptr;bool mini=false;int gx=30,gy=60,gw=1110,gh=225;
};
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, public juce::Slider::Listener, private juce::Timer {
    ...
    void pushUndoPoint() { undoStack.push_back(processor.parameters.copyState()); if (undoStack.size() > 60) undoStack.erase(undoStack.begin()); redoStack.clear(); }
    void applyTree(juce::ValueTree vt) { processor.parameters.replaceState(vt); bind(); repaint(); }
    ...
    explicit Surface(MonomachineNovaAudioProcessor& p):processor(p),machineButton(""),menuButton("MENU"),tempoButton(""),arpButton("ARP"),gateOuter(p,"arp_length",1,127,64),matrixButton("MATRIX"),lfo1Button("1"),lfo2Button("2"),lfo3Button("3"),msego1("1"),msego2("2"),msego3("3"),closeButton("BACK"),gateButton(""){
        ...
        auto wireMseg=[this](ModSourceButton& b,int m){
            ...
            b.numberClick=[this,m]{showMseg(m);}; // 1.6.21: клик = открыть окно на этой странице
            ...
            b.endDrag=[this,m](juce::Point<int> s,bool dragged,bool){cableLayer.hide();for(auto& column:cells)for(auto& cell:column)if(cell)cell->setPickMode(false);if(modHover)modHover->setHover(false);
                if(dragged){auto* cell=cellAtScreen(s);if(cell&&cell->canBeTarget()){const int slot=processor.addMsegRoute(m,cell->targetId());setViewportMode(true);if(matrixPage)matrixPage->flashDestCell(slot);}} // 1.6.34: назначение MSEG-кордом открывает матрицу, мигает DEST-ячейка (как LFO)
                modHover=nullptr;modMsegAim=0;};
            b.setTooltip("MSEG"+juce::String(m+1)+": click the number = open the curve editor on this page; pull the CORD from the jack onto a knob and release = a matrix route driven by this curve."); // 1.6.21
        };
 ...
 closeExtraPanel(); };
        ...
        const std::pair<juce::Component*,const char*> layIds[]{{&machineButton,"machine"},{&menuButton,"menu"},{&tempoSync,"sync"},{&tempoButton,"bpm"},{&arpEnabled,"arpon"},{&arpButton,"arp"},{&gateOuter,"gate"},{&trigToggle,"trig"},{&matrixButton,"matrix"},{&dspModeButton,"dspmode"},{&undoBtn,"undo"},{&redoBtn,"redo"},{&syntCatButton,"catsynt"},{&lfoCatButton,"catlfo"},{&filtCatButton,"catfilt"},{&effxCatButton,"cateffx"},{&msegCatButton,"catmseg"},{&syntModeCombo,"modesynt"},{&filtModeCombo,"modefilt"},{&effxDistCombo,"modedist"},{&effxDlyCombo,"modedly"},{&ampModeCombo,"modeamp"},{&level,"lev"},{&closeButton,"back"},{&lfo1Button,"lfo1"},{&lfo2Button,"lfo2"},{&lfo3Button,"lfo3"},{&msego1,"msego1"},{&msego2,"msego2"},{&msego3,"msego3"},{&ampButton,"ampcat"}}; // 1.6.49: кнопка AMP -- тоже двигается в LAYOUT EDIT
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    ~Surface()override{removeMouseListener(&rmbWatcher);closeExtraPanel();stopTimer();viewport.setViewedComponent(nullptr);setLookAndFeel(nullptr);} // 1.6.41: убрать плавающее окно доп-настроек
    ...
    void bind(){
        boundMachine=processor.machineIndex();updateDspModeLabel();const auto& m=nova::machines()[static_cast<size_t>(boundMachine)];machineButton.setButtonText(m.name);machineButton.textHeight=25; // 1.6.13
    ...
    void beginModDrag(int src,bool matrix){modDragSrc=src;modDragMatrix=matrix;modHover=nullptr;lfoHoldIndex=-1;
        ...
        if(src>=4&&src<=6){auto* pp=processor.parameters.getParameter(nova::pageParam(3+src-4,0));auto* dd=processor.parameters.getParameter(nova::pageParam(3+src-4,1));
    ...
    void endModDrag(juce::Point<int> screen,bool dragged,int lfoIndex,bool matrix){
        ...
        const int src=modDragSrc;modDragSrc=-1;if(modHover)modHover->setHover(false);modHover=nullptr;modDragMatrix=false;lfoHoldIndex=-1;lfo1Button.grayBlink=lfo2Button.grayBlink=lfo3Button.grayBlink=false; // 1.6.31: мигание параметра убрано -- рамка DEST в матрице
        if(src<0)return;
        bool landed=false;
        if(dragged){
            ...
            if(cell&&cell->canBeTarget()){
                landed=true;
                if(matrix){const int slot=processor.addRouteFromSource(src,cell->targetId()); // ПКМ: маршрут в матрицу
        ...
        if(dragged&&!matrix&&src>=4&&src<=6&&!landed&&overLfoButton(screen)<0&&aimBackupValid){
            ...
            set(nova::pageParam(3+src-4,0),aimBackupPage);set(nova::pageParam(3+src-4,1),aimBackupDest);
            lfoPageSel=juce::jlimit(0,2,src-4);bind();}
        repaint();
    }
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.7.1 / Synth":"BUILD 1.7.1 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
    ...
    void showEnvelope(int knob){dismissDock();viewport.setViewedComponent(nullptr,false);settings.reset();matrixPage.reset();samplePage.reset();msegPage.reset();envelopePage=std::make_unique<EnvelopePage>(processor);viewport.setViewedComponent(envelopePage.get(),false);viewport.setVisible(true);viewport.toFront(false);closeButton.setVisible(true);closeButton.toFront(false);if(knob>=0)envelopePage->focusCurve(knob);}
    void setViewportMode(bool matrix){
        ...
        else{matrixPage.reset();settings=std::make_unique<ArpPage>(processor);settings->pushEdit=[this]{pushUndoPoint();};settings->armRequest=[this](int slot){beginPlockPick(slot);};viewport.setViewedComponent(settings.get(),false);} // 1.7.1: прицел P-LOCK
    ...
    void beginPlockPick(int slot){ // 1.7.1: прицел P-LOCK -- окно ARP прячется, клик по ручке = цель слота
        plockPickSlot=juce::jlimit(0,3,slot);pickingTarget=true;
    ...
    void completeTargetPick(uint8_t target){
        if(pickingTarget&&plockPickSlot>=0){ // 1.7.1: цель для слота P-LOCK -- назад на ARP, степы страницы назначены
            const int slot=plockPickSlot;plockPickSlot=-1;pickingTarget=false;
            processor.plockSetTarget(slot,static_cast<int>(target));
            setViewportMode(false);
            if(settings)settings->plockAfterBind(slot);
            repaint();return;}
 ...
 // 1.6.30: новый стандарт -- список НИЖЕ своего хитбокса
    ...
    void toggleEnvMini(){if(dockPanel!=nullptr){dismissDock();return;}if(envelopePage!=nullptr)return; // 1.6.44: мини-ENV НЕ поверх большого окна
        showDocked(std::make_unique<EnvelopePage>(processor,true),juce::Rectangle<int>(471,95,387,236),false);ampButton.toFront(false);ampModeCombo.toFront(false);} // 1.6.42: AMP и MODE остаются видны и кликабельны поверх окна (шапка AMP|MNM по скрину)
    ...
    void showMenu(){juce::PopupMenu menu;menu.setLookAndFeel(&smallMenuLaf()); // 1.6.14/33: чёрный компактный скин, список под кнопкой
        ...
        juce::Component::SafePointer<Surface> safe(this);menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&menuButton),[safe](int result){if(!safe)return;
            ...
            if(result==5){safe->processor.requestPanic();safe->processor.resetAllMsegs();for(auto* parameter:safe->processor.getParameters()){parameter->beginChangeGesture();parameter->setValueNotifyingHost(parameter->getDefaultValue());parameter->endChangeGesture();}safe->bind();} // 1.6.32: ресет -- и MSEG-страницы
    ...
    PixelButton machineButton,menuButton,tempoButton,arpButton,matrixButton,syntCatButton{"SYNT"},lfoCatButton{"LFO"},filtCatButton{"FILT"},effxCatButton{"EFFX"},msegCatButton{"MSEG"};DragValueBox gateOuter;ModSourceButton lfo1Button,lfo2Button,lfo3Button,msego1,msego2,msego3;CableLayer cableLayer;int modMsegAim=0;PixelButton closeButton,gateButton;juce::Slider level;PixelButton dspModeButton{"DSP MODE"};UndoRedoButton undoBtn{true},redoBtn{false}; // 1.6.32
