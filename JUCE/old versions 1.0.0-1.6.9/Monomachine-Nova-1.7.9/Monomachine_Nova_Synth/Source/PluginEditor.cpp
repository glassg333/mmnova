...
public:
    explicit ArpPage(MonomachineNovaAudioProcessor& p) : processor(p), lane(p, [this] { return pageDisplay(); }, [this](int pg, int st, const char* f, float v) { setStepParameter(pg, st, f, v); }), // 1.6.42: FOLLOW PLAY
        ...
        slotBtn.setButtonText("P-LIST"); slotBtn.textHeight = 12; slotBtn.crosshair = true; addAndMakeVisible(slotBtn); // 1.7.9: крестик как прицел в матрице // 1.7.5: P-LIST -- клик = список, драг = листать слоты
        ...
        if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; for (int i = 0; i < 9; ++i) menu.addItem(i + 1, signedPercent(values[i])); } // -100% ... 0% ... +100%
            ...
            { juce::PopupMenu mxSub; // 1.7.5/1.7.7: цели-маршруты 1..64: номер строки и то, что она модулирует (модулируют её ГЛУБИНУ)
              for (int n = 0; n < 64; ++n) { auto* dp = processor.parameters.getRawParameterValue("r" + juce::String(n) + "_dest");
                  ...
                  mxSub.addItem(69 + n, juce::String(n + 1) + " - " + (dv > 1 ? targetName(processor, juce::jlimit(0, 131, dv - 1)) : juce::String("OFF")), true, selected == 68 + n); }
...
private:
    juce::String destName(int id) const { if (id == 131) return "LFO FM"; if (id >= 67 && id < 131) { auto* dp = processor.parameters.getRawParameterValue("r" + juce::String(id - 67) + "_dest"); const int dv = dp ? juce::roundToInt(dp->load()) : 0; return juce::String(id - 66) + " - " + (dv > 1 ? targetName(processor, juce::jlimit(0, 131, dv - 1)) : juce::String("OFF")); } return targetName(processor, id); } // 1.7.7: строки-маршруты 1..64 в DEST -- номер и то, что модулирует
...
private:
    ...
    static const std::array<juce::String, 132>& targetNames() { static const auto names = [] { std::array<juce::String, 132> a{}; for (int i = 0; i < 132; ++i) a[static_cast<size_t>(i)] = juce::String(monomachine::ModulationMatrix::getDestinationName(static_cast<uint8_t>(i))); return a; }(); return names; } // 1.7.7: ROUTE 1..64; 1.6.29: +MSEG OUT; 1.7.1: +ARP RATE/GATE
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    juce::String matrixRouteName(int r) const { // 1.7.5/1.7.7: динамическое имя маршрута (0..63)
        ...
        return v > 0 ? targetName(processor, juce::jlimit(0, 131, v - 1)) : juce::String("OFF"); }
    void showMsegDestList(int pg, int row) { // 1.7.5: UNASSIGNED-строка MSEG -- список целей (папки как в P-LIST), выбор создаёт маршрут
        juce::PopupMenu m; m.setLookAndFeel(&getLookAndFeel());
        static const char* groups[]{"MACHINE","AMP","FILTER","EFFX","LFO1","LFO2","LFO3"};
        for (int g = 0; g < 7; ++g) { juce::PopupMenu sub; for (int k = 0; k < 8; ++k) { const int idx = k == 0 ? 1 : (k == 1 ? 0 : k); sub.addItem(10 + g * 8 + idx, targetName(processor, g * 8 + idx), true, false); } m.addSubMenu(groups[g], sub); } // 1.7.9: SYN TUNE первым
        ...
        m.showMenuAsync(juce::PopupMenu::Options(), [safe, pg, row](int res) { if (!safe || res <= 0) return; const int tg = juce::jlimit(0, 131, res - 10); // 1.7.7
