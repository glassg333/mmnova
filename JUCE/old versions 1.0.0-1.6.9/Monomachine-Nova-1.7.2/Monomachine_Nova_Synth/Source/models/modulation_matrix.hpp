...
public:
    ...
    static constexpr size_t kTargetCount = 66; // 8 synth + 8 AMP + 8 FILT + 8 FX + 24 LFO + 8 MSEG OUT (56..63) + ARP RATE (64) / ARP GATE (65) -- 1.7.1
    ...
    void evaluate(const std::array<T, N>& baseParams, std::array<T, N>& modulatedParams, float* msegDests = nullptr, float* arpDests = nullptr) const noexcept { // 1.7.1: +arpDests (0 = RATE, 1 = GATE)
        std::array<float, N> sums{};
        for (size_t i = 0; i < N; ++i) sums[i] = static_cast<float>(baseParams[i]);
        for (const auto& route : m_routings) {
            if (!route.enabled || route.depth == 0 || route.targetParamId >= kTargetCount) continue;
            ...
            if (route.targetParamId >= 64) { if (arpDests != nullptr) arpDests[route.targetParamId - 64] += source * static_cast<float>(route.depth); continue; } // 1.7.1: папка ARP
            if (route.targetParamId >= 56) { if (msegDests != nullptr) msegDests[route.targetParamId - 56] += source * static_cast<float>(route.depth); continue; } // 1.6.29: MSEG OUT
