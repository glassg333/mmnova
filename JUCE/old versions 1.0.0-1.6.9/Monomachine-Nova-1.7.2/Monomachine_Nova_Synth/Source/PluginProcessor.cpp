...
void MonomachineNovaAudioProcessor::getStateInformation(juce::MemoryBlock& out){
    ...
    { auto pl=juce::ValueTree("PLOCK"); // 1.7.1: слоты P-LOCK в состояние
        for(int s=0;s<kPlockSlots;++s){if(plockTarget(s)<0)continue;auto n=juce::ValueTree("S");n.setProperty("slot",s,nullptr); // 1.7.2: пустые слоты не хранимn.setProperty("target",plockTarget(s),nullptr);n.setProperty("pol",plockPolarity(s),nullptr);
