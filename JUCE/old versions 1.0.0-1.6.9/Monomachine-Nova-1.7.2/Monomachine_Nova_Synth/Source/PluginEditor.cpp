...
public:
    explicit ArpPage(MonomachineNovaAudioProcessor& p) : processor(p), lane(p, [this] { return pageDisplay(); }, [this](int pg, int st, const char* f, float v) { setStepParameter(pg, st, f, v); }), // 1.6.42: FOLLOW PLAY
        ...
        armBtn.setTooltip("P-LOCK: LMB = crosshair, then click a knob on the face panel -- the steps of the current page get that parameter (Elektron-style). Same knob = same slot, another knob = a new slot (unlimited). RMB = parameter list.");
        armBtn.onClick = [this] { if (armRequest) armRequest(curSlot()); };
        armBtn.rightClick = [this] { showDestList(); };
        slotBtn.setButtonText("S1"); slotBtn.textHeight = 12; addAndMakeVisible(slotBtn); // 1.7.2: слоты безграничны -- лейн один, список вызывает нужный, играют все
        slotBtn.setTooltip("P-LOCK slots: click = list of bound parameters (ALL of them keep playing). Pick one to edit its lane; a fresh slot binds the next P-LOCK aim.");
        slotBtn.onClick = [this] { showDestList(); };
        ...
        const std::pair<juce::Component*,const char*> layIds[]{{&arpEnable,"enable"},{&arpHold,"hold"},{&mode,"mode"},{&rateButton,"rate"},{&octavesBox,"oct"},{&gateBox,"gate"},{&hostTempo,"hosttempo"},{&wrapBox,"wrap"},{&velocityMode,"velmode"},{&pageRndButton,"rndpage"},{&pitchRndButton,"rndpitch"},{&velRndButton,"rndvel"},{&holdRndButton,"rndhold"},{&autoSwapToggle,"autoswap"},{&followPlayToggle,"follow"},{&pageBox,"page"},{&pageLimit,"pagelimit"},{&wrapBox,"wrap"},{&lane,"lane"},{&stepRndToggle,"steprnd"},{&plck,"plocklane"},{&plkRndButton,"rndplk"},{&slideRndButton,"rndslide"},{&armBtn,"plock"},{&polBtn,"plockpol"},{&slotBtn,"plockslot"}}; // 1.7.2: кнопка слота в LAYOUT EDIT // 1.7.1: +P-LOCK/STEP RND в LAYOUT EDIT
    ...
    void resized() override {
        ...
        slotBtn.setBounds(670, 312, 44, 26); // 1.7.2: список слотов
...
private:
    ...
    void showDestList() { // список параметров: автоматизированные (слоты) сверху с индикацией, остальные по папкам матрицы
        ...
        for (int s = 0; s < processor.kPlockSlots; ++s) { const int tg = processor.plockTarget(s); if (tg < 0) continue;
            m.addItem(100 + s, "S" + juce::String(s + 1) + " -- " + targetName(processor, tg), true, curSlot() == s); ++used; } // 1.7.2: слотов безгранично
        ...
        m.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&armBtn), [safe](int res) { if (!safe || res <= 0) return;
            if (res >= 100) { safe->setCurSlot(res - 100); return; } // выбор слота = показать его строку
    ...
    void setCurSlot(int s) { plockSlot = juce::jlimit(0, processor.kPlockSlots - 1, s);
        slotBtn.setButtonText("S" + juce::String(plockSlot + 1)); // 1.7.2
    ...
    std::unique_ptr<PlockDestBox> destBox; PixelButton slotBtn{"S1"}; int plockSlot = 0; // 1.7.2: слоты безграничны (32), играют все, лейн один
...
public:
    MsegPage(MonomachineNovaAudioProcessor& p,int startPage) : processor(p), page(juce::jlimit(0,7,startPage)) {
        ...
        retrig.setTooltip("RETRIG (this page): ON = every new note (incoming MIDI or the arp) restarts THIS page phase. OFF = the curve free-runs. Default ON.");
    ...
    void setPage(int m) {
        ...
        retrigLink = std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment>(processor.parameters, tag + "_retrig", retrig); // 1.7.2: RETRIG -- постраничная галка (как LOOP/SYNC)
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.7.2 / Synth":"BUILD 1.7.2 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
