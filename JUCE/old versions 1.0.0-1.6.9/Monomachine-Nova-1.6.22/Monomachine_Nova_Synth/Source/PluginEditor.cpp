...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.22 / Synth":"BUILD 1.6.22 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
        ...
        struct Picker final : juce::Component {
            ...
            void paint(juce::Graphics& g) override {
                ...
                for(int i=0;i<static_cast<int>(nova::machines().size());++i){
                    ...
                    if(i==hover){g.setColour(juce::Colour(0xff2f2f2f));g.fillRect(4,y-1,getWidth()-8,22);g.setColour(ink);g.fillRect(4,y-1,3,22);} // 1.6.22: тусклая подсветка + белый маркер слева
                    const juce::String cat(m.category);
                    int nameX=10; // 1.6.22: тип слева от имени одним словом (как в 1.6.20)
                    if(firstOfCat||i==hover){pixel::text(g,cat,{nameX,y,static_cast<int>(cat.length())*12+8,20},14,false);nameX+=static_cast<int>(cat.length())*12+8;}
                    pixel::text(g,shortName(i),{nameX,y,240,20},14,false);
