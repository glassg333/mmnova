...
public:
    ...
    inline void process(float delParam, float feedback, float send, bool pingPong, int ppMode,
        ...
        // 1.6.6: line-write topology copied verbatim from the old DSP delayWrite --
        // that is the send-knob law the user approved: ping-pong mono-mixes the input
        // into the left line (right line gets cross-feed only), stereo sends per side.
        const float fbL = pingPong ? aR : aL;
        const float fbR = pingPong ? aL : aR;
        left[static_cast<size_t>(write)] = (pingPong ? 0.5f * (inL + inR) : inL) * send + fbL * feedback;
        right[static_cast<size_t>(write)] = (pingPong ? 0.0f : inR) * send + fbR * feedback;
        write = (write + 1) % static_cast<size_t>(size);

        // 1.6.12: альтернативный ping-pong "MID SAFE". Классика (ppMode 0) даёт
        // узкое "всё в сайде" звучание: моно-вход пишется только в левую линию,
        // тапы прыгают между каналами. MID SAFE смешивает оба тапа в выходе
        // (65/35): возврат остаётся движущимся, но середина не вычитается.
        if(pingPong&&ppMode==1){
            outL=0.65f*aL+0.35f*aR;
            outR=0.65f*aR+0.35f*aL;
        }else{
            outL=aL;
            outR=aR;
        }
