...
public:
    ...
    void resized() override {
        ...
        retrig.setBounds(600, 70, 72, 26); // 1.7.4 FIX: было 150,70 -- галка сидела ПОД комбой SHAPE и была невидима
...
levelLink=std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(p.parameters,"level",level);
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.7.4 / Synth":"BUILD 1.7.4 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
