...
public:
    ...
    struct ArpSettings { Mode mode=Mode::Off; Play play=Play::Up; double speed=6; uint8_t range=1, noteLength=64, wrap=16; bool sync=true; double milliseconds=125; };
    ...
    void setSettings(const ArpSettings& s) { settings=s; settings.range=std::clamp<uint8_t>(s.range,1,4); settings.wrap=std::clamp<uint8_t>(s.wrap,1,16); }
    ...
    ArpNoteEvent poll() {
        ...
        const double duration=settings.sync?sampleRate*60.0/tempo/24.0*std::max(0.125,settings.speed):sampleRate*std::clamp(settings.milliseconds,5.0,2000.0)/1000.0;
        ...
        else if(settings.play==Play::Cycl && cycle>1) {size_t x=step%(2*cycle-2);index=x<cycle?x:2*cycle-2-x;}
        else index=step%cycle;
        ++step;
