...
public:
    ...
    ArpNoteEvent poll() {
        ...
        if (settings.play == Play::Step || settings.play == Play::StepChord) {
            ...
            if (settings.stepRandom > 0.0f) {
                rng ^= rng << 13; rng ^= rng >> 17; rng ^= rng << 5;
                const auto randomIndex = static_cast<size_t>(rng % stepCount);
                // 0 = ordered, 1 = completely random; blend by choosing random
                // only after the requested probability threshold.
                const float chance = static_cast<float>(rng & 0xffffu) / 65535.0f;
                if (chance < settings.stepRandom) stepIndex = randomIndex;
            }
            const auto& s = settings.steps[static_cast<size_t>(settings.stepPage) * 8 + stepIndex];
