...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    if(auto* loop=parameters.getRawParameterValue("m7_loop")){auto* start=parameters.getRawParameterValue("m7_loop_start");auto* end=parameters.getRawParameterValue("m7_loop_end");machine.bbox.setLoopParameters(start?start->load():0.0f,end?end->load():1.0f,loop->load()>0.5f);}
