...
MonomachineNovaAudioProcessor::MonomachineNovaAudioProcessor():AudioProcessor(isSynthVersion?
    ...
    const char* g[]={"level","bpm","host_sync","fx_mix","gate","arp_mode","arp_play","arp_speed","arp_range","arp_length","macro_x","macro_y","arp_on","arp_hold","arp_sync","arp_time","arp_grid"};
...
void MonomachineNovaAudioProcessor::prepareToPlay(double rate,int block){
    ...
    machine.prepare(sr);chain.volumeReference=isSynthVersion?127.0f:64.0f;chain.prepare(sr);midiGateBlend.reset(sr,0.005);envelope.reset(sr);arp.reset(sr);matrix.reset();
...
void MonomachineNovaAudioProcessor::snapshot(){
    ...
    settings.sync=global[ArpSync]>0.5f;settings.milliseconds=global[ArpTime];settings.range=static_cast<uint8_t>(global[ArpRange]);settings.noteLength=static_cast<uint8_t>(global[ArpLength]);
...
void MonomachineNovaAudioProcessor::panic(){
    ...
    envelope.reset(sr);dryDelay.clear();machine.clear();chain.clear();arp.reset(sr,bpm);matrix.reset();for(auto& l:lfos)l.reset();
}
...
void MonomachineNovaAudioProcessor::getStateInformation(juce::MemoryBlock& out){
    auto state=parameters.copyState();state.setProperty("schema",6,nullptr);auto samples=juce::ValueTree("Samples");
...
void MonomachineNovaAudioProcessor::setStateInformation(const void* data,int size){
    ...
    if(auto xml=getXmlFromBinary(data,size))if(xml->hasTagName(parameters.state.getType().toString())){
        ...
        state.setProperty("schema",6,nullptr);
