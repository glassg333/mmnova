...
def main() -> int:
    ...
    present(text, "BUILD 1.6.14", "1.6.14 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.14\"", "1.6.14 Editor: версия в About")
    ...
    present(text, "viewPos().y", "1.6.14 Editor: страница ARP скроллится драгом по пустому месту")
    present(text, "void mouseWheelMove(const juce::MouseEvent&,const juce::MouseWheelDetails&)override{}", "1.6.14 Editor: колесо страницу ARP не крутит")
