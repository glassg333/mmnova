    ...
    for(const auto& s:nova::specs()){
        if(s.choices.isNotEmpty())result.add(std::make_unique<juce::AudioParameterChoice>(juce::ParameterID(s.id,1),s.name,juce::StringArray::fromTokens(s.choices,"|",""),static_cast<int>(s.def)));
            ...
            juce::AudioParameterFloatAttributes()
                .withStringFromValueFunction([](float v,int){return juce::String(v < 0.0f ? -1 : 1) + juce::String(std::round(std::abs(v) * 100.0f / 64.0f), 0) + "%";})
                .withValueFromStringFunction([](const juce::String& text){return juce::jlimit(-64.0f,63.0f,text.retainCharacters("-0123456789").getFloatValue() * 64.0f / 100.0f);}))) ;
...
MonomachineNovaAudioProcessor::MonomachineNovaAudioProcessor():AudioProcessor(isSynthVersion?
    ...
    const char* g[]={"level","bpm","host_sync","fx_mix","gate","arp_mode","arp_play","arp_speed","arp_range","arp_length","macro_x","macro_y","arp_on","arp_hold","arp_sync","arp_time","arp_grid","arp_wrap","arp_velocity_mode","arp_step","arp_step_velocity","arp_step_transpose","arp_step_hold","mseg_rate","mseg_sync","mseg_loop"};
...
void MonomachineNovaAudioProcessor::prepareToPlay(double rate,int block){
    ...
    machine.prepare(sr);chain.volumeReference=isSynthVersion?127.0f:64.0f;chain.prepare(sr);midiGateBlend.reset(sr,0.005);envelope.reset(sr);arp.reset(sr);matrix.reset();mseg.reset();msegPhase=0.0;msegValue=0.0f;liveStepVelocity=0.0f;liveStepTranspose=0.0f;liveArpGate=0.0f;
...
void MonomachineNovaAudioProcessor::snapshot(){
    envelope.configure(juce::roundToInt(ampRaw[0]->load()),{ampRaw[1]->load(),ampRaw[2]->load(),ampRaw[3]->load()});
    ...
    for(size_t r=0;r<16;++r)matrix.configureRouting(r,routeRaw[r][0]->load()>0.5f,static_cast<monomachine::ModSource>(juce::jlimit(0,15,juce::roundToInt(routeRaw[r][1]->load()))),static_cast<uint8_t>(juce::jlimit(0,55,juce::roundToInt(routeRaw[r][2]->load()))),static_cast<int8_t>(juce::jlimit(-64,63,juce::roundToInt(routeRaw[r][3]->load()))),static_cast<monomachine::ModPolarity>(juce::jlimit(0,2,juce::roundToInt(routeRaw[r][4]->load()))));
...
void MonomachineNovaAudioProcessor::panic(){
    ...
    envelope.reset(sr);dryDelay.clear();machine.clear();chain.clear();arp.reset(sr,bpm);matrix.reset();msegPhase=0.0;msegValue=0.0f;liveStepVelocity=0.0f;liveStepTranspose=0.0f;liveArpGate=0.0f;for(auto& l:lfos)l.reset();
}
...
void MonomachineNovaAudioProcessor::getStateInformation(juce::MemoryBlock& out){
    auto state=parameters.copyState();state.setProperty("schema",7,nullptr);auto samples=juce::ValueTree("Samples");
