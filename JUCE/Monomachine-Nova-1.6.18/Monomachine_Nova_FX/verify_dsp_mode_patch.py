...
def main() -> int:
    ...
    present(text, "BUILD 1.6.18", "1.6.18 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.18\"", "1.6.18 Editor: версия в About")
    ...
    present(text, "speedReset", "1.6.15 Editor: кнопка RESET у сетки скорости")
    ...
    present(text, "else if (e.y < geo.yHold + geo.holdH) setParam(pg, st, \"hold\", 0.0f);", "1.6.16 Editor: ПКМ стирает только свой ряд (включая HOLD)")
    ...
    # ---- 20. обещания 1.6.18
    present(text, "if(modDragSrc<4){lfoHoldIndex=-1;return;}", "1.6.18 Editor: удержание переключает страницу и в ПКМ-режиме (матрица)")
