...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    ...
    void timerCallback()override{
        ...
        if(modDragSrc<4){lfoHoldIndex=-1;return;}
        ...
        if(over>=0&&over!=modDragSrc-4){
            ...
            else if(now-lfoHoldSince>=1000&&lfoPageSel!=over){
                lfoPageSel=juce::jlimit(0,2,over);bind();
            }
        } else lfoHoldIndex=-1;
    }
    void bind(){
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.18 / Synth":"BUILD 1.6.18 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
