...
public:
    ...
    void drawToggleButton(juce::Graphics& g, juce::ToggleButton& b, bool, bool) override { g.fillAll(juce::Colours::black); const int oy=(b.getHeight()-18)/2; g.setColour(ink); g.drawRect(2, oy, 18, 18, 1); if (b.getToggleState()) { g.drawLine(5, oy+9, 9, oy+13, 2); g.drawLine(9, oy+13, 16, oy+4, 2); } pixel::text(g, b.getButtonText(), {26, 0, b.getWidth() - 28, b.getHeight()}, 13, false); } // 1.6.25: галочка по центру высоты -- ровно с ARP/GATE
...
public:
    ...
    void mouseDown(const juce::MouseEvent&e)override{
        ...
        if(arrows&&arrowClick){const int end=getWidth()-56; // 1.6.24: полоса стрелок забронирована целиком
            if(e.x>=end){ // вся правая полоса -- стрелки, список не открыть
                arrowHl=e.x<end+24?0:1;arrowPressed=true;repaint(); // 1.6.25: каждая стрелка -- свой хитбокс и своя подсветка
    ...
    void mouseUp(const juce::MouseEvent&e)override{if(arrowPressed){arrowPressed=false;arrowHl=-1;repaint();}juce::TextButton::mouseUp(e);wasDragged=false;} // 1.6.25: подсветка стрелки гаснет
...
public:
    ...
    void setPickMode(bool enabled){pickMode=enabled&&modTarget;slider.setVisible(parameter&&!isChoice);repaint();} // 1.6.25: ручка не прячется при прицеле
...
public:
    ...
    void resized() override {
        ...
        hostTempo.setBounds(24, 110, 150, 28); // 1.6.16
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        for (int r = 0; r < 16; ++r) {
            ...
            for (int c = 1; c <= 5; ++c) {
                ...
                if (c == 4) { // 1.6.25: БЕЗ серой заливки -- только контур ячейки (видны границы) + белая полоса заполнения
    ...
    void openValueMenu(int r, int c, juce::Point<int> screen) { // 1.6.24: единый список значений ячейки
        ...
        else if (c == 2) { const int n = 18; const int selected = juce::roundToInt(par->convertFrom0to1(par->getValue())); for (int i = 0; i < n; ++i) menu.addItem(i + 1, sourceName(i), true, i == selected); } // 1.6.25: 18 источников = порядок ModSource
        const juce::Component::SafePointer<MatrixPage> safe(this);
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.27 / Synth":"BUILD 1.6.27 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
        ampButton.setBounds(473,95,120,28);ampModeCombo.setBounds(762,97,91,25); // 1.6.26: ровно над ПОСЛЕДНЕЙ крутилкой AMP (x=474+3*96)
        syntModeCombo.setBounds(363,97,91,25);filtModeCombo.setBounds(363,337,91,25);effxDlyCombo.setBounds(666,337,91,25);effxDistCombo.setBounds(762,337,91,25); // 1.6.26: MODE над ПОСЛЕДНЕЙ крутилкой страницы, второй мод -- над ПРЕДПОСЛЕДНЕЙ (EFFX)
    ...
    void showMachineMenu(){
        ...
        struct Picker final : juce::Component {
            ...
            explicit Picker(MonomachineNovaAudioProcessor& p,int current):processor(p),bound(current){
                ...
                for(int idx:order){colW=std::max(colW,static_cast<int>(juce::String(nova::machines()[static_cast<size_t>(idx)].category).length())*6);}
                colW=std::max(72,colW+20);
                setSize(std::min(640,colW+150),std::min<int>(464,static_cast<int>(order.size())*24+10)); setWantsKeyboardFocus(false); }
