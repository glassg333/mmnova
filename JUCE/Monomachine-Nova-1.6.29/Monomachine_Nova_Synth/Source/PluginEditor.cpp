...
static bool mapPostFlash=false; // 1.6.26: пост-анимация подтверждения мапинга -- выключена по умолчанию, включается галкой MAP FLASH (GUI OPTIONS)
...
public:
    ...
    void positionComboBoxText(juce::ComboBox& box, juce::Label& label) override { label.setBounds(2, 2, box.getWidth() - 6, box.getHeight() - 4); } // 1.6.25: текст режимов -- по центру всего окна
    ...
    void drawLinearSlider(juce::Graphics& g,int x,int y,int width,int height,float pos,float min,float max,const juce::Slider::SliderStyle style,juce::Slider& slider)override{
        if(style==juce::Slider::LinearVertical){g.setColour(ink);g.fillRect(x+width/2-6,juce::roundToInt(pos),12,std::max(1,y+height-juce::roundToInt(pos)));
            const auto rng=slider.getRange(); // 1.6.29: две ОДИНАКОВЫЕ узкие риски -- дефолт (100) и максимум
            if(rng.getLength()>0){const auto levTick=[&](float v){if(v>=rng.getStart()&&v<=rng.getEnd()){const int ty=juce::roundToInt(y+height*(rng.getEnd()-v)/static_cast<float>(rng.getLength()));g.setColour(juce::Colours::white);g.drawHorizontalLine(ty,x+width/2-8.0f,x+width/2+6.0f);}};
                levTick(100.0f);levTick(static_cast<float>(rng.getEnd()));}}
    ...
    void drawPopupMenuBackgroundWithOptions(juce::Graphics& g,int width,int height,const juce::PopupMenu::Options&)override{
        g.fillAll(juce::Colours::black);g.setColour(juce::Colours::white);g.drawRect(0,0,width-1,height-1); // 1.6.25: белая рамка ВСЕХ списков -- чёрный фон больше не сливается
    }
    ...
    void drawPopupMenuSectionHeaderWithOptions(juce::Graphics& g,const juce::Rectangle<int>& area,const juce::String& sectionName,const juce::PopupMenu::Options&)override{
        g.setColour(juce::Colours::black);g.fillRect(area.reduced(1,0));g.setColour(ink);pixel::text(g,sectionName,area.reduced(8,0),21,false); // 1.6.25: не затираем боковые кромки рамки
    }
...
public:
    ...
    void mouseDown(const juce::MouseEvent&e)override{
        ...
        if(arrows&&arrowClick){const int end=getWidth()-56; // 1.6.24: полоса стрелок забронирована целиком
...
public:
    ...
    void setPickMode(bool enabled,bool matrixPick=false){pickMode=enabled&&modTarget;pickMatrix=matrixPick&&pickMode;slider.setVisible(parameter&&!isChoice);slider.setInterceptsMouseClicks(!pickMode,false);repaint();} // 1.6.28: в прицеле ручка НЕ крутится (ЛКМ = мапить, Alt+ЛКМ = крутить)
    ...
    void flashPick(){if(!mapPostFlash)return;flashTicks=6;repaint();} // 1.6.26: пост-анимация (один бледно-белый мигок) -- только если включена MAP FLASH
    ...
    void paint(juce::Graphics& g)override{ // 1.6.25: как в прошлой версии -- всё содержимое видно, рамка только на текущей цели прицела, без мигания
        ...
        if(flashTicks>0){g.setColour(juce::Colours::white.withAlpha(0.40f));g.drawRect(getLocalBounds().reduced(2),3);} // 1.6.26: бледно-белый мигок подтверждения (если MAP FLASH включена)
    ...
    void mouseDown(const juce::MouseEvent&e)override{
        if(pickMode){ // 1.6.28: прицел -- ЛКМ только мапит; крутить -- с зажатым Alt
            ...
            if(pickMatrix&&parameter&&modTarget&&pickTarget)pickTarget(modulationTarget());
            return; }
...
private:
    ...
    int dragOriginY=0,dragBaseIndex=0;bool dragMoved=false,gestureOpen=false,popupHandled=false,pickMode=false,pickMatrix=false,hoverNow=false,altTurn=false;int altTurnX=0,altTurnY=0;float altTurnBase=0; // 1.6.28: Alt-кручение в прицеле
...
public:
    LfoLockPanel(MonomachineNovaAudioProcessor& p,int lfoIndex,bool isDest):processor(p),lfo(lfoIndex),dest(isDest){
        refreshNames(); // 1.6.25: имена теперь ЖИВЫЕ (пересчитываются в paint)
        setSize(220,30+8*24+34); startTimerHz(10); // 1.6.25: панель следит за свапом страниц
    }
    ~LfoLockPanel() override { stopTimer(); }
    void refreshNames(){ // 1.6.25: имена адресатов/страниц от ТЕКУЩЕЙ страницы LFO -- свап больше не оставляет панель на странице позади
        const int page=3+lfo;
        auto* knobPar=processor.parameters.getParameter(nova::pageParam(page,dest?1:0));
    ...
    void timerCallback() override { repaint(); } // 1.6.25: репаинт -- paint() пересчитывает имена от живой страницы
    void paint(juce::Graphics& g)override{
        refreshNames(); // 1.6.25: КАЖДЫЙ кадр от живой страницы
        g.fillAll(juce::Colours::black);g.setColour(ink);
...
public:
    GuiSpeedPanel(){
        ...
        mapFlash.setButtonText("MAP FLASH");mapFlash.textHeight=11;mapFlash.setClickingTogglesState(true);addAndMakeVisible(mapFlash); // 1.6.26
        mapFlash.setTooltip("MAP FLASH: after a modulation is dropped on a knob, the cell blinks pale-white ONCE. OFF (default) = no post animation.");
        mapFlash.setToggleState(mapPostFlash,juce::dontSendNotification);
        mapFlash.onClick=[]{mapPostFlash=!mapPostFlash;};
        setSize(1140,140);
    }
    ...
    void resized() override { reset.setBounds(1010,6,120,24); mapFlash.setBounds(845,6,155,24); // 1.6.26: галка пост-анимации мапинга
...
private:
    PixelButton reset{"RESET"};PixelButton mapFlash{"MAP FLASH"};std::vector<std::unique_ptr<PixelLabel>> labels;std::vector<std::unique_ptr<FloatBox>> boxes;
};
...
public:
    ...
    void paint(juce::Graphics& g) override {
        ...
        pixel::text(g, "MODE", {328, 48, 80, 20}, 14, true); pixel::text(g, "RATE", {518, 48, 80, 20}, 14, true); // 1.6.28: надпись PLAY убрана
        pixel::text(g, "OCT", {925, 48, 80, 20}, 14, true); pixel::text(g, "GATE", {995, 48, 80, 20}, 14, true);
        pixel::text(g, "WRAP", {24, 166, 80, 20}, 14, true); pixel::text(g, "VELOCITY", {210, 166, 110, 20}, 14, true);
        pixel::text(g, "PAGE", {24, 166+62, 80, 20}, 14, true); pixel::text(g, "PAGE LIMIT", {92, 166+62, 110, 20}, 14, true); // 1.6.25: подпись рядом с окном (окна вплотную)
        pixel::text(g, "RANDOM", {470, 166, 110, 20}, 14, true);
        pixel::text(g, "STEPS", {24, 294, 80, 20}, 15, true);
    }
    void resized() override {
        arpEnable.setBounds(20, 64, 135, 28); arpHold.setBounds(170, 64, 125, 28); mode.setBounds(328, 64, 170, 28); // 1.6.28: компактнее без PLAY
        rateButton.setBounds(518, 64, 250, 28); // 1.6.13: единая кнопка RATE в одном месте
        octavesBox.setBounds(925, 64, 56, 28); gateBox.setBounds(995, 64, 64, 28); // 1.6.16: GATE -- квадрат со значением
        hostTempo.setBounds(24, 98, 150, 28); // 1.6.16/28
        wrap.setBounds(24, 195, 160, 28); velocityMode.setBounds(210, 195, 110, 28);
        ...
        pageBox.setBounds(24, 238, 64, 28); pageLimit.setBounds(92, 238, 64, 28); // 1.6.25: PAGE и PAGE LIMIT вплотную
...
public:
    explicit MatrixPage(MonomachineNovaAudioProcessor& p) : processor(p) {
        ...
        clearButton.onClick=[this,wipeField]{for(const char* f:{"on","src","dest","depth","mode","lock","aux"})wipeField(f);repaint();}; // 1.6.21: ЛКМ = ВСЁ в дефолт (включая замки)
        clearButton.rightClick=[this,wipeField]{juce::PopupMenu m;m.setLookAndFeel(&getLookAndFeel());
            ...
            m.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&clearButton),[safe,this,wipeField](int res){if(!safe||res<=0)return;
                if(res==1)for(const char* f:{"on","src","dest","depth","mode","lock","aux"})wipeField(f);
    ...
    void paint(juce::Graphics& g) override {
        ...
        const char* headers[]{"", "ON", "SOURCE", "DESTINATION", "DEPTH", "MODE", "AUX SRC"}; // 1.6.24: AUX SOURCE
        for (int c = 0; c < 7; ++c) pixel::text(g, headers[c], {colX(c), 30, colWidth(c), 19}, 13, true);
        ...
        for (int r = 0; r < 16; ++r) {
            ...
            for (int c = 1; c <= 5; ++c) {
                ...
                if (c == 3) { auto* onp = cell(r, 1); if (value == 0 && !(onp && juce::roundToInt(onp->convertFrom0to1(onp->getValue())) > 0)) text = "---"; } // 1.6.28: не назначено -- прочерки
                ...
                pixel::text(g, text, {colX(c) + 5, y + 2, colWidth(c) - (c == 3 ? 58 : 8), 25}, c == 3 ? 14 : 16, c == 4); // 1.6.24: DEPTH -- по центру ячейки
            ...
            { // 1.6.24: AUX SOURCE -- вторичный источник масштабирует глубину (Sylenth-style)
                ...
                g.setColour(ink.withAlpha(0.25f)); g.drawVerticalLine(colX(6), float(y), float(y + 31));
    ...
    void mouseWheelMove(const juce::MouseEvent& e, const juce::MouseWheelDetails& w) override { const int r = rowAt(e.y), c = colAt(e.x); if (r < 0 || c < 1 || std::abs(w.deltaY) < 0.0001f) return; auto* p = c == 6 ? processor.parameters.getParameter("r" + juce::String(r) + "_aux") : cell(r, c); if (!p) return; const int old = juce::roundToInt(p->convertFrom0to1(p->getValue())); const int lo = c == 4 ? -64 : 0, hi = c == 4 ? 63 : (c == 5 ? 1 : c == 6 ? 18 : choiceCount(p) - 1); p->beginChangeGesture(); p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(juce::jlimit(lo, hi, old + (w.deltaY > 0 ? 1 : -1))))); p->endChangeGesture(); repaint(); } // 1.6.25: колесо работает и над AUX
    void mouseMove(const juce::MouseEvent& e) override { // 1.6.14: подсказка при наведении на замок
        const int r = rowAt(e.y); const bool lockZone = r >= 0 && e.x >= colX(3) + colWidth(3) - 56 && e.x < colX(3) + colWidth(3) - 28; // 1.6.25: хитбокс замка шире, в сторону списка DEST
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        ...
        if (c == 3 && e.x >= colX(c) + colWidth(c) - 56 && e.x < colX(c) + colWidth(c) - 28) { // 1.6.25: шире -- меню DEST больше не открывается при попытке замка
        ...
        if (c == 2 || c == 3 || c == 5 || c == 6) { // 1.6.25: клик = список, ЛКМ-драг = листать значение (конфликтов нет: список только если не тянули)
            hitWasRmb = e.mods.isPopupMenu();
            hitRow = r; hitCol = c; hitY = static_cast<int>(e.getScreenPosition().y); dragged = false;
            auto* par = c == 6 ? processor.parameters.getParameter("r" + juce::String(r) + "_aux") : cell(r, c);
    ...
    void mouseDoubleClick(const juce::MouseEvent& e) override { // 1.6.24: двойной ЛКМ = сброс в 0 (ПКМ списки НЕ сбрасывает)
        if (!e.mods.isLeftButtonDown()) return;
        const int r = rowAt(e.y), c = colAt(e.x); if (r < 0 || c < 4) return; auto* p = cell(r, c); if (!p) return;
        if (c == 4 || c == 5) { p->beginChangeGesture(); p->setValueNotifyingHost(p->convertTo0to1(0.0f)); p->endChangeGesture(); repaint(); } }
    void mouseDrag(const juce::MouseEvent& e) override {
        ...
        auto* p = hitCol == 6 ? processor.parameters.getParameter("r" + juce::String(hitRow) + "_aux") : cell(hitRow, hitCol); if (!p) return;
        if (hitCol == 4) { const int amount = e.mods.isShiftDown() ? dy / 10 : dy; p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(juce::jlimit(-64, 63, hitBase + amount)))); } // 1.6.25: DEPTH -- только ЛКМ
        else { const int hi = hitCol == 3 ? 55 : hitCol == 5 ? 1 : hitCol == 6 ? 18 : 17; p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(juce::jlimit(0, hi, hitBase + dy / 8)))); } // 1.6.25: SRC/DEST/MODE/AUX -- листание драгом
        repaint();
    }
    void mouseUp(const juce::MouseEvent& e) override {
        ...
        if (gesture) { if (auto* gp = hitCol == 6 ? processor.parameters.getParameter("r" + juce::String(hitRow) + "_aux") : cell(hitRow, hitCol)) gp->endChangeGesture(); gesture = false; } // 1.6.25: без дубля
        ...
        if (c == 2 || c == 3 || c == 5) openValueMenu(r, c, e.getScreenPosition()); // 1.6.25: SOURCE/DEST/MODE -- список по клику
    ...
    void openValueMenu(int r, int c, juce::Point<int> screen) { // 1.6.24: единый список значений ячейки
        ...
        if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; for (int i = 0; i < 9; ++i) menu.addItem(i + 1, signedPercent(values[i])); } // -100% ... 0% ... +100%
        ...
        else if (c == 3) { // 1.6.29: ПАПКИ-подменю как раньше (раскрытие -- дефолтное у JUCE, без дополнительной задержки); MSEG OUT -- после модулей
            ...
            juce::PopupMenu msegSub; // 1.6.29: страницы MSEG как DEST (56..63)
            for (int pg = 0; pg < processor.msegPageCount(); ++pg) msegSub.addItem(56 + pg + 1, "MSEG" + juce::String(pg + 1) + " OUT", true, 56 + pg == selected);
            menu.addSubMenu("MSEG", msegSub);
        }
        else if (c == 2) { // 1.6.28: RANDOM первым, папки MIDI/LFO/MSEG/ARP (плоско, без задержки подменю)
            static const int vals[]{17, 2, 3, 0, 1, 9, 8, 7, 4, 5, 6, 10, 11, 12, 18, 19, 20, 21, 22, 13, 14, 15, 16}; // 1.6.29: +MSEG4..8
            const int selected = juce::roundToInt(par->convertFrom0to1(par->getValue()));
            menu.addSectionHeader("RANDOM"); menu.addItem(1, sourceName(17), true, selected == 17);
            menu.addSectionHeader("MACRO"); menu.addItem(2, sourceName(2), true, selected == 2); menu.addItem(3, sourceName(3), true, selected == 3);
            menu.addSectionHeader("MIDI"); menu.addItem(4, sourceName(0), true, selected == 0); menu.addItem(5, sourceName(1), true, selected == 1); menu.addItem(6, sourceName(9), true, selected == 9); menu.addItem(7, sourceName(8), true, selected == 8); menu.addItem(8, sourceName(7), true, selected == 7);
            menu.addSectionHeader("LFO"); for (int i = 0; i < 3; ++i) menu.addItem(9 + i, sourceName(4 + i), true, selected == 4 + i);
            menu.addSectionHeader("MSEG"); // 1.6.29: страницы 1..8 (источники 10,11,12 и 18..22)
            for (int i = 0; i < 8; ++i) { const int sv = i < 3 ? 10 + i : 15 + i; menu.addItem(12 + i, sourceName(sv), true, selected == sv); }
            menu.addSectionHeader("ARP"); for (int i = 0; i < 4; ++i) menu.addItem(18 + i, sourceName(13 + i), true, selected == 13 + i);
        }
        const juce::Component::SafePointer<MatrixPage> safe(this);
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this).withTargetScreenArea(juce::Rectangle<int>(screen.x, screen.y, 1, 1)), [safe, r, c](int result) { if (!safe || result <= 0) return; auto* p = safe->cell(r, c); if (!p) return; if (c == 4) { const int values[]{-64, -48, -32, -16, 0, 16, 32, 48, 63}; p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(values[result - 1]))); } else if (c == 2) { static const int vals[]{17, 2, 3, 0, 1, 9, 8, 7, 4, 5, 6, 10, 11, 12, 18, 19, 20, 21, 22, 13, 14, 15, 16}; p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(vals[result - 1]))); } else p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(result - 1))); }); // 1.6.28: порядок списка SOURCE != порядка enum
    }
    void openAuxMenu(int r, juce::Point<int> screen) { // 1.6.24: AUX SOURCE -- второй источник решает, сколько модуляции пройдёт
        juce::PopupMenu m; m.setLookAndFeel(&getLookAndFeel());
        static const char* auxNames[]{"OFF", "KEY", "VEL", "MACRO X", "MACRO Y", "LFO1", "LFO2", "LFO3", "PITCH WHL", "MOD WHL", "AFTERTOUCH", "MSEG1", "MSEG2", "MSEG3", "STEP VEL", "STEP TRANS", "ARP GATE", "ARP RATE", "RANDOM", "MSEG4", "MSEG5", "MSEG6", "MSEG7", "MSEG8"}; // 1.6.25/29: OFF + порядок ModSource
        ...
        for (int i = 0; i < 24; ++i) m.addItem(i + 1, auxNames[i], true, i == cur);
        const juce::Component::SafePointer<MatrixPage> safe(this);
        m.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(this).withTargetScreenArea(juce::Rectangle<int>(screen.x, screen.y, 1, 1)), [safe, r](int result) { if (!safe || result <= 0) return; if (auto* p = safe->processor.parameters.getParameter("r" + juce::String(r) + "_aux")) { p->beginChangeGesture(); p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(result - 1))); p->endChangeGesture(); } });
    }
private:
    ...
    int colX(int c) const { const int x[]{8, 28, 78, 245, 580, 735, 935}; return x[juce::jlimit(0, 6, c)]; } // 1.6.24: + AUX
    int colWidth(int c) const { const int w[]{18, 42, 165, 330, 150, 190, 120}; return w[juce::jlimit(0, 6, c)]; }
    int rowY(int r) const { return 58 + r * 34; }
    int colAt(int x) const { for (int c = 0; c < 7; ++c) if (x >= colX(c) && x < colX(c) + colWidth(c)) return c; return -1; }
...
public:
    MsegPage(MonomachineNovaAudioProcessor& p,int startPage) : processor(p), page(juce::jlimit(0,2,startPage)) {
        ...
        grid.setTooltip("DRAW GRID: snapping for new points, drags and steps. Wheel changes it while you draw steps. ALT+1 coarser, ALT+2 finer, ALT+3 triplets, ALT+4 snap on/off. Hold Alt to bypass.");
        grid.onChange = [this] { if (linkBtn.getToggleState() && viewGrid.getSelectedItemIndex() != grid.getSelectedItemIndex()) { viewGrid.setSelectedItemIndex(grid.getSelectedItemIndex(), juce::dontSendNotification); } repaint(); };
        ...
        viewGrid.setSelectedItemIndex(3, juce::dontSendNotification); // 1/4 визуально
        ...
        linkBtn.setButtonText("LINK"); linkBtn.textHeight = 11; linkBtn.setClickingTogglesState(true); linkBtn.setToggleState(true, juce::dontSendNotification);
        ...
        clearButton.onClick = [this] { pushUndo(); processor.resetMseg(page); markCustom(); };
    ...
    void rebuildPageStrip() { // 1.6.29: кнопки страниц + «+»/«−»; драг листает, ПКМ открывает список
        pageButtons.clear(true);
        for (int m = 0; m < processor.msegPageCount(); ++m) { auto* b = pageButtons.add(new PixelButton{juce::String(m + 1)}); b->textHeight = 14; b->setClickingTogglesState(true);
            b->onClick = [this, m] { setPage(m); };
            b->verticalDrag = true; b->dragPixelsPerStep = 16; b->dragHandler = [this](int steps, bool) { if (steps != 0) setPage(page + steps); }; // LMB-драг -- список прокручивается
            b->rightClick = [this] { showPageList(); }; // RMB -- список страниц
            addAndMakeVisible(b); }
        addAndMakeVisible(addPageBtn); addAndMakeVisible(delPageBtn);
        addPageBtn.onClick = [this] { processor.addMsegPage(); rebuildPageStrip(); setPage(processor.msegPageCount() - 1); resized(); }; // новая страница сама появится в матрице
        delPageBtn.onClick = [this] { removePageWithConfirm(); };
        delPageBtn.setEnabled(processor.msegPageCount() > 1);
    }
    void showPageList() { // 1.6.29: RMB -- список страниц
        juce::PopupMenu m; for (int i = 0; i < processor.msegPageCount(); ++i) m.addItem(i + 1, "MSEG " + juce::String(i + 1), true, i == page);
        auto safe = juce::Component::SafePointer<MsegPage>(this);
        m.showMenuAsync(juce::PopupMenu::Options(), [safe](int res) { if (safe != nullptr && res > 0) safe->setPage(res - 1); });
    }
    ...
    void setPage(int m) {
        page = juce::jlimit(0, processor.msegPageCount() - 1, m); if (isShowing()) grabKeyboardFocus(); // 1.6.29: страницы 1..8
    ...
    void paint(juce::Graphics& g) override {
        ...
        pixel::text(g, "ROUTING / SIGNED AMOUNT (SORTED AND SIZED LIKE THE MATRIX)", {20, routeY() - 24, 760, 18}, 13, false);
        int row = 0;
        for (int r = 0; r < 16 && row < 8; ++r) { // 1.6.29: тот же порядок и размер строк, что в матрице
            ...
            if (mIdx < 0) continue;
            const int target = juce::jlimit(0, 63, juce::roundToInt(dst->load()));
        ...
        for (int rest = row; rest < 3; ++rest) pixel::text(g, juce::String(rest + 1) + "  UNASSIGNED", {20, routeY() + rest * 34, 330, 22}, 15, false);
        pixel::text(g, "DOUBLE CLICK: ADD / REMOVE   DRAG POINT: MOVE   DRAG LINE: BEND (EXPO)   RIGHT CLICK LINE: STRAIGHTEN   RIGHT CLICK POINT: REMOVE   ALT: BYPASS SNAP   ALT+1/2 GRID -/+   ALT+3 TRIPLET   ALT+4 SNAP   B DRAW", {20, getHeight() - 24, 1120, 17}, 12, false);
    }
    void resized() override {
        shape.setBounds(20, 40, 210, 26);
        for (int i = 0; i < pageButtons.size(); ++i) pageButtons.getUnchecked(i)->setBounds(240 + i * 28, 40, 24, 26); // 1.6.29: лента страниц
        addPageBtn.setBounds(240 + pageButtons.size() * 28 + 4, 40, 24, 26); delPageBtn.setBounds(240 + pageButtons.size() * 28 + 32, 40, 24, 26);
        stepsButton.setBounds(520, 40, 90, 26); grid.setBounds(618, 40, 108, 26); linkBtn.setBounds(730, 40, 54, 26); viewGrid.setBounds(788, 40, 108, 26);
        shift.setBounds(904, 40, 266, 26);
    ...
    void mouseDown(const juce::MouseEvent& e) override {
        for (int row = 0; row < 8; ++row) { const int rr = routeRows[static_cast<size_t>(row)]; if (rr < 0) continue;
            const int y = routeY() + row * 34;
            if (e.x >= 990 && e.x <= 1010 && e.y >= y && e.y <= y + 30) { if (auto* on = processor.parameters.getParameter("r" + juce::String(rr) + "_on")) { on->beginChangeGesture(); on->setValueNotifyingHost(on->convertTo0to1(0.0f)); on->endChangeGesture(); } repaint(); return; }
            if (e.x >= 393 && e.x <= 912 && e.y >= y - 3 && e.y <= y + 28) { routeDrag = rr; routeBase = processor.parameters.getRawParameterValue("r" + juce::String(rr) + "_depth")->load(); routeY0 = static_cast<int>(e.y); if (auto* d = processor.parameters.getParameter("r" + juce::String(rr) + "_depth")) d->beginChangeGesture(); return; } // 1.6.28: хитбокс ампаунта шире по всем сторонам
        }
        if (e.x < graphX || e.x > graphX + graphW || e.y < graphY || e.y > graphY + graphH) return;
        if (isShowing()) grabKeyboardFocus(); // 1.6.29: хоткеи сетки
        if (drawMode > 0 && !e.mods.isPopupMenu()) { // 1.6.29: рисование пером
            pushUndo();
            if (drawMode == 1) { stroke.clear(); stroke.push_back({toX(e.x), toY(e.y), 0.0f}); writeStroke(); } // штрих -- накапливаем точки
        ...
        if (e.mods.isPopupMenu()) { // 1.6.21: ПКМ по линии = сбросить наклон; по точке = удалить
            ...
            if (near > 0 && near < processor.msegPointCount(page) - 1) { pushUndo(); processor.removeMsegPoint(page, near); selected = -1; markCustom(); }
    ...
    void mouseDrag(const juce::MouseEvent& e) override {
        ...
        if (drawing && drawMode > 0) { // 1.6.22: перо
            if (drawMode == 1) { const float x = toX(e.x), y = toY(e.y); // 1.6.29: пишем ШТРИХ в любую сторону
    ...
    void mouseUp(const juce::MouseEvent&) override { if (draggingPoint) pushUndo(); draggingPoint = false; bendSeg = -1; if (drawing && drawMode == 1) writeStroke(); stroke.clear(); drawing = false; if (routeDrag >= 0) { if (auto* d = processor.parameters.getParameter("r" + juce::String(routeDrag) + "_depth")) d->endChangeGesture(); routeDrag = -1; } } // 1.6.29: штрих фиксируется на отпускании
    ...
    void mouseWheelMove(const juce::MouseEvent& e, const juce::MouseWheelDetails& w) override { // 1.6.29: колесо крутит сетку
        if (e.x < graphX || e.x > graphX + graphW || e.y < graphY || e.y > graphY + graphH || std::abs(w.deltaY) < 0.0001f) return;
        if (drawMode == 2 && drawing) gridStep(w.deltaY > 0 ? 1 : -1); // при рисовании степов -- размер сетки рисования
        else { const int idx = juce::jlimit(0, 11, viewGrid.getSelectedItemIndex() + (w.deltaY > 0 ? 1 : -1)); viewGrid.setSelectedItemIndex(idx, juce::dontSendNotification); if (linkBtn.getToggleState()) grid.setSelectedItemIndex(idx, juce::dontSendNotification); repaint(); } }
private:
    ...
    bool keyPressed(const juce::KeyPress& key) override { // 1.6.29: хоткеи как в Ableton
        if (key.getModifiers().isAltDown() && key.getKeyCode() == '1') { gridStep(-1); return true; }
        if (key.getModifiers().isAltDown() && key.getKeyCode() == '2') { gridStep(1); return true; }
        if (key.getModifiers().isAltDown() && key.getKeyCode() == '3') { gridTriplet(); return true; }
        if (key.getModifiers().isAltDown() && key.getKeyCode() == '4') { gridSnapToggle(); return true; }
        if (!(key.getModifiers().isAltDown() || key.getModifiers().isCtrlDown() || key.getModifiers().isCommandDown()) && (key.getKeyCode() == 'B' || key.getKeyCode() == 'b')) {
            drawMode = drawMode == 0 ? 1 : 0; syncDrawButtons(); if (drawMode == 1) { pushUndo(); startFreehand(); } repaint(); return true; }
    ...
    void writeStroke() { auto pts = stroke; if (pts.empty()) return; // 1.6.29: сортируем по X, прореживаем, держим концы
        ...
        if (thin.front().x > 0.02f) thin.insert(thin.begin(), {0.0f, thin.front().y, 0.0f});
        if (thin.back().x < 0.98f) thin.push_back({1.0f, thin.back().y, 0.0f});
        thin.back().x = 1.0f; thin.front().x = 0.0f;
        if (static_cast<int>(thin.size()) > 32) { std::vector<Pt> kept; for (int i = 0; i < 32; ++i) kept.push_back(thin[static_cast<size_t>(i * (static_cast<int>(thin.size()) - 1) / 31)]); kept.back().x = 1.0f; thin.swap(kept); }
        if (thin.size() < 2) return; processor.setMsegPointCount(page, static_cast<int>(thin.size()));
        for (size_t i = 0; i < thin.size(); ++i) processor.setMsegPoint(page, static_cast<int>(i), thin[i].x, thin[i].y); }
    std::vector<Pt> stroke;
    void startStairs() { const auto pts = snapshot(); for (int c = 0; c < 16; ++c) stepLevels[static_cast<size_t>(c)] = sampled(pts, (static_cast<float>(c) + 0.5f) / 16.0f); rebuildStairs(); selected = -1; repaint(); } // 1.6.29: вход не сбрасывает нарисованное
    ...
    juce::ToggleButton loop, sync; juce::Slider rate, shift; WheelCombo shape, grid, viewGrid; PixelButton linkBtn{"LINK"}; // 1.6.22/29
    int straightIdx = 5, snapSavedIdx = 5; std::vector<Pt> customBackup; bool lastShapeCustom = true; // 1.6.29: память CUSTOM-волны
    PixelButton stepsButton{"STEPS"}; PixelButton clearButton{"clear"}; PixelButton addPageBtn{"+"}, delPageBtn{"-"}; juce::OwnedArray<PixelButton> formButtons, toolButtons, pageButtons; // 1.6.29: страницы списком
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    explicit Surface(MonomachineNovaAudioProcessor& p):processor(p),machineButton(""),menuButton("MENU"),tempoButton(""),arpButton("ARP"),gateOuter(p,"arp_length",1,127,64),matrixButton("MATRIX"),lfo1Button("1"),lfo2Button("2"),lfo3Button("3"),msego1("1"),msego2("2"),msego3("3"),closeButton("BACK"),gateButton(""){
        ...
        addAndMakeVisible(ampButton);ampButton.setButtonText("AMP");ampButton.textHeight=20;ampButton.textAlign=1;ampButton.onClick=[this]{showEnvelope(-1);}; // 1.6.29: шрифт категории как у остальных
    ...
    void bind(){
        boundMachine=processor.machineIndex();updateDspModeLabel();const auto& m=nova::machines()[static_cast<size_t>(boundMachine)];machineButton.setButtonText(m.name);machineButton.textHeight=28; // 1.6.13
    ...
    void paint(juce::Graphics& g)override{
        ...
        pixel::dotted(g,8,94,8,570);pixel::dotted(g,61,94,61,570);pixel::dotted(g,8,94,63,94);pixel::dotted(g,8,570,63,570); // 1.6.29: низ пунктира = низ секций
        ...
        for(int panel=0;panel<6;++panel){int x=72+(panel%3)*399,y=94+(panel/3)*240;
            g.setColour(ink.withAlpha(0.35f));g.drawRect(x,y,387,236,1);g.setColour(ink);g.drawRect(x,y,387,30,2); // 1.6.28: название категории -- кнопка (текст рисует она)
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.29 / Synth":"BUILD 1.6.29 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
        ampButton.setBounds(471,95,94,28);ampModeCombo.setBounds(762,97,92,25); // 1.6.28: кнопка AMP от линии панели до края первой крутилки; MODE 92 -- почти вплотную к пунктиру
        syntCatButton.setBounds(72,95,94,28);lfoCatButton.setBounds(870,95,94,28);filtCatButton.setBounds(72,335,94,28);effxCatButton.setBounds(471,335,94,28);msegCatButton.setBounds(870,335,94,28); // 1.6.28: кнопки категорий
        syntModeCombo.setBounds(363,97,92,25);filtModeCombo.setBounds(363,337,92,25);effxDlyCombo.setBounds(666,337,92,25);effxDistCombo.setBounds(762,337,92,25); // 1.6.28: 92 px -- правый край у линии конца страницы
        ...
        arpEnabled.setBounds(471,58,26,30);arpButton.setBounds(499,58,120,30);gateOuter.setBounds(623,58,34,30);trigToggle.setBounds(661,58,105,30);matrixButton.setBounds(870,58,140,30); // 1.6.29: ARP прилегает к галке, шаги ARP->GATE->TRIG сохранены
    ...
    void cycleMachine(int steps){
        ...
        p->beginChangeGesture();p->setValueNotifyingHost(p->convertTo0to1(static_cast<float>(raw)));p->endChangeGesture();bind();machineButton.flashFor(160); // 1.6.29: строка мигает -- пресет сменился и отжался
    }
    ...
    void showMachineMenu(){
        ...
        struct Picker final : juce::Component {
            MonomachineNovaAudioProcessor& processor;std::vector<int> order;int colW=0;int bound=-1,hover=-1;std::function<void(int)> onPick;
            ...
            void mouseDown(const juce::MouseEvent& e) override { const int i=(e.y-5)/24; if(i>=0&&i<static_cast<int>(order.size())&&onPick){onPick(order[static_cast<size_t>(i)]);if(auto* o=findParentComponentOfClass<juce::CallOutBox>())o->exitModalState(0);} }
        ...
        juce::CallOutBox::launchAsynchronously(std::move(panel),juce::Rectangle<int>(machineButton.getScreenX()+2,machineButton.getScreenBounds().getBottom()+6,1,1),this); // 1.6.29: окно ПОД строкой машины, в пределах окна плагина
    }
