...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        juce::String targets;
        for(int i=0;i<64;++i) {if(i) targets+="|";targets+=i<8?"SYN "+juce::String(i+1):i<56?juce::String(sections[(i-8)/8])+" "+pageLabel((i-8)/8,(i-8)%8):"MSEG"+juce::String(i-55)+" OUT";} // 1.6.29: +MSEG1..8 OUT (56..63)
        for(int row=0;row<16;++row) {
            ...
            s.push_back({prefix+"dest","Route target",0,63,0,1,targets}); // 1.6.22: дефолт цели = 0; 1.6.29: 64 цели
