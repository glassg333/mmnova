...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        // BBOX sample loop: two normalized handles are intentionally separate from the machine's eight hardware slots.
        s.push_back({"m7_loop","BBOX loop",0,1,0,1,"OFF|ON"});
        s.push_back({"m7_loop_start","BBOX loop start",0,1,0,0.001f,{}});
        s.push_back({"m7_loop_end","BBOX loop end",0,1,1,0.001f,{}});
