...
public:
    ...
    void paintButton(juce::Graphics& g,bool /*over*/,bool down)override{
        g.fillAll(down&&outlined?ink:(grayBlink&&blinkPhase()?juce::Colour(0xff3a3a3a):juce::Colours::black));
...
private:
    ...
    void applyErase(const juce::MouseEvent& e, const Geo& geo) {
        ...
        if (y >= geo.yTr && y < geo.yVel) setParam(pg, st, "transpose", 0.0f);
        else if (y >= geo.yVel && y <= geo.yVel + geo.pad) setParam(pg, st, "velocity", 0.0f);
        else if (y >= geo.yHold && y < geo.yHold + geo.holdH) setParam(pg, st, "hold", 0.0f);
        repaint();
    }
...
struct MonomachineNovaAudioProcessorEditor::Surface final : juce::Component, public juce::DragAndDropContainer, private juce::Timer {
    explicit Surface(MonomachineNovaAudioProcessor& p):processor(p),machineButton(""),menuButton("MENU"),tempoButton(""),arpButton("ARP"),arpSettingsButton("SET"),matrixButton("MATRIX"),lfo1Button("LFO1"),lfo2Button("LFO2"),lfo3Button("LFO3"),msegButton("MSEG"),msegOutputButton("MSEG OUT"),closeButton("BACK"),gateButton(""){
        ...
        auto wireLfo=[this](ModSourceButton& b,int i){
            ...
            b.setTooltip("LFO"+juce::String(i+1)+": click = show this LFO panel; LEFT-drag onto a knob = this LFO modulates it directly (its own PAGE/DEST, no matrix); RIGHT-drag onto a knob = create a matrix route. While aiming: hold over another LFO button for 1 s (LMB or RMB) to switch to its page, then drop on any knob of that page.");
        };
    ...
    void timerCallback()override{
        ...
        if(modDragSrc<4){lfoHoldIndex=-1;lfo1Button.grayBlink=lfo2Button.grayBlink=lfo3Button.grayBlink=false;return;}
        ...
        lfo1Button.grayBlink=(modDragSrc-4!=0&&lfoPageSel==0);
        lfo2Button.grayBlink=(modDragSrc-4!=1&&lfoPageSel==1);
        lfo3Button.grayBlink=(modDragSrc-4!=2&&lfoPageSel==2);
        if(over>=0&&over!=modDragSrc-4&&lfoPageSel!=over)lfoPageSel=juce::jlimit(0,2,over);
    ...
    void beginModDrag(int src,bool matrix){modDragSrc=src;modDragMatrix=matrix;modHover=nullptr;lfoHoldIndex=-1;for(auto& column:cells)for(auto& cell:column)if(cell)cell->setPickMode(true);}
    ...
    void paint(juce::Graphics& g)override{
        ...
        g.setColour(ink.withAlpha(0.65f));g.drawText(processor.isSynthVersion?"BUILD 1.6.19 / Synth":"BUILD 1.6.19 / FX",850,570,370,13,juce::Justification::right); // версия сборки видна всегда
    }
    void resized()override{
        ...
        struct Picker final : juce::Component {
            ...
            void paint(juce::Graphics& g) override {
                ...
                for(int i=0;i<static_cast<int>(nova::machines().size());++i){
                    ...
                    pixel::text(g,shortName(i),{10,y,230,20},14,true);
                    if(lastCategory!=juce::String(m.category)){ // тип -- напротив первого генератора категории
                        pixel::text(g,juce::String(m.category),{250,y,174,20},14,i!=hover);
