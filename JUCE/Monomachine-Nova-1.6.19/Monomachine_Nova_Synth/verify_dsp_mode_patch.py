...
def main() -> int:
    ...
    present(text, "BUILD 1.6.19", "1.6.19 Editor: маркер версии сборки (обновляется +1 каждым патчем)")
    present(text, "BUILD 1.6.19\"", "1.6.19 Editor: версия в About")
    ...
    present(text, "HOLD: LMB click/drag = paint holds", "1.6.16 Editor: подсказка управления при наведении на степы")
    present(text, "else if (y >= geo.yHold && y < geo.yHold + geo.holdH) setParam(pg, st, \"hold\", 0.0f);", "1.6.16/1.6.19 Editor: ПКМ стирает только свой ряд (полосы applyErase явные)")
    ...
    present(text, "if(over>=0&&over!=modDragSrc-4&&lfoPageSel!=over)lfoPageSel=juce::jlimit(0,2,over);", "1.6.19 Editor: прицел переключает страницу МГНОВЕННО (без 1000 мс задержки)")
