...
inline const std::vector<Spec>& specs() {
    static const std::vector<Spec> all=[] {
        ...
        for(const auto& m:machines()) for(int i=0;i<8;++i) {
            ...
            juce::String choices;
            if(m.id==7 && i==2) choices="SAMP|OSC";
            if(m.id==7 && i==6) choices="OFF|ON";
        ...
        const int defaults[6][8]={{0,0,64,64,64,NOVA_SYNTH?64:127,64,0},{0,127,0,0,0,93,64,64},{64,64,0,64,64,28,0,127},
        ...
        for(int page=0;page<6;++page) for(int i=0;i<8;++i) {
            ...
            if(page>=3) {
                ...
                if(i==3) choices="TRI|SAW|SQR|PUL|S&H";
        ...
        s.push_back({"level","Master level",0,127,NOVA_SYNTH?64:100,1,{}});
        ...
        if(!NOVA_SYNTH) s.push_back({"gate","FX trigger: free running or MIDI note gated",0,1,0,1,"FREE|MIDI"});
