...
void MonomachineNovaAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
  ...
  // Добавь эти две строчки:
    ...
    arpeggiator.reset(sampleRate, 120.0);
...
void YourAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // 1. Обработка входящих MIDI сообщений с клавиатуры
    for (const auto metadata : midiMessages)
    {
        auto msg = metadata.getMessage();
        if (msg.isNoteOn())
        {
            // Нажали ноту: запускаем осциллятор FM и огибающие фильтра/усилителя
        ...
        {
            // Отпустили ноту: переводим в Release
            fmSynth.noteOff();
            voiceChain.noteOff();
    ...
    // 2. Генерируем звук FM-осциллятора
    int numSamples = buffer.getNumSamples();
    auto* channelLeft  = buffer.getWritePointer(0);
    auto* channelRight = buffer.getWritePointer(1);

    // Сначала FM+ Dynamic генерирует чистый звук:
    fmSynth.processStereo(channelLeft, channelRight, numSamples);

    // 3. Пропускаем его через всю цепочку Monomachine: Overdrive -> Filter -> EQ -> Amp -> Delay
    voiceChain.processBlock(channelLeft, channelRight, channelLeft, channelRight, numSamples);
}
