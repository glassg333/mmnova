...
public:
    ...
    // ВНИМАНИЕ: firmware (P:147798) outputs OLD state (state BEFORE update),
    // так что возвращаем s_ ДО обновления. Это совпадает с behavior хоруса.
    inline float processLP(float x) {
        float out = s_;                       // OLD state = output (as in firmware)
        s_ = one_minus_a_ * s_ + a_ * x;        // update state
        return out;
    }

    // HP-режим: y[i] = x[i] - state (из той же формулы, но subtraction)
    // (как FX-REV HP: один и тот же 1-pole используется и для HP, и для LP,
    //  разница — формула вывода). Firmware также outputs OLD state (pre-update).
    inline float processHP(float x) {
        float out = x - s_;                    // OLD state used for output
        s_ = one_minus_a_ * s_ + a_ * x;        // update state
        return out;
    }
