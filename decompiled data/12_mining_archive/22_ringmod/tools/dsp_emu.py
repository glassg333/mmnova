    ...
    def do_moves(self, toks):
        ...
        for tk in toks:
            ...
            if src.startswith("#"):
                ...
                # 8-bit immediate to accumulator: HW stores it in bits 31..24
                # (A1 high byte) -> A1 = imm<<16. 24-bit form prints '#>...'/'#<...'.
                if dst in ("a", "b") and src.startswith("#$") \
    ...
    def alu_exec(self, mnem, toks, pc):
        ...
        if mnem in ("add", "sub", "subr", "addr", "addc", "sbc", "addl", "subl",
            ...
            if mnem == "add":
                ...
                # CLB S,D: count leading bits equal to the MSB of S.
                # For accumulator sources the count is over the 24-bit A1 field
                # (matches the firmware normalize-to-[0.5,1) idiom).
                sv = self.get_reg(s)
                if s in ("a", "b"):
                    val = (sv >> 24) & MASK24
                else:
                ...
                for i in range(23, -1, -1):
                    if ((val >> i) & 1) == msb:
                        ...
                        break
                self.set_reg(d, c)
                self.f["n"] = 1 if sext(c, 56) < 0 else 0
                self.f["z"] = 1 if c == 0 else 0
            elif mnem == "normf":
                # NORMF S,D: D = D shifted arithmetically by (S − 1).
                # Firmware idiom: clb a,b; normf b1,a -> mantissa in [0.5,1).
                sh = (self.get_reg(s) & MASK24) - 1
                dv = self.get_reg(d) & MASK56
                if sh >= 0:
                    res = dv << sh
                else:
                    res = sext(dv, 56) >> (-sh)
                self.set_reg(d, sext(res & MASK56, 56))
            elif mnem == "insert":
                raise EmuError("unimplemented alu insert at %06X" % pc)
            return
