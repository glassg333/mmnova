...
class DSP56300:
    ...
    def acc_part_for_dst(self, src_name, sval, limit=True):
        ...
        DSP56300 Data Limit Checking (FM §5.4.1.2): transfers of A/B to memory
        or X0/X1/Y0/Y1 SATURATE to $7FFFFF / $800000 when the 56-bit value
        exceeds the 24-bit range. Explicit part transfers (a0/a1/a2) bypass
        the limit checker. On real HW this is always enabled."""
        if src_name in ("a", "b"):
            # normalize to signed 56-bit (guards against raw bit-pattern regs)
            v = ((sval + (1 << 55)) & ((1 << 56) - 1)) - (1 << 55)
            if limit:
                if v > 0x007FFFFFFFFFFF:         # $7FFFFF.FFFFFF max in-range
                    return 0x7FFFFF
                if v < -0x00800000000000:        # −$800000.000000 min in-range
                    return 0x800000
            return (v >> 24) & MASK24
    ...
    def move_imm(self, src, dst):
        """Immediate source value for a MOVE instruction.
        DSP56300 8-bit immediate form (#$xx, no > / < marker):
          - destination A/B (full accumulators): RAW imm8 (kernel div idiom
            P:$07A0 'move #$8,a; 24x div y0,a' needs dividend = 8);
          - destination R/M/N: RAW imm8 (e.g. 'move #$10,n4' LFO step);
          - other destinations (x0/x1/y0/y1/parts/memory): LEFT-JUSTIFIED
            (xx<<16) — e.g. 'move #$40,y1' = $400000 = 0.5 in the reverb HP
            filter (the 0.5*(1-c) weight confirms the alignment).
        Long form (#>) and forced-short (#<) are raw values."""
        v = self.abs_val(src)
        m = re.match(r"^#\$([0-9a-fA-F]{1,2})$", src)
        if m and dst not in ("a", "b") \
                and not re.match(r"^(r\d|n\d|m\d)$", dst):
    ...
    def upd_r(self, rnum, delta):
        ...
        if m == MASK24:
            ...
            return
        # DSP56300 modulo: buffer length = m+1, base aligned (base & m == 0).
        # Works for NON-power-of-two lengths too (boundary-crossing rule):
        # the pointer walks base..base+m linearly and wraps at the edges.
        # (Old formula ((r&m)+delta)%(m+1) only handled pow-2 masks.)
        base = r & ~m
        off = (r - base) + delta
        if off > m:
            off -= m + 1
    ...
    def exec(self, mnem, toks, pc):
        ...
        if mnem == "jsr":
            ...
            self.ret_stack.append(self.next_addr.get(pc, pc + 1))
            return tgt
        if mnem == "bsr":
            self.ret_stack.append(self.next_addr.get(pc, pc + 1))
            return self.branch_target(toks[0])
            ...
            if re.match(r"^[ab]\d?$", dst) or dst in ("x0", "x1", "y0", "y1", "x", "y") \
               or re.match(r"^(r\d|n\d|m\d)$", dst):
                if dst in ("a", "b") and srckind == "meml":
                    ...
                    # R/N targets bypass the limit checker (FM §5.4.1.2)
                    sval = self.acc_part_for_dst(src, sval, limit=False)
    ...
    def alu_exec(self, mnem, toks, pc):
        ...
        if mnem in ("add", "sub", "subr", "addr", "addc", "sbc", "addl", "subl",
            ...
            if mnem == "add":
                ...
                # CLB S,D: count leading bits of 24-bit S incl. sign bit.
                # Accumulator source -> operate on the MSW (a1/b1).
                v = self.get_reg(s)
                if s in ("a", "b"):
                    v = (v >> 24) & MASK24
                else:
                ...
                for bit in range(22, -1, -1):
                    if ((v >> bit) & 1) == sign:
                        ...
                        break
                self.set_reg(d, cnt & MASK24)
                self.f["n"] = 0
                self.f["z"] = 1 if cnt == 0 else 0
            elif mnem == "normf":
                # NORMF S,D: D <- D shifted LEFT by (S-1); S is 24-bit signed
                # (MSW for accumulators). Negative -> arithmetic right shift.
                sv = self.get_reg(s)
                if s in ("a", "b"):
                    sv = (sv >> 24) & MASK24
                else:
                ...
                r = sext(r, 56)
                self.set_reg(d, r)
                self.f["n"] = 1 if r < 0 else 0
                self.f["z"] = 1 if (r & MASK56) == 0 else 0
